...
| 结论 | 数量 | 用例 |
|---|---|---|
| **已实质解开**（撤回 daily + 补纪律 / 补 harness） | 5 | D6-1/D6-3/D6-4（压测，探针已存在）、D9-6（跨客户端，有历史结论）、D10-3/EXP-E（路由评测，run-eval.mjs 落地） |
| **待真云拆分**（免费/低频可跑，高危才挂起） | 2 | D3-C4/EXP-C4、D2-1 |
| **待只读子账号/审计**（免费可建） | 2 | D4-13、D4-14 |
| **真·外部依赖**（需 LLM Agent harness，才配挂起） | 2 | D10-2、D10-5 |
...
## 四、D4-13 / D4-14 只读子账号与审计

- **D4-13（最小权限凭证通过率）**：创建只读 IAM 子账号（免费）→ 用只读 AK/SK 跑 D3 只读用例 → 断言「只读 100% 可用、写被正确识别为权限不足」。
- **D4-14（操作可审计性）**：CTS 云审计免费 → 真云只读 + 写操作后查 CTS 记录 → 断言「agent/人工可区分 + 不含明文凭证」。
