...
## 期望结果（精确断言）
<插件应表现的行为>（引用依据：【仓】safety-model.md 第X条 / 工具描述 / README 承诺；写成**唯一可判定断言**——精确错误码/字段/返回值，避免"正常""合理"等模糊词）

## 实际结果
<实际行为 + 证据（回显/日志/截图）>

## 根因定位（文件 + 行号）
<源码层定位：出问题函数 + 文件 + 行号，如 `risk-rule-engine.mjs:23`；未定位到则写"待定位"并给线索（方法见技能 huaweicloud-devkit-source-coverage：源码文件地图 + 核对四步）>
