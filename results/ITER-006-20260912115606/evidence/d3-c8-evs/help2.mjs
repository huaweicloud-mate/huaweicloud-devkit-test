...
try {
  await send('initialize', { protocolVersion: '2024-11-05', clientInfo: { name: 'evshelp', version: '1' } });
  await send('notifications/initialized', {});
  const r = await send('tools/call', { name: 'huaweicloud_run_readonly_command', arguments: { args: ['EVS', 'CreateVolume', '--help'] } });
  const t = textOf(r);
  const lines = t.split('\n').filter((l) => /required|availability|zone|volume.type|volume.size|--[a-z]/i.test(l));
