#!/usr/bin/env node
// D3-C8 发现：企业项目列表 + EVS CreateVolume 必填参数
...
try {
  await send('initialize', { protocolVersion: '2024-11-05', clientInfo: { name: 'd3c8d', version: '1' } });
  await send('notifications/initialized', {});
  // 1) 企业项目列表
  const ep = await send('tools/call', { name: 'huaweicloud_run_readonly_command', arguments: { args: ['EPS', 'ListEnterpriseProject'] } });
  const ept = textOf(ep);
  console.log('===== EPS 企业项目 =====');
  console.log(ept.slice(0, 500).replace(/\n/g, ' '));
  // 2) EVS CreateVolume 必填参数
  const ev = await send('tools/call', { name: 'huaweicloud_run_readonly_command', arguments: { args: ['EVS', 'CreateVolume', '--help'] } });
  const evt = textOf(ev);
  const lines = evt.split('\n').filter((l) => /required|availability|volume.type|volume.size|enterprise_project|--[a-z]/i.test(l));
  console.log('===== EVS CreateVolume 参数(节选) =====');
