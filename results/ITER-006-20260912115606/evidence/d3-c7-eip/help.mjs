#!/usr/bin/env node
// 发现 VPC CreatePublicip 必填参数（--help）
...
try {
  await send('initialize', { protocolVersion: '2024-11-05', clientInfo: { name: 'help', version: '1' } });
  await send('notifications/initialized', {});
  const r = await send('tools/call', { name: 'huaweicloud_run_readonly_command', arguments: { args: ['VPC', 'CreatePublicip', '--help'] } });
  const t = textOf(r);
  // 只打印含 required / -- 参数 的行
  const lines = t.split('\n').filter((l) => /required|--*[a-z]|cli-region|publicip|bandwidth/i.test(l));
