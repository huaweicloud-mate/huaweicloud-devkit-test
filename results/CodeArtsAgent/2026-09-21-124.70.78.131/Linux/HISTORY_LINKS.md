...
- 今日证据：`evidence/D4-2/stdout.log`、`evidence/_probes/d4-security-probe.mjs`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
- 今日证据：`evidence/D4-16/stdout.log`、`evidence/_probes/wrap-probe.mjs`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#761](https://github.com/huaweicloud/huaweicloud-devkit/issues/761)（open）**[测试报告] huaweicloud-devkit v1.1.5 每日测试缺陷合并单（OfficeAce Windows 2026-09-20，8+11 项）**
    - 历史单内容：## 每日测试缺陷合并单 — OfficeAce/Windows/2026-09-20 ### 基本信息 - **客户端**: OfficeAce - **操作系统**: Windows (AMD64) - **被测版本**: huaweicloud-devkit@1.1.5 - **测试日期**: 2026-09-20 - **设计级**: 100 条 (92 PASS, 8 FAIL) - *
...
- 今日证据：`evidence/D10-3/stdout.log`、`evidence/D3-S1/stdout.log`、`evidence/D3-S5/stdout.log`、`eval/results/eval-run-*.csv`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
- 今日证据：`evidence/D4-27/stdout.log`、`evidence/D4-26/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#761](https://github.com/huaweicloud/huaweicloud-devkit/issues/761)（open）**[测试报告] huaweicloud-devkit v1.1.5 每日测试缺陷合并单（OfficeAce Windows 2026-09-20，8+11 项）**
    - 历史单内容：## 每日测试缺陷合并单 — OfficeAce/Windows/2026-09-20 ### 基本信息 - **客户端**: OfficeAce - **操作系统**: Windows (AMD64) - **被测版本**: huaweicloud-devkit@1.1.5 - **测试日期**: 2026-09-20 - **设计级**: 100 条 (92 PASS, 8 FAIL) - *
  - [#752](https://github.com/huaweicloud/huaweicloud-devkit/issues/752)（open）**[测试报告] huaweicloud-devkit 1.1.5 每日测试缺陷合并单（9 项，AtomCode/Linux）**
...
## D4-29 classifyRawCommand 未导出（SPEC 契约漂移）
- 今日证据：`evidence/D4-29/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#755](https://github.com/huaweicloud/huaweicloud-devkit/issues/755)（open）**[测试报告] huaweicloud-devkit v1.1.5 每日测试缺陷合并单（1 项）**
    - 历史单内容：## 测试概览 - 被测版本：v1.1.5 - 缺陷：1 项 ## 缺陷清单 ### 14. [P1] D4-29 classifyRawCommand 未导出（本轮新增 SPEC 漂移） - **描述**：设计用例声明 `safety-policy.classifyRawCommand(=classifyTextCommand 包装)`，但源码 `safety-policy.mjs` 仅导出 `

## D9-8 / D9-9 SPEC 契约漂移（schema 版本 / cancellation 未声明）
- 今日证据：`evidence/D9-8/stdout.log`、`evidence/D9-9/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
