...
> **落盘路径**：`results/CodeArtsAgent/2026-09-17-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：2026-09-17 07:14:00（北京时间）
> **被测对象**：huaweicloud-devkit npm latest v1.1.5（gitHead e7ed6f6）
> **本轮结论**：全部缺陷为历史 issue 复现或延续，无本轮新增产品缺陷；另 1 项【非产品缺陷】环境阻塞（D4-13 只读子账号权限）。

---

## #1【P0】D4-2 凭证 env 打印拦截不完整（hook 层）

- **现象**：`hook_check_command("printenv HW_ACCESS_KEY")`（evaluateCommandRisk 层）返回 `{decision:allow, findings:[]}`；源码 safety-policy 层 `classifyTextCommand` 已返回 `deny`（#650 已修），分层修复不完整。
...
- **现象**：源码 npm 1.1.5 `safety/policy.json` writeOperationPrefixes 33 项含 `Apply`（classifyHcloudArgs `EIP ApplyEip` 返回 deny/write 正确）；但框架运行时 `~/.codeartsdoer/huaweicloud-plugins/safety/policy.json` 仅 32 项缺 `Apply`。
...
- **现象**：`huaweicloud-core/SKILL.md` 会话启动节指引调用 `huaweicloud_check_update`/`huaweicloud_upgrade`，框架运行时未暴露该 2 工具。
...
- **现象**：`hook_check_command("hcloud ecs CreateServers --adminPass xxx")` 返回 `allow`、`findings:[]`，无明文密码告警。
...
- **现象**：`evaluateCommandRisk("")` 与畸形输入返回 `allow`（fail-open）。
...
- **证据**：`evidence/D10-3/stdout.txt`、`eval/results/eval-run-20260916230005.csv`
- **状态**：上一轮 #11 延续

## #11【非产品缺陷】D4-13 只读子账号具备写权限（环境/账号权限异常）

- **现象**：只读子账号（AK HPUA3XUK）只读 API 100% 可用（VPC ListVpcs/ECS ListServersDetails 成功），但 `CreateVpc` 写操作成功创建 VPC（未被 IAM 拒绝，违反最小权限）；已归零删除 VPC。
- **说明**：非 DevKit 代码缺陷——run-as-readonly 仅注入 env 凭证，权限由 IAM 决定；疑似 readonly 组配置过宽，需维护者核查 test001 权限，不计入提单。
