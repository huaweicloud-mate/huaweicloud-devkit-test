...
> **执行归档**：`results/CodeArtsAgent/2026-09-19-124.70.78.131/Linux/`
...
> **执行方法**：探针脚本（.mjs）直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数 + spawn mcp-server.mjs 驱动 JSON-RPC；CLI 真机（install/doctor/status/uninstall/update/install-hcloud）隔离 HOME 执行；真云 E2E（OBS 建删归零 + 只读子账号只读/写权限 + CTS 审计 + auth 三端同步）真机执行。证据统一落 `evidence/<case-id>/` + `evidence/_probes/*.log`。
...
| 已执行（非 BLOCKED/NOT_RUN） | 113 |
| PASS / FAIL / BLOCKED / SPEC-MISMATCH / NOT_RUN | 87 / 21 / 6 / 4 / 1 |
| 通过率（分母 = PASS+FAIL+SPEC-MISMATCH） | 77.7%（87/112） |
| P0 / P1 / P2 新增缺陷 | 0（全为历史 #673/#685/#650/#726 复现 + D9-9/D10-3 延续） |
| 红线（I 类）违规 | 0 |
| 资源释放 | 本轮新增真云写资源：OBS 桶（建→删归零）+ 只读子账号 VPC 探测（未创建，配额超限），无残留 |
...
| 状态 | 数量 | 说明 |
|---|---|---|
| PASS | 61 | 含真云 OBS 建删归零、CTS 审计、auth 三端同步、D2-26 备份恢复 |
| FAIL | 10 | 全部历史复现（#673/#685/#650/#726）+ D9-2/D10-3 延续 |
| BLOCKED | 5 | D1-2（多客户端共存）/ D4-13（只读子账号 IAM 权限过宽）/ D7-4（国内镜像）/ D9-6（跨客户端）/ D10-4（LLM harness） |
| SPEC-MISMATCH | 3 | D5-3、D9-1（工具枚举 40 vs 37）、D9-9（超时/取消未声明） |
| NOT_RUN | 1 | D1-39（Windows 专属，Linux 由 NR3 disttags 展开级覆盖，OS 列标注「专属」豁免） |
| **合计** | **80** | |
...
> 本轮无新增产品缺陷。10 条设计级 FAIL + 11 条展开级 FAIL + 4 条 SPEC，全部为历史 issue 复现或延续，详见 FINDINGS.md。
...
| 7 | P1 | D4-6 | hook 层明文密码无告警（母版源码级脱敏断言已 PASS） | #673 |
...
| 11 | P1 | D10-3 | serviceCatalog 中文路由 MISS 21.4% | 上轮 #11 延续 |
...
| D4-13 | 设计级 | P1 | BLOCKED | 补环境 | 只读子账号 test001 只读 6/6 可用，但写 CreateVpc 返回配额超限(VPC.0114)而非 IAM 权限拒绝（只读组权限过宽）；已真机执行并归零核实 | 需维护者核查 readonly 组权限 |
...
- [x] 红线（I 类）违规：无
- [x] 脱敏复核：D2-4 show_profile_redacted 无原始凭证落盘；D4-27 裸 token= 未脱敏已记 #726（对象路径安全关键字无残留）
- [x] 真云资源红线：OBS 桶建→删归零；只读子账号 VPC 探测未创建（配额超限），无残留

## 七、资源释放

| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| OBS 桶（testbot2-ca-obs-*） | 是 | 已删 | Delete bucket successfully + 404 NoSuchBucket |
| 只读子账号 VPC（tctest-ro-probe-*） | 否（配额超限未创建） | — | ListVpcs 归零核实 created=false |
| 隔离 HOME 临时目录 | 是（探针用） | 已 rm | 无残留 |

## 八、遗留与建议

- 本轮沿用 D4-6-probe 口径：D4-6 母版源码级脱敏断言（adminPass=xxx→<redacted>）PASS，hook 层明文密码无告警作为独立缺陷记入 FINDINGS #7（历史 #673）。
- D4-13 只读子账号 test001 权限过宽（写操作被配额超限而非 IAM 拒绝），需维护者核查 readonly 组权限后复测。
- v1.1.5 修复验证：①D4-2 safety-policy 层 HW_ 已拦截，但 hook 层 cloud-risk-rules.json 仍缺 HW_（分层修复不完整）；②D4-16 shell-wrap hook 层已有效（bash -c/sudo 内层 DeleteServer warn）；③D9-2 -32601 已修、-32602 未修。
- 建议：cloud-risk-rules.json env-dump 规则补 HW_ 前缀；框架运行时 policy.json + MCP 暴露集同步源码（40 工具/33 项 Apply 前缀）；tools.mjs routeMap 补全 CJK 中文关键词；safety-policy.mjs redactString 补裸 token / --admin-pass 关键字（#726）。
