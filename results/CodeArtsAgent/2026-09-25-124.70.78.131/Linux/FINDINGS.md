...
> **落盘路径**：`results/CodeArtsAgent/2026-09-25-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：2026-09-25（北京时间）
> **被测对象**：huaweicloud-devkit npm latest `v1.1.7`（gitHead `7456d059`）
> **本轮结论**：设计级 FAIL 14 + SPEC-MISMATCH 1，展开级 FAIL 12（EXP-E 与 D10-3 同源路由 MISS）。全部为历史 issue 复现/延续（1.1.7-next.1→1.1.7 未修复；D9-12 首测同源缺陷已由历史 #699/#774 跟踪）；**D4-26/D4-27 裸 token 脱敏在 1.1.7 已修复（今日转 PASS）**。
...
## #6【P0】D4-3 kms DecryptData 明文 secret API 未拦截
...
## #9【P2】D8-1 文档与能力漂移（声明 39 tools，实现 40）
...
## #10【P2】D8-9 sanitizeValue 不脱敏敏感值
...
## #11【P1】D9-9 tools/call 取消语义未声明（SPEC）
...
## #12【P0】D9-12 initialize 时序未强制（未 initialize 先 tools/list 未返回 -32600）

- **现象**：spawn mcp-server 后不发 initialize，直接发 `tools/list`，服务端正常返回 40 工具列表（`errorCode=null`，未拒绝），未按 JSON-RPC 时序返回 -32600。
- **断言**：非法时序（未 initialize 先 tools/list）应被拒并返回 -32600。
- **根因**：`plugins/huaweicloud-core/src/mcp-server.mjs:166` handleMessage 不维护 initialize 状态机（仅对非对象消息返回 -32600）；`plugins/huaweicloud-core/src/mcp-protocol.mjs:57` tools/list 无条件返回，无 initialize 时序校验。
- **影响**：协议客户端可跳过握手直接枚举工具，破坏 MCP 初始化时序契约。
- **证据**：`evidence/D9-12/stdout.log`
- **状态**：历史 #699/#774 复现（MCP 未强制 initialize 时序；今日新增 P0 用例 D9-12 首测同源缺陷）
