...
// D1-65/66/68 env vars + telemetry
{
  ...
  // telemetry DEBUG: only 'true' accepted (known defect D1-65)
  add('D1-65', 'telemetry DEBUG=1 亦开启(契约)', false, 'telemetry.mjs:81 仅认 "true"，不认 "1"');
...
// ============ D2 认证 (source-level) ============
{
  ...
  add('D2-4', '小写 ak=/sk= 脱敏', lower.includes('<redacted>'), lower); // known FAIL
}

// D2-5 missing-cred guidance
{
  add('D2-5', '缺失凭证报错含指引(HDKIT_CRED_MISSING)', true, 'resolveCredentials 抛 HDKIT_CRED_MISSING + onboarding.steps');
}
...
// ============ D4 安全 ============
{
  ...
  // D4-24 确认令牌 (double consume + tamper expiry)
  {
    ...
    try {
      ...
      // structured-code contract check
      add('D4-24', '返回结构化 {code:CONFIRM_TOKEN_EXPIRED}', false, 'consumeApprovalToken 返回 null(无结构化 code)');
    } finally { uniso(); }
  }
  // D4-25 Python hook 遥测分类 (source-level: python file regex)
  {
    ...
    add('D4-25', '写→cli:write 分类正确(契约)', false, 'WRITE_OPERATION_RE 前缀锚定错误(空格前漏判)');
  }
  // D4-26 findings 脱敏
  {
...
// ============ D9 协议 ============
{
  ...
  add('D9-9', '取消能力按 capabilities 实测', hasCancel === true ? false : false, 'capabilities=' + JSON.stringify(init.capabilities));
  setStatus('D9-9', 'SPEC-MISMATCH', 'initialize capabilities 未声明 notifications.cancellation（mcp-protocol.mjs:63-65 仅 {tools:{}}）；未知方法/超时无 -32000 timeout 分支（mcp-server.mjs 统一 -32603 兜底）');
