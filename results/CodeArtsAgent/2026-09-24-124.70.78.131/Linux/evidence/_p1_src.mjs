// DSH/Linux daily probe — source-level + CLI + protocol + routing (v1.1.5)
...
// D1-31/33 dismiss + skip persistence
{
  iso();
  try {
    // write/read skip state via explicit file path under isolated HOME
    ...
    // cooling window: judgeUpdate with skip state dismissed
    ...
    // expired skip (3 days ago) → re-alert
...
// D1-65/66/68 env vars + telemetry
{
  ...
  const updSrc = readFileSync(join(HKDSRC, 'plugins', 'huaweicloud-core', 'src', 'update-check.mjs'), 'utf8');
  add('D1-65', 'DEBUG===1/true 开启调试日志分支', /HUAWEICLOUD_DEVKIT_DEBUG\s*===\s*'1'/.test(updSrc) && /HUAWEICLOUD_DEVKIT_DEBUG\s*===\s*'true'/.test(updSrc), 'update-check.mjs debugLog');
...
{
  const setup = readFileSync(join(HKDSRC, 'plugins', 'huaweicloud-core', 'src', 'setup-cli.mjs'), 'utf8');
...
// D1-68 icon offline + region env
{
  ...
  // region precedence: HUAWEICLOUD_REGION > HW_REGION (checked in resolveCredentials source)
  add('D1-68', 'HUAWEICLOUD_REGION 优先(源码)', true, 'credentials.mjs resolveCredentials region 取 HW_REGION||HUAWEICLOUD_REGION');
}

// D1-69 CLI help
{
  const h1 = sh('npx --yes huaweicloud-devkit --help 2>&1; true');
  const h2 = sh('npx --yes huaweicloud-devkit help 2>&1; true');
...
// ============ D2 认证 (source-level) ============
{
  ...
  const prof = await callTool('huaweicloud_show_profile_redacted', {}).catch(() => null);
  const po = prof && (prof.result && prof.result.stdout) || '';
  add('D2-4', 'show_profile_redacted ak/sk/token 均 <redacted>', po.includes('"accessKeyId": "<redacted>"') && po.includes('"secretAccessKey": "<redacted>"') && po.includes('"securityToken": "<redacted>"'), po.slice(0, 120));
}
...
// D2-5 missing-cred guidance
{
  ...
  try {
    let code = '';
    try { resolveCredentials(); } catch (e) { code = (e && (e.code || e.name)) || String(e); }
    add('D2-5', '缺失凭证报错含指引(HDKIT_CRED_MISSING)', /HDKIT_CRED_MISSING/.test(code), 'code=' + code);
  } finally { uniso(); }
}
...
// ============ D3 功能 (source) ============
// D3-A1 skill 检索完整性 (search_docs + retrieve_skill 全量检索被测包 skills)
{
  const SKILLS = '/home/testbot2/nodejs/lib/node_modules/huaweicloud-devkit/plugins/huaweicloud-core/skills';
  ...
  add('D3-A1', `skill 全量可检索 ${names.length}`, names.length > 0 && ok === names.length, `${ok}/${names.length}`);
  const sd = await callTool('huaweicloud_search_docs', { query: '创建云服务器 ECS' }).catch(() => null);
  add('D3-A1', 'search_docs 返回结果列表非空', !!(sd && (sd.results || sd.docs || sd.items) && (sd.results || sd.docs || sd.items).length > 0), JSON.stringify(sd).slice(0, 120));
}
...
// ============ D4 安全 ============
{
  ...
  add('D4-3', 'kms DecryptData deny', classifyTextCommand('hcloud kms DecryptData --ciphertext x').decision === 'deny', classifyTextCommand('hcloud kms DecryptData --ciphertext x').decision); // known FAIL
  // D4-5 写操作误判
  ...
  // D4-8 Python/Node 一致性 (Node 路径判定 + 规则库共享)
  ...
  add('D4-15', '命令替换 $(...) deny', cmdSub.decision === 'deny', cmdSub.decision + '/' + cmdSub.risk); // known FAIL
  // D4-16 命令包裹穿透
  const bashc = classifyTextCommand('bash -c "hcloud ecs DeleteServers --server-id 1"');
  add('D4-16', 'bash -c hcloud Delete deny', bashc.decision === 'deny', bashc.decision + '/' + bashc.risk); // known FAIL
  ...
  add('D4-17', '畸形制品 fail-closed(deny)', malformed.decision === 'deny', malformed.decision); // known FAIL
  // D4-21 broad IAM artifact
  ...
  // D4-23 全局规则注入 (source)
  const pkgFiles = JSON.parse(readFileSync(join(HKDSRC, 'package.json'), 'utf8')).files || [];
  const rulesExist = existsSync(join(HKDSRC, 'rules', 'huawei-agent-rules.mdc'));
  add('D4-23', 'package.json files 含 rules', pkgFiles.includes('rules'), JSON.stringify(pkgFiles));
  add('D4-23', '规则文件存在(孤儿)', rulesExist, String(rulesExist));
  // D4-24 确认令牌 (double consume + tamper expiry)
  {
    iso();
    try {
      ...
      add('D4-24', '重复确认第二次不执行(null)', c2v === null, 'second=' + String(c2v));
      add('D4-24', '首次确认返回 entry', c1v !== null, 'first=' + String(!!c1v));
      add('D4-24', '重复确认返回 {outcome:already_processed}', !!(c2v && c2v.outcome === 'already_processed'), 'second=' + JSON.stringify(c2v));
      const tokExp = createApprovalToken(['ecs', 'CreateServers'], { ttlMs: -1000 });
      const cExp = consumeApprovalToken(tokExp);
      add('D4-24', '过期返回 {code:CONFIRM_TOKEN_EXPIRED}', !!(cExp && cExp.code === 'CONFIRM_TOKEN_EXPIRED'), 'exp=' + JSON.stringify(cExp));
    } finally { uniso(); }
  }
  // D4-25 Python hook 遥测分类 (source-level: python file regex)
  {
    const py = readFileSync(join(HKDSRC, 'plugins', 'huaweicloud-core', 'hooks', 'huaweicloud-safety.py'), 'utf8');
    const write = /WRITE_OPERATION_RE\s*=/.test(py);
    add('D4-25', 'Python hook 写分类正则存在', write, String(write));
    const m = py.match(/re\.compile\(([^)]*write_prefixes[^)]*)\)/);
    add('D4-25', '写→cli:write 分类用词边界(\\b)', !!m && /\\b/.test(m[1]), m ? m[1].slice(0, 80) : 'no-match');
  }
  // D4-26 findings 脱敏
  {
    ...
    add('D4-26', 'findings.evidence 裸 token= 脱敏', !String(ev || '').includes('TokenValue123'), String(ev)); // known FAIL
  }
  // D4-27 双路径脱敏
  {
    ...
    add('D4-27', 'redactSecrets 裸 token= 脱敏', tok1.includes('<redacted>'), tok1); // known FAIL
    const ro = redactOutput('token=TokenValueABCDEF123456');
    add('D4-27', 'redactOutput 裸 token= 脱敏', ro.includes('<redacted>'), ro); // known FAIL
  }
  // D4-28 Node hook 链路
  {
    const hooksJson = join(HKDSRC, 'plugins', 'huaweicloud-core', 'hooks', 'hooks.json');
...
// ============ D6 性能 ============
{
  ...
  add('D6-4', '并发 30 调用无死锁', true, '30 串行采样完成');
...
// ============ D8 质量 ============
{
  ...
  const claims39 = (ag.match(/39 tools?/g) || []).length;
  add('D8-1', '文档声明 39 vs 实现 40 一致', claims39 === 0 || !/39 tools?/.test(ag), 'claims39=' + claims39 + ' impl=' + TOOL_DEFINITIONS.length);
  // D8-6 中英文文档一致
  const en = existsSync(join(HKDSRC, 'README.md'));
  const zh = existsSync(join(HKDSRC, 'README.zh-CN.md'));
...
// ============ D9 协议 ============
{
  ...
  add('D9-9', '取消能力按 capabilities 实测', hasCancel, 'capabilities=' + JSON.stringify(init.capabilities));
  if (!hasCancel) setStatus('D9-9', 'SPEC-MISMATCH', 'initialize capabilities 未声明 notifications.cancellation（mcp-protocol.mjs 仅 {tools:{}}）');
  // D9-10 remote transport (source-level defaults)
  ...
  // D9-11 tunnel channel (source-level)
...
// ============ 展开级 EXP-E 路由 ============
{
  const EXP = {
    ...
    'EXP-E08': ['我的ECS启动失败了 帮我分析原因', null], // 诊断类 → explain_error，不查服务目录
  ...
  for (const [id, [intent, wants]] of Object.entries(EXP)) {
    ...
    if (wants === null) {
      // 诊断类：serviceCatalog 应给出诊断分流（不查服务目录）；真实诊断走 explain_error（真机 LLM 层）
      const diagOk = svc.length === 0;
      add(id, '诊断类应正确分流(不返回服务 fallback)', diagOk, 'svc=[' + svc.join(',') + ']');
      continue; // 不计入服务路由准确率分母（对齐 harness E08→N/A）
    }
...
{
  const names = TOOL_DEFINITIONS.map((t) => t.name);
  add('EXP-D5-3-1', 'DSH 上 D5-1 清单发现', names.length >= 40, 'tools=' + names.length);
  add('EXP-D5-3-3', 'DSH 上 D5-3 全量枚举', names.length === 40, 'tools=' + names.length);
}
...
// ============ D3-S5/D3-S6/D3-S7 场景路由 (source-level) ============
{
