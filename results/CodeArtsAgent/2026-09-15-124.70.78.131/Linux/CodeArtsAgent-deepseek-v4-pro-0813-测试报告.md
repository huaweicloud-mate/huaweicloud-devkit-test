...
| 状态 | 数量 | 说明 |
|---|---|---|
| PASS | 23 | EXP-D5-3-1 + EXP-C4-01~22（服务矩阵只读规划冒烟） |
| FAIL | 1 | EXP-D5-3-3（tools/list 37 vs 40，与 D5-3/D9-1/D1-26 同源） |
...
|---|---|---|---|---|---|---|---|---|
| 1 | P0 | D4-5 | framework Apply* 写误判（版本漂移） | `EIP ApplyEip` 应 `deny/write` | `allow/unknown_read` | `~/.codeartsdoer/huaweicloud-plugins/safety/policy.json`（缺 Apply，源码 policy.json:39 已含） | 新增 | 待提单 |
...
| 3 | P0 | D4-15 | hook 命令替换+ANSI-C 绕过 | `$(echo $'E\x43S DeleteServer')` 应 `deny` | `allow` | `risk-rule-engine.mjs:50-54`（纯正文本匹配） | 复现 #673 | 已提单 |
| 4 | P0 | D4-23 | 全局规则未注入（孤儿文件） | 11 目标注入且无孤儿 | `.mdc` 存在但零代码引用 | `rules/huawei-agent-rules.mdc` | 复现 #673 | 已提单 |
| 5 | P0 | D8-7 | 7 meta 技能指引断链 | 指引均可机械执行 | 指引引用未暴露的 check_update/upgrade | huaweicloud-core SKILL.md 会话启动节 → 与 D1-26 同源 | 复现 #673 | 已提单 |
| 6 | P1 | D1-26 | 工具协议暴露漂移 40 vs 37 | check_update/upgrade 应可调 | 3 工具未暴露 | `tools.mjs`（框架未暴露 check_update/upgrade/obs_set_website_config） | 复现 #673 | 已提单 |
...
> D5-3/D9-1/EXP-D5-3-3（工具枚举 40 vs 37 漂移）与 #6（D1-26）同源，不单列。

### 根因详情（关键 P0）

**#1 [P0] D4-5 framework Apply* 写误判（版本漂移，本轮新增）**
...
- 待裁决：D5-3/D9-1（工具枚举 37 vs 40，需维护者判定框架层过滤是否为设计意图）；D4-23（规则注入契约 .md vs .mdc）
