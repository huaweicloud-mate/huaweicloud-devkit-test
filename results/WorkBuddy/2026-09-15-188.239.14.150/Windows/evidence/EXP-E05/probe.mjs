...
try {
  const r = await callTool('huaweicloud_service_catalog', { intent: '看一下我的云数据库MySQL实例的状态' });
  const text = JSON.stringify(r);
  console.log('Routing result:', text.substring(0, 300));
  // Check if the expected service is mentioned
  const expectSvc = 'RDS'.toLowerCase();
  const hasService = text.toLowerCase().includes(expectSvc.split('/')[0]);
  console.log('Expected service:', 'RDS', 'Found:', hasService);
