...
> **执行归档**：`results/WorkBuddy/2026-09-17-188.239.14.150/Windows/`
...
> **执行方法**：探针脚本（.mjs）直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数；CLI 真机执行记录日志；MCP 工具通过 Agent 会话直接调用；eval harness 直调 serviceCatalog。证据统一落 `evidence/<case-id>/`。
...
| 已执行 | `95`（PASS+FAIL+SPEC-MISMATCH+BLOCKED） |
| PASS / FAIL / BLOCKED / SPEC-MISMATCH / NOT_RUN | `73 / 21 / 18 / 1 / 4` |
| 通过率（分母 = PASS+FAIL+SPEC-MISMATCH = 95） | `77.9%` |
| P0 / P1 / P2 新增缺陷 | `4 / 6 / 1` |
| 红线（I 类）违规 | `0` |
| 资源释放 | `全部归零（未创建真云资源）` |
...
| FAIL | `8` | D1-39, D1-45, D4-2, D4-9, D4-15, D4-22, D6-3, D10-3 |
| BLOCKED | `18` | 环境阻塞（需真云写操作/隔离HOME/多profile等） |
...
| 4 | P0 | `D4-15` | Deleteserver 变体未被拦截 | `decision=deny/warn` | `decision=allow` | `tools.mjs:1104` evaluateCommandRisk | P | 待提单 |
| 5 | P1 | `D10-3` | serviceCatalog 路由准确率仅 21.4% | ≥90% | 21.4% | `tools.mjs:1776` serviceCatalog | P | 待提单 |
| 6 | P1 | `D1-45` | applyUpdateHint 未设置 _updateInfo | `_updateInfo` 存在 | undefined | `update-check.mjs:361` | P | 待提单 |
| 7 | P1 | `EXP-C4-14` | DMS list_operations 不支持 | 返回操作列表 | `[USE_ERROR]` | KooCLI 服务名映射 | P | 待提单 |
| 8 | P1 | `EXP-C4-18` | DEW list_operations 不支持 | 返回操作列表 | `[USE_ERROR]` | KooCLI 服务名映射 | P | 待提单 |
| 9 | P1 | `D4-22` | hook_check_deploy_plan 未检测公网暴露（同#2） | `decision=warn/deny` | `decision=allow` | `risk-rule-engine.mjs:119` | P | 待提单 |
| 10 | P2 | `D6-3` | MCP冷启探针未捕获stdout | coldStart<5s | 未捕获 | 探针未发送initialize | P | 待提单 |
...
**#2 [P0] D4-9/D4-22 hook_check_deploy_plan 未检测公网暴露**
...
**#4 [P0] D4-15 Deleteserver 变体未被拦截**
- 期望：`hook_check_command("Deleteserver --instance-id xxx")` → `deny/warn`
- 实际：`allow`
- 根因：`tools.mjs:1104` — `evaluateCommandRisk` 仅匹配 `hcloud` 前缀命令的写动词，未检测裸 delete 关键词。

**#5 [P1] D10-3 serviceCatalog 路由准确率 21.4%**
- 期望：中文意图路由准确率 ≥90%
- 实际：3/14 HIT = 21.4%
...
- 实际：`undefined`
- 根因：`update-check.mjs:361` — `applyUpdateHint` 函数未正确设置 `result._updateInfo`。
...
| D2-5 | 设计级 | P1 | BLOCKED | 补环境 | 需构造无凭证/错误凭证/过期凭证环境 | — |
...
| D6-3 | 设计级 | P2 | FAIL | — | MCP冷启探针未捕获stdout（已记缺陷） | — |
...
- 建议：
  1. D4-2/D4-15 修复：`tools.mjs:1104` 应改用 `classifyTextCommand` 或在 `evaluateCommandRisk` 中增加 env/printenv/delete 关键词规则
  2. D4-9/D4-22 修复：`risk-rule-engine.mjs` deploy plan 规则应匹配 `publicIP` 字段
  3. D10-3 修复：serviceCatalog 中文意图匹配需大幅增强，当前 3/14 HIT
  4. D1-39 修复：Windows `spawnSync('npm.cmd', ...)` 需加 `shell:true` 或使用 `queryDistTagsFetch` fallback
  5. D1-45 修复：`applyUpdateHint` 应正确设置 `result._updateInfo`
  6. EXP-C4-14/EXP-C4-18：DMS/DEW 服务名在 KooCLI 中不支持，需映射到正确服务名
