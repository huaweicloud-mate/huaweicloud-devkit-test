...
## D1-39 Windows 升级检测链失效（parseDistTagsOutput 丢弃 npm 数组输出致 check_update 恒 check_failed）
- 今日证据：`evidence/D1-39/stdout.log`（含 MCP 响应、queryDistTagsSync=null、parseDistTagsOutput=null、npm 原始输出）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D1-42 dismiss 真实闭环不成立（同源 parse 缺陷致 targetVersion 恒 null）
- 今日证据：`evidence/D1-42/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#673](https://github.com/huaweicloud/huaweicloud-devkit/issues/673)（open）**[测试报告] huaweicloud-devkit 1.1.4-next.6 全量测试缺陷合并单（6 项）**
    - 历史单内容：## 测试概览 - 被测版本：1.1.4-next.6 - 缺陷：6 项 ## 缺陷清单 ### 1. [P0] 凭证 env 打印拦截不完整 — HW_ACCESS_KEY/HW_SECRET_KEY 前缀未被 env-dump 规则覆盖 - **描述**：`env | grep HW_ACCESS_KEY`、`printenv HW_ACCESS_KEY HW_SECRET_KEY` 经 `h

## EXP-E01~E14 / D10-3 serviceCatalog 中文意图路由准确率仅 21.4%
- 今日证据：`evidence/D10-3/stdout.log`、`evidence/EXP-E01/stdout.log` ~ `evidence/EXP-E14/stdout.log`；harness 结果：`eval/results/eval-run-20260923092452.csv`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## EXP-C4-14/18 DMS/DEW list_operations 静默失败
- 今日证据：`evidence/EXP-C4-14/stdout.log`、`evidence/EXP-C4-18/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  ...
  - [#732](https://github.com/huaweicloud/huaweicloud-devkit/issues/732)（open）**[daily-test] KooCLI 7.2.12 缺少 DMS/DEW 服务映射 (EXP-C4-14/EXP-C4-18)**
    - 历史单内容：## 问题描述 KooCLI 7.2.12 缺少 DMS 和 DEW 两个服务的映射，导致 EXP-C4-14 和 EXP-C4-18 两个用例 FAIL。 ## 复现 ``` $ hcloud DMS --help [USE_ERROR]服务不存在:DMS $ hcloud DEW --help [USE_ERROR]服务不存在:DEW ``` ## 影响 - EXP-C4-14 (P1): D
  - [#700](https://github.com/huaweicloud/huaweicloud-devkit/issues/700)（open）**[测试报告] huaweicloud-devkit 1.1.5 每日测试缺陷合并单（1 项）**
    - 历史单内容：## 测试概览 - 被测版本：1.1.5 - 缺陷：1 项 ## 缺陷清单 ### 8. [P1] EXP-C4-14/EXP-C4-18 DMS/DEW 服务 list_operations 返回"不支持的服务名称" - **描述**：`list_operations("DMS")` 和 `list_operations("DEW")` 均返回 `[USE_ERROR]不支持的服务名称`。Koo
- **仅出现过（正文含用例号但非缺陷语义，未据此判历史）**：#767

## D8-9 sanitizeValue 未移除 AK/SK/token 敏感值
