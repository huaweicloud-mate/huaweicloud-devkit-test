...
> **落盘路径**：`results/WorkBuddy/2026-09-20-188.239.14.150/Windows/FINDINGS.md`
...
- **证据**：`evidence/EXP-E01/stdout.txt`（全量15条评测结果），`evidence/d5-c4-probe.mjs`（probe脚本），eval harness 命令：`node eval/harness/run-eval.mjs <mcp-server.mjs>`
- **状态**：待提单
...
- **根因**：`plugins/huaweicloud-core/src/update-check.mjs` queryDistTagsSync 函数在 Windows 环境下查询 npm registry 失败，导致无法获取 dist-tags 信息，返回 check_failed。可能与 Windows 下 npm.cmd spawnSync 的网络请求、DNS 解析或 registry 配置有关。
- **影响**：用户在 Windows 环境下无法获取正确的版本更新状态，check_update 工具功能不可用。
- **证据**：`evidence/D1-27/stdout.txt`（result=check_failed），`evidence/D1-31/stdout.txt`（dismiss 后 result=check_failed），`evidence/p0-design-probe.mjs`（probe脚本）
- **状态**：待提单

## #3【非产品缺陷】D10-4 安全干预评测需真实 Agent 会话（LLM harness）

- **现象**：D10-4 安全干预评测需要真实 Agent 会话行为评测（LLM harness），serviceCatalog 路由层无法代理安全干预行为。
- **说明**：D10-4 是 P0 用例，测试安全干预行为（Agent 在检测到高危操作时是否主动拦截/提醒）。该评测需要真实 LLM Agent 会话环境（如 dsh --profile headless），非 DSH 客户端（WorkBuddy）无 dsh 工具，无法运行 run-agent-eval.mjs。这是环境依赖而非产品缺陷。
