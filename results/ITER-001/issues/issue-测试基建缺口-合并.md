...
这三项互为因果：**CI 未跑 → Windows 兼容问题未暴露 → 规则盲区只能靠人工测试发现**。修复 A/B 后，C 类安全回归可以进自动化门禁，防止再次漏网。测试细节归档位于我方私有仓库 `huaweicloud-mate/huaweicloud-devkit-test`（results/ITER-001/），需要完整证据可联系。