...
## D4-16 命令包裹穿透（sh -c 包裹凭证 env 打印未被拦截）
- 今日证据：`evidence/d4-security/stdout.log`（D4-16 wrap-sh 测试项，实测 `allow`）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D10-3 / EXP-E serviceCatalog 路由层命中率仅 21.4%
- 今日证据：`eval/results/eval-run-20260916015504.csv`；`evidence/EXP-E01..E14/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-2 JSON-RPC 错误码——invalid params 未返回 -32602
- 今日证据：`eval/results/protocol-probe-20260916015543.json`（D9-2b-invalid-params 测试项 FAIL）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D8-4 INSTALL.md 未随 npm 包发布
- 今日证据：`evidence/d8-installmd/stdout.log`（`Test-Path INSTALL.md = False`）
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#694](https://github.com/huaweicloud/huaweicloud-devkit/issues/694)（open）**[测试报告] huaweicloud-devkit 1.1.4 每日测试缺陷合并单（4 项，OpenCode-glm-5.2 Windows）**
    - 历史单内容：## 测试概要 - 被测版本：1.1.4（npm latest，gitHead 9b67256） - 客户端：OpenCode-glm-5.2 / Windows Server 2022 - 测试日期：2026-09-15 - 设计级用例：81 条（PASS 77 / FAIL 4 / BLOCKED 0 / NOT_RUN 0） - 展开级用例：39 条（PASS 39 / FAIL 0 / B
  - [#681](https://github.com/huaweicloud/huaweicloud-devkit/issues/681)（open）**[测试报告] huaweicloud-devkit v1.1.4 全量测试缺陷合并单（4 项）**
...
## D9-9 tools/call 超时协议语义——未声明 cancellation 能力（SPEC-MISMATCH）
- 今日证据：`eval/results/protocol-probe-20260916015543.json`（D9-9a-capabilities.cancellation SPEC-MISMATCH）
- **关联历史单（已作为缺陷提过，本次为复核）**：
