// DSH/Linux daily probe — misc: hint sequence / import erase / sandbox / SBOM / cold-start / stdio / cross-client (v1.1.6)
...
// ============ D1-45 兜底提示真实序列与预热竞态 ============
{
  const hint = { currentVersion: '1.1.4', latestStable: '1.1.6', targetVersion: '1.1.6', updateAvailable: true };
  const base = { ok: true };
...
// ============ D4-12 供应链安装期安全 ============
{
  const pkg = JSON.parse(readFileSync(join(SUT, 'package.json'), 'utf8'));
  const scripts = pkg.scripts || {};
  const pi = join(SUT, 'bin', 'dsh-postinstall.cjs');
...
// ============ D8-4 引导步骤可机械执行 ============
{
  const skillsRoot = join(SUT, 'plugins', 'huaweicloud-core', 'skills');
