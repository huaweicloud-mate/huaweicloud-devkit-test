...
> **落盘路径**：`results/DSH/2026-09-23-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：2026-09-23 05:35（北京时间）
> **被测版本**：`huaweicloud-devkit@1.1.6`（npm latest）
> **说明**：本轮 daily 全量执行设计级 100 + 展开级 39。共 19 项缺陷（18 FAIL + 1 SPEC-MISMATCH），与 2026-09-21 历史提单（v1.1.5）逐条复核一致，#1~#19 全部为历史/同源缺陷（v1.1.6 未修复）。展开级 12 项 EXP-E 路由 miss 同源于 #7（D10-3 serviceCatalog 中文路由缺陷），不重复开单。
...
- **根因**：`plugins/huaweicloud-core/hooks/huaweicloud-safety.py:46` `WRITE_OPERATION_RE` 要求写前缀前是字母数字或行首，而 `hcloud VPC CreateVpc` 中 `Create` 前是空格，漏判为 `cli:invoke`；同文件 READ_OPERATION_RE 用 `\b` 正确。
