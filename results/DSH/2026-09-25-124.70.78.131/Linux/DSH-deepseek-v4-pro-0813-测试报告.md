...
> **生成时间**：`2026-09-25 05:35:00`（北京时间）
> **执行归档**：`results/DSH/2026-09-25-124.70.78.131/Linux/`
...
| OS / 架构 | Linux x64（ecs-hd-ai-work-00-0011） |
...
> **执行方法**：探针脚本（.mjs）直调 `huaweicloud-devkit` 插件源码导出函数，决策/结果落 `stdout.log`；CLI 真机执行记录日志；证据统一落 `evidence/<case-id>/`。
...
| P0 / P1 / P2 缺陷 | `6 / 19 / 6`（P1 含 12 条展开级路由 MISS + 1 SPEC） |
| 红线（I 类）违规 | `0` |
...
| BLOCKED | `1` | D3-S3 沙箱 VeSPACE 配给超时（补环境） |
...
|---|---|---|---|---|
| 1 | P0 | D9-12 | initialize 未强制时序（非法 tools/list 未 -32600） | mcp-protocol.mjs:30-55 / mcp-server.mjs:166-190 |
| 2 | P0 | D4-13 | 只读子账号 env 动态切换失效 | credentials.mjs:245-250 (R9) |
| 3 | P1 | D10-3 | serviceCatalog 中文意图路由命中率仅 21.4% | tools.mjs:1947 |
| 4 | P0 | D2-4 | 凭证脱敏漏小写 ak=/sk= | safety-policy.mjs:31/49 |
...
| 9 | P1 | D4-17 | hook 模糊输入未 fail-closed | risk-rule-engine.mjs:150-153 |
| 10 | P1 | D4-24 | 审批令牌重复/过期未结构化返回 | hcloud-cli.mjs:85-92 |
| 11 | P1 | D3-S7 | 跨服务复合意图未命中 RDS+部署 | tools.mjs:1947 |
| 12 | P1 | D9-9 | tools/call 未声明取消/超时语义（SPEC） | mcp-protocol.mjs:47 |
| 13 | P2 | D1-65 | 调试开关仅接受 'true' | telemetry.mjs:81 |
...
| 17 | P2 | D8-1 | 文档工具数 39 vs 实现 40 | 文档 AGENTS.md/README |
| 18 | P2 | D8-9 | sanitizeValue 未移除小写 ak=/sk= | telemetry.mjs:189 |
| 19 | P1 | D9-2 | tools/list 非法 params 未返回 -32602 | mcp-protocol.mjs:57-59 |
...
> D9-6（跨客户端互通）原标注 BLOCKED，经 `eval/harness/protocol-probe.mjs` 10 客户端 clientInfo 矩阵复测 10/10 通过，已改判 PASS（协议不依赖特定客户端）。
...
- 本轮依赖隔离说明：探针运行于沙箱 workspace 下的隔离 HOME（复制 `.config/huaweicloud`、`.hcloud`、`.dsh`），被测对象为 npm 全局包 v1.1.7。
