...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:45` `.replace(/(AK|SK)\s*[:=]\s*(...)/g, ...)` 大小写敏感（无 `i`）且仅覆盖大写 `AK|SK`，未覆盖 obsutilconfig 的小写 `ak=`/`sk=` 键。
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:336` env-dump 判定正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/i` 未覆盖官方凭证变量名 `HW_ACCESS_KEY`/`HW_SECRET_KEY` 的 `HW_` 前缀。
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:349`（classifyTextCommand secret 正则 `/ShowSecretVersion|GetSecretValue|secret_string|secret_binary/i`）及 classifyHcloudArgs 对应正则均未覆盖 KMS `DecryptData`/`DecryptDataKey`。
...
- **现象**：`hcloud $(echo $'E\x43S DeleteServer') --id x` 判 `decision=allow`（shell 展开后等价破坏性删除）。
...
- **现象**：`bash -c "hcloud ecs DeleteServers --server-id 1"`、`sh -c "echo x && hcloud rds CreateInstance"` 均判 `allow`。
- **断言**：内层含 `hcloud <Svc> Delete*/Create*` 的包裹命令应判 `deny`。
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:345` 正则 `/(^|\s)hcloud(\.exe)?\s+/i.test(text)` 仅匹配行首/空白后的 `hcloud`，引号包裹、`&&` 链接、`-c` 内层均绕过。
...
- **根因**：`hdk/package.json:8` `files` 数组（`[cordis.patch.yml, bin, .agents, plugins/huaweicloud-core, integrations/...]`）未列 `rules`；`hdk/rules/huawei-agent-rules.mdc` 沦为孤儿文件，未被打包/注入任何 client。
...
- **根因**：`plugins/huaweicloud-core/src/mcp-protocol.mjs:95` 对 unknown method 直接 `throw new Error(...)` 未封装 `{code,message}`；`mcp-server.mjs:169` 统一硬编码 `code:-32603`。
...
- **现象**：「云主机」「云服务器」「云数据库MySQL」「弹性公网IP」「备份策略」「费用」「监控告警」「HTTPS证书」「权限审计」「函数」等中文意图 miss，返回 `Run hcloud --help...`；仅 Redis→DCS、Kubernetes→CCE、代金券→voucher 命中（3/15，涉及 EXP-E01~E14）。
- **断言**：中文意图「创建一台 2C4G 的 Ubuntu 云服务器」应路由到 ECS（recommendedService 含 ECS）。
- **根因**：`plugins/huaweicloud-core/src/tools.mjs:1776-1907` `serviceCatalog()` 的 `routeMap` 关键词均为英文，`String(intent).toLowerCase()` 未做中文意图映射（仅 voucher/sandbox 有零星中文关键词）。
- **影响**：中文用户（主要目标群体）意图路由大幅 miss，D10-3 路由准确率仅 3/15，远低于 ≥90% 门槛。
- **证据**：`evidence/routing/stdout.log`（`3/15 hit`，EXP-E01~E14 共 12 条 FAIL）

## #10【P2】D8-1 文档与能力漂移（AGENTS.md 39 vs 实现 40 工具）

- **现象**：`TOOL_DEFINITIONS.length` 实测 40，但源码仓库 `AGENTS.md:27` 仍写「39 tools in tools.mjs」、`:45` 仍写「39 MCP tool definitions」。
