...
> **执行归档**：`results/DSH/2026-09-24-124.70.78.131/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）`v1.1.7-next.1`（npm @next）
> **结论**：`FAIL`（设计级 18 FAIL + 1 SPEC-MISMATCH + 2 BLOCKED + 1 NOT_RUN；展开级 12 FAIL；全部 19 项为 v1.1.5 历史/同源缺陷，v1.1.7-next.1 复核确认，无当日新增）
...
| 被测版本（SUT） | `v1.1.7-next.1`（npm @next，自动取最高 next>latest；本地 hdk 已 checkout 到 gitHead `657ceb7b`，tag `v1.1.7-next.1`，PR #806） |
| 工具全集 | `40`（`tools.mjs` TOOL_DEFINITIONS） |
| hcloud / 依赖 | hcloud（KooCLI）7.2.12 / doctor 已配置 |
| 真云凭证 | cn-north-4（AK/SK 管理员 hw018619646 + 只读子账号 test001） |
| 测试类型 | 源码级探针 / 真机 CLI（doctor/status/help）/ MCP 协议(stdio+冷启) / 真云 E2E（VPC/SG/OBS 建删归零 + CTS 审计） |
| daily 基础用例 | 设计级 100 / 展开级 39（预筛剔除 27 非本客户端/OS）/ 追踪表 211 |

> **执行方法**：探针脚本（p1_src.mjs / p2_realcloud.mjs / p3_misc.mjs / p4_d413.mjs / p5_d425.py）直调 `huaweicloud-devkit@1.1.7-next.1` 安装包 `plugins/huaweicloud-core/src/*` 导出函数 + 真机 CLI + 真云建删资源，结论 JSON 落 `evidence/<case-id>/stdout.log`（status/why/executedAt），`backfill_daily.py --write` 一次性回填设计级/展开级 CSV。
...
| P0 / P1 / P2 新增缺陷 | `0 / 0 / 0`（全部 19 项为 v1.1.5 历史/同源缺陷，v1.1.7-next.1 复核确认，无新增） |
| 红线（I 类）违规 | `0` |
...
> 19 项（18 FAIL + 1 SPEC-MISMATCH）。与 2026-09-21（v1.1.5）历史提单逐条复核一致，#1~#19 全部为历史/同源缺陷，v1.1.7-next.1 未修复。详表见 `FINDINGS.md`。
...
> D3-S6/D3-S7 已由 source-level 确定性断言（serviceCatalog 路由）实测，均 miss（同 D10-3），转 FAIL（非 BLOCKED）。
...
- **v1.1.7-next.1 缺陷复核确认**：npm @next 已更至 1.1.7-next.1，但 #1~#19 项缺陷（与 2026-09-21 v1.1.5 / 2026-09-23 v1.1.6 一致）均在 1.1.7-next.1 复现，未修复；`file_issue.py` 历史查重后生成 `HISTORY_LINKS.md`，不重复开单。
- **版本说明**：`prepare_env.py --update` 默认「自动取 latest/next 版本号更高者」，本轮 next=1.1.7-next.1 > latest=1.1.6，故 SUT 自动切至预发布 @next 版。根 AGENTS.md「被测包=npm latest 正式版、测 NR 预发布才用 --next」口径，此自动化默认值与文档存在口径差异，建议维护 agent 核对其是否为预期（每日测试应默认 latest 还是追最高 tag）。
- **D4-13 最小权限**：本轮用 `run-as-readonly.py` 等价 env 注入实测「动态切换」仍失效（返回管理员）；只读子账号写被 IAM 正确拒绝（VPC.0010 PolicyNotAuthorized），写保护层正确。
- **建议优先修复**：P0 安全类（D2-4/D4-3/D4-15/D4-16/D4-23）仍为最高优先级；裸 `token=` 脱敏缺口已在三处路径复现（D4-26 redactEvidence / D4-27 safety-policy + redactOutput）。