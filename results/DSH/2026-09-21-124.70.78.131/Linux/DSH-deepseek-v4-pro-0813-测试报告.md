...
> **生成时间**：2026-09-21 05:50（北京时间）
> **执行归档**：`results/DSH/2026-09-21-124.70.78.131/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）`v1.1.5`（npm latest，gitHead `e7ed6f66e`）
> **结论**：`FAIL`（设计级 18 FAIL + 1 SPEC-MISMATCH + 2 BLOCKED + 1 NOT_RUN；展开级 12 FAIL；其中 #1~#18 为历史/同源缺陷，#19 D4-24 为当日解除假阻塞后新增缺陷）
...
| 测试类型 | 源码级探针 / 真机 CLI（install/doctor/status/help）/ MCP 协议(stdio+冷启) / 真云 E2E（VPC/SG/OBS 建删归零） |
| daily 基础用例 | 设计级 100 / 展开级 39（预筛剔除 27 非本客户端/OS）/ 追踪表 209 |

> **执行方法**：探针脚本（_p1_src.mjs / _p2_realcloud.mjs / _p3_misc.mjs / _p4_d413.mjs）直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数 + 真机 CLI + 真云建删资源，结论 JSON 落 `evidence/<case-id>/stdout.log`（status/why/executedAt），`backfill_daily.py --write` 一次性回填三份 CSV（执行状态/执行时间/evidencePath）。
...
| P0 / P1 / P2 新增缺陷 | `0 / 1 / 0`（#19 D4-24 P1；其余 #1~#18 为历史/同源缺陷，复测确认） |
| 红线（I 类）违规 | `0` |
...
> 19 项（18 FAIL + 1 SPEC-MISMATCH）。#1~#18 为 v1.1.5 历史已提单/同源缺陷（本轮复核确认）；#19（D4-24）为当日新增。详表见 `FINDINGS.md`。
...
| 19 | P1 | D4-24 | 确认令牌过期/重复确认返回非结构化码（新增） | hcloud-cli.mjs:84-96 / tools.mjs:1747-1751 |
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| VPC（tctest-dsh-c4/s2/c14/c18/ro 等） | 是（多次建删） | 全部删除 | ListVpcs 过滤 tctest-dsh- 计数 0 |
...
- **v1.1.5 缺陷确认**：npm latest 仍为 1.1.5（gitHead `e7ed6f66e`）。#1~#18 项缺陷与 2026-09-19/09-20 复测一致，均为历史已提单/同源问题；`file_issue.py` 查重后生成 `HISTORY_LINKS.md`，不重复开单。
- **新增缺陷（#19 D4-24）**：昨日误标 BLOCKED（「需时钟夹具」），本轮 source-level 直调 `createApprovalToken`/`consumeApprovalToken` 实测：过期与重复确认均返回 `null`（通用 Error 字符串），无 `{code:'CONFIRM_TOKEN_EXPIRED'}` / `{outcome:'already_processed'}` 结构化字段——审批流健壮性的机器可断言边界缺失，转为新增 P1 缺陷。
- **D3-S6/D3-S7 判定调整**：昨日标 BLOCKED（补环境），本轮按「假阻塞能解必解」改走确定性 source-level 断言（serviceCatalog 路由），二者均 miss（同 D10-3），故转 FAIL（路由缺陷）；其「真云 FG 定时触发器 / RDS 建删」步骤仍属计费实例真云专项，建议母版将这两步拆为「路由断言 + 真云专项」两段。
- **建议优先修复**：P0 安全类（D2-4/D4-3/D4-15/D4-16/D4-23）仍为最高优先级；裸 `token=` 脱敏缺口已在三处路径复现（D4-26 redactEvidence / D4-27 safety-policy）——建议把 `token` 关键字统一收口到 safety-policy 与 risk-rule-engine 共用的关键字表。