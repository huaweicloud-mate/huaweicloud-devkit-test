...
> **落盘路径**：`results/DSH/2026-09-21-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：2026-09-21 05:45（北京时间）
> **被测版本**：`huaweicloud-devkit@1.1.5`（npm latest，gitHead `e7ed6f66e`）
> **说明**：本轮 daily 全量执行设计级 100 + 展开级 39。共 19 项缺陷（18 FAIL + 1 SPEC-MISMATCH）：#1~#18 与 2026-09-19/09-20 历史提单一致（复核确认），#19（D4-24）为当日由「假 BLOCKED」转补测后暴露的新增缺陷。展开级 12 项 EXP-E 路由 miss 同源于 #7（D10-3 serviceCatalog 中文路由缺陷），不重复开单。
...
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:50-53` `conditionMatches()` 纯文本正则，不做 shell 命令替换展开，`$(...)` 破坏 `hcloud <service> <op>` 匹配。
...
- **根因**：v1.1.5 `classifyHcloudArgs` 内新增 `stripExecutable`/`findHcloudCommandSegments` 修复 `&&` 链接，但 `plugins/huaweicloud-core/src/safety-policy.mjs:428` 入口正则 `/(^|\s)hcloud(\.exe)?\s+/i` 仍不匹配引号包裹的 `bash -c "hcloud ..."`。
...
- **根因**：`plugins/huaweicloud-core/../package.json:8`（`files` 数组未列 `rules`），规则文件未被打包/注入任何 client。
- **影响**：全局安全约束（禁直连 CSMS/KMS 等）未交付到任何 agent，安全约束降级（P0）。
- **证据**：`evidence/D4-23/stdout.log`（`files includes rules`=false + 规则文件孤儿存在）

## #7【P1】D10-3 serviceCatalog 中文意图路由大量 miss（准确率 21.4%）

- **现象**：eval-set-v1 15 条中文意图中仅 Redis→DCS、Kubernetes→CCE、代金券→voucher 命中（3 HIT / 14，21.4%）；「云主机」「云服务器」「弹性公网IP」「备份策略」「费用」「监控告警」「证书」「权限审计」「函数」等全 miss。展开级 EXP-E01~E14 中 12 条 miss。
...
- **影响**：中文用户（主要目标群体）意图路由大幅 miss（21.4%，门槛 ≥90%）。
- **证据**：`evidence/D10-3/stdout.log`（`路由准确率 3/14 = 21.4%`）；`eval/results/eval-run-*.csv`
...
- **证据**：`evidence/D4-13/stdout.log`（`resolved=HPUAN1 期望=HPUA3X` + `只读写被 IAM 拒绝: true`）

## #9【P2】D8-1 文档与能力漂移（AGENTS.md 39 vs 实现 40 工具）

- **现象**：`TOOL_DEFINITIONS.length` 实测 40，但源码仓库 `AGENTS.md` 写「39 tools in tools.mjs」。
...
- **根因**：`plugins/huaweicloud-core/src/mcp-protocol.mjs:63-65`（capabilities 仅 `{tools:{}}`，无 `notifications.cancellation`）；`mcp-server.mjs:169`（error 统一 `-32603` 兜底）。
...
- **现象**：复合意图「物联网时序数据+前端托管」返回 fallback `Run hcloud --help`，未拆分命中 DDS/GaussDB/OBS/ECS。
- **断言**：复合中文意图应正确拆分并命中多个对应 service。
- **根因**：同 #7——serviceCatalog routeMap 无中文意图映射。
- **证据**：`evidence/D3-S5/stdout.log`（`svc=[Run hcloud --help...]`）

## #13【P2】D3-S6 场景-FunctionGraph 定时任务路由 miss（同 D10-3）

- **现象**：`serviceCatalog('部署一个定时函数任务')` 返回 fallback，未路由到 FunctionGraph。
- **断言**：中文意图「定时函数」应路由命中 FunctionGraph。
- **根因**：同 #7——serviceCatalog 英文关键词 `functiongraph/lambda/trigger` 未覆盖中文「定时函数」。
- **证据**：`evidence/D3-S6/stdout.log`（`路由命中 FunctionGraph` FAIL）

## #14【P1】D3-S7 场景-跨服务（Web 应用 + RDS）复合路由不完整（同 D10-3）

- **现象**：复合意图「Web 应用 + 云数据库 RDS」仅命中 RDS，未命中部署目标（sandbox/ECS/OBS），多服务编排入口缺失。
- **断言**：复合意图应命中 RDS + 部署目标（多服务）。
- **根因**：同 #7——serviceCatalog 单关键字命中后不拆分多服务。
- **证据**：`evidence/D3-S7/stdout.log`（`svc=[RDS]` 缺少部署目标）
...
- **现象**：`hcloud ecs DeleteServers`（写操作）被分类为 `cli:invoke` 而非 `cli:write`。
- **断言**：写→`cli:write`、只读→`cli:read`、其他→`cli:invoke`。
- **根因**：`plugins/huaweicloud-core/hooks/huaweicloud-safety.py:46` `WRITE_OPERATION_RE` 要求写前缀前是字母数字或行首，而 `hcloud ecs DeleteServers` 中 `Delete` 前是空格，漏判为 `cli:invoke`；同文件 READ_OPERATION_RE 用 `\b` 正确。
...
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:19-27` `redactEvidence()` 关键字列表未含裸 `token`（与 D4-27 同源，第三处脱敏路径）。
...
- **现象**：`consumeApprovalToken()` 对「过期」与「已消费」两种情况均返回 `null`（无差别）；`runApprovedCommand` 抛出的错误为通用 `Invalid or expired approval token. Please re-plan the command.` 字符串，无 `{code:'CONFIRM_TOKEN_EXPIRED'}` / `{outcome:'already_processed'}` 结构化字段，机器无法区分「过期」与「重复确认」。
- **断言**（按用例契约）：过期令牌返回精确 `{code:'CONFIRM_TOKEN_EXPIRED'}`（无资源创建）；重复确认第二次返回 `{outcome:'already_processed'}`（计数不 +1）；错误/结果 JSON 字段可机器断言。
- **根因**：`plugins/huaweicloud-core/src/hcloud-cli.mjs:84-96`（`consumeApprovalToken` 对过期 `Date.now()-createdAt > APPROVAL_TTL_MS` 与已消费均 `return null`，无结构化错误码）；`plugins/huaweicloud-core/src/tools.mjs:1747-1751`（`runApprovedCommand` 统一抛出字符串 Error）。
