...
> **说明**：本轮在 v1.1.5 正式版复测；**确认修复 3 项**（D4-2 凭证 env 打印拦截、D9-2 JSON-RPC -32601、D4-16 部分修复）。以下 10 项为 v1.1.5 仍存的产品缺陷（9 FAIL + 1 SPEC-MISMATCH），历史查重后按语义/用例号去重，仅在既有单上补复核、不重复开单。
...
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:50-53` `conditionMatches()` 用 `new RegExp(condition.regex,'ims').test()` 纯文本匹配，不做 shell 命令替换/ANSI-C quoting 展开，`$(...)` 与 `$'\x43'`(=C) 破坏 `hcloud <service> <op>` 匹配。
...
- **根因**：v1.1.5 在 `classifyHcloudArgs` 内新增 `stripExecutable`/`findHcloudCommandSegments`（修复 `&&` 链接），但 `plugins/huaweicloud-core/src/safety-policy.mjs:428` 的入口正则 `/(^|\s)hcloud(\.exe)?\s+/i.test(text)` 仍不匹配引号包裹的 `bash -c "hcloud ..."`，导致该文本不进入 `classifyHcloudArgs` 解包，直接落到 `allow/not_huaweicloud`。修复不完整。
...
- **现象**：「云主机」「云服务器」「弹性公网IP」「备份策略」「费用」「监控告警」「HTTPS证书」「权限审计」「函数」等中文意图 miss，多数返回 `Run hcloud --help...`；仅 Redis→DCS、Kubernetes→CCE、代金券→voucher 命中（3/15）。
...
- **现象**：`initialize` 返回 `capabilities = { tools: {} }`，未声明 `notifications.cancellation`；服务端无 `-32000 timeout` 语义（未知方法已正确返回 `-32601`）。
...
- **证据**：`evidence/protocol/stdout-blocked.log`（`cancellation 未声明` + `未知方法 code=-32601 非 -32000`）