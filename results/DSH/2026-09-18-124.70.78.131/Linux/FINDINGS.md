...
> **落盘路径**：`results/DSH/2026-09-18-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：2026-09-18 05:31（北京时间）
> **被测版本**：`huaweicloud-devkit@1.1.5`（npm latest 正式版，gitHead `e7ed6f66e`）
> **说明**：本轮在 v1.1.5 正式版复测（npm latest 仍为 1.1.5，gitHead `e7ed6f666ac5049ab467ff03daedd463da754d8a`，与 2026-09-16 复测同版本）。以下 11 项为 v1.1.5 仍存的产品缺陷（10 FAIL + 1 SPEC-MISMATCH），**均为历史已提单/已查重问题**（历史单以用例号/语义查重命中后仅复核、不重复开单，关联清单见 `HISTORY_LINKS.md`）。其中 #11 D4-27 为本日新增纳入 daily 精选的用例，缺陷已由同批客户端（Hermes #726）先行提单。
...
- **现象**：「云主机」「云服务器」「弹性公网IP」「备份策略」「费用」「监控告警」「HTTPS证书」「权限审计」「函数」等中文意图 miss；仅 Redis→DCS、Kubernetes→CCE、代金券→voucher 命中（3/15，准确率 21.4%）。
...
- **证据**：`evidence/routing/stdout.log`（`3/15 hit`）+ `evidence/routing/stdout-eval.log`（harness 21.4%）
...
- **证据**：`evidence/protocol/stdout-blocked.log`（`cancellation 未声明` + `未知方法 code=-32603 非 -32000`）
...
- **证据**：`evidence/D4-27/stdout.log`（`redactSecrets 裸 token= 脱敏 => token=TokenValueABCDEF123456`）
- **状态**：已提单 #726（本日同批 Hermes 客户端先行提单）