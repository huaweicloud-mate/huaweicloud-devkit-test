...
> **提单状态说明**：以下 4 项缺陷与 2026-09-13 上轮完全一致（SUT 版本未变），已由上一轮统一提单到 `huaweicloud/huaweicloud-devkit#650`（当前 OPEN）。本轮为每日回归确认：探针已重跑复现，未发现新增缺陷，故**不重复提单**，仅更新回归确认记录。
...
- **断言**：`printenv HW_ACCESS_KEY` 应返回 `decision=deny`。
...
- **状态**：已提单 #650（2026-09-14 回归确认仍存在，未重复提单）
...
- **断言**：`bash -c "hcloud ecs DeleteServers ..."` 应返回 `decision=deny`。
...
- **断言**：install 后 `~/.dsh` 下应有 `huawei-agent-rules.*` 文件。
...
- **断言**：未知方法应返回包含 `-32601` 的结构化 JSON-RPC 错误。
