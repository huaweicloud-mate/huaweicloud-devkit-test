// DSH/Linux daily probe — upgrade detection chain (next.6)
...
check('D1-28','update_available target=1.1.4', r28.result==='update_available'&&r28.updateAvailable===true&&r28.targetVersion==='1.1.4', r28);
...
// D1-39 detection chain (Linux)
const inst = readInstalledVersion();
check('D1-39','readInstalledVersion returns version', typeof inst==='string'&&inst.length>0, inst);
let dist=null; try { dist=queryDistTagsSync({timeoutMs:20000}); } catch(e){}
check('D1-39','queryDistTags returns dist-tags', !!dist&&(typeof dist.latest==='string'||typeof dist.next==='string'), dist);
check('D1-39','dist latest parseable', dist && semverParse(dist.latest)!==null, dist?.latest);
