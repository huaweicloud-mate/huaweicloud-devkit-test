# -*- coding: utf-8 -*-
"""DSH/Linux 2026-09-14 daily 执行回填。

只改 results/DSH/2026-09-14-124.70.78.131/Linux/ 本目录副本，不碰 test-cases 真源。
回填列：执行状态 + 执行时间 + evidencePath；追踪表回填「执行时间」。
"""
...
TS = datetime.datetime.now().strftime("%Y%m%d%H%M%S")  # 北京时间
...
BLOCKED = {
    "D1-39": "Windows 专属升级检测链（EINVAL/文件锁），本机 Linux；Linux 检测链语义已覆盖",
    "D1-5": "uninstall 破坏性操作（删除本机 ~/.dsh 插件/skills），执行后需重装+重启，headless 环境不做",
    "D2-11": "需真云 STS securityToken 临时凭证轮换场景（R3 落盘拒绝需真实 Token 链路验证）",
    "D2-10": "需真云 KooCLI current profile 多账号切换（R7 跟随语义）",
    "D2-1": "需真云 AK/SK 三端同步写入（KooCLI/env/session）",
