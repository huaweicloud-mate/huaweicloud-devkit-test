...
try {
  ...
  check('D2-11','persist+token -> rejected', sts && (sts.status==='error' || sts.scope==='rejected'), sts);
...
try {
  ...
  check('D4-1','hook blocks credential file read',
    hook && (hook.riskLevel === 'high' || hook.blocked === true || hook.allowed === false),
    hook);
...
try {
  ...
  check('D4-2','hook blocks credential env print',
    hook2 && (hook2.riskLevel === 'high' || hook2.blocked === true || hook2.allowed === false),
    hook2);
...
try {
  ...
  check('D4-3','hook blocks plaintext adminPass',
    hook3 && (hook3.riskLevel === 'high' || hook3.blocked === true || hook3.allowed === false || hook3.warnings),
    hook3);
...
try {
  ...
  check('D4-5','ListServers not classified as write',
    planRo && (planRo.isWrite === false || planRo.classification === 'readonly' || planRo.readonly === true),
    planRo);
  ...
  check('D4-5','CreateServers classified as write',
    planWr && (planWr.isWrite === true || planWr.classification === 'write' || planWr.needsApproval === true),
    planWr);
...
try {
  ...
  check('D4-9','hook flags destructive DeleteServers',
    hook9 && (hook9.riskLevel === 'high' || hook9.blocked === true || hook9.warnings),
    hook9);
...
try {
  ...
  check('D4-15','hook catches shell injection bypass',
    hook15 && (hook15.riskLevel === 'high' || hook15.blocked === true || hook15.warnings),
    hook15);
...
try {
  const art = await callTool('huaweicloud_hook_check_artifacts', {
    artifacts: [{
      path: 'test.tf',
      content: 'resource "huaweicloud_vpc" "test" { name = "public" cidr = "0.0.0.0/0" }'
    }]
  });
  check('D4-21','hook_check_artifacts returns structured result',
    art !== null && typeof art === 'object', art);
  check('D4-21','artifacts check flags public exposure',
    art && (art.riskLevel === 'high' || art.warnings || art.findings || art.allowed === false),
    art);
...
try {
  ...
  check('D4-22','deploy plan check flags EIP exposure',
    plan22 && (plan22.warnings || plan22.findings || plan22.riskLevel),
    plan22);
} catch(e) { check('D4-22','hook_check_deploy_plan works', false, e.message); }

// ============ D4-23 全局规则注入生效性 ============
try {
  // Check that safety policy rules are loaded
  const rules = safety.getRules ? safety.getRules() : (riskEngine.getRules ? riskEngine.getRules() : null);
  check('D4-23','safety rules loaded',
    rules !== null && (Array.isArray(rules) ? rules.length > 0 : Object.keys(rules).length > 0),
    rules ? (Array.isArray(rules) ? `array:${rules.length}` : `object:${Object.keys(rules).length}`) : 'null');
...
try {
  ...
  check('D10-4','static rule blocks bulk delete',
    hook10 && (hook10.riskLevel === 'high' || hook10.blocked === true || hook10.warnings),
    hook10);
