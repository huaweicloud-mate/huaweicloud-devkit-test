...
## D2-4 JSON 凭证字符串未脱敏
- 今日证据：`evidence/d2-auth/stdout.log`（D2-4 redact-json）。
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D4-16 sh 包裹写命令穿透
- 今日证据：`evidence/d4-security/stdout.log`（D4-16 wrap-sh）。
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#694](https://github.com/huaweicloud/huaweicloud-devkit/issues/694)（open）**[测试报告] huaweicloud-devkit 1.1.4 每日测试缺陷合并单（4 项，OpenCode-glm-5.2 Windows）**
    - 历史单内容：## 测试概要 - 被测版本：1.1.4（npm latest，gitHead 9b67256） - 客户端：OpenCode-glm-5.2 / Windows Server 2022 - 测试日期：2026-09-15 - 设计级用例：81 条（PASS 77 / FAIL 4 / BLOCKED 0 / NOT_RUN 0） - 展开级用例：39 条（PASS 39 / FAIL 0 / B
...
## D8-4 npm 包缺失 INSTALL.md
- 今日证据：`evidence/d2-auth/stdout.log`（D8-4 install-doc）。
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#694](https://github.com/huaweicloud/huaweicloud-devkit/issues/694)（open）**[测试报告] huaweicloud-devkit 1.1.4 每日测试缺陷合并单（4 项，OpenCode-glm-5.2 Windows）**
    - 历史单内容：## 测试概要 - 被测版本：1.1.4（npm latest，gitHead 9b67256） - 客户端：OpenCode-glm-5.2 / Windows Server 2022 - 测试日期：2026-09-15 - 设计级用例：81 条（PASS 77 / FAIL 4 / BLOCKED 0 / NOT_RUN 0） - 展开级用例：39 条（PASS 39 / FAIL 0 / B
  - [#681](https://github.com/huaweicloud/huaweicloud-devkit/issues/681)（open）**[测试报告] huaweicloud-devkit v1.1.4 全量测试缺陷合并单（4 项）**
...
## D9-2 tools/list 非法参数未返回 -32602
- 今日证据：`evidence/D9-2/stdout.log`（D9-2b-invalid-params）。
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-9 未声明 cancellation capability
- 今日证据：`evidence/D9-2/stdout.log`（D9-9a-capabilities.cancellation）。
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#704](https://github.com/huaweicloud/huaweicloud-devkit/issues/704)（open）**[D9-2] tools/call 缺少 required 参数校验**
    - 历史单内容：## 缺陷描述 `tools/call` 请求缺失 inputSchema 中标注为 required 的参数时，server 未返回参数校验错误（-32602 Invalid Params），而是直接执行导致 undefined 异常。 ## 复现步骤 1. 启动 MCP server 2. 调用任意工具但故意省略 required 参数：`{"jsonrpc":"2.0","method":"
  - [#699](https://github.com/huaweicloud/huaweicloud-devkit/issues/699)（open）**[测试报告] huaweicloud-devkit 1.1.4 每日测试缺陷合并单（1 项）**
