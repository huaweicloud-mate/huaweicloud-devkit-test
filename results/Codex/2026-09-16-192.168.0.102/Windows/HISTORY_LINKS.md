...
## D4-16 命令包裹穿透未拦截
- 今日证据：`evidence/D4-security-rerun/stdout.json`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-2 非法 tools/list 参数缺少 JSON-RPC 错误
- 今日证据：`evidence/D9-protocol-rerun/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-9 cancellation capability 未声明
- 今日证据：`evidence/D9-protocol-rerun/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
