...
if (rdsId) {
  for (let i = 0; i < 90; i++) {
    ...
    if (inst) {
      note(`D3-S7 RDS status=${inst.status} (${i + 1}/90)`);
  ...
  if (!active) {
    note('D3-S7 RDS 未在 900s 内 ACTIVE');
    check('D3-S7', 'RDS 实例到达 ACTIVE', false, true);
  }
}

// ⑤ 归零：删 RDS（等待异步删除）→ 等 compute 端口释放 → 删 router→subnet→VPC
try {
  if (rdsId) {
    const del = hcloud(['RDS', 'DeleteInstance', `--instance_id=${rdsId}`]);
    note(`D3-S7 DeleteInstance rc=${del.status} job=${(parse(del) || {}).job_id}`);
    check('D3-S7', '删除本次 RDS 受理(job_id)', !!(parse(del) || {}).job_id, true);
  }
  // 轮询等 RDS 删除完成（实例从列表消失）
  let rdsGone = false;
  if (rdsId) {
    for (let i = 0; i < 60; i++) {
      await new Promise((r) => setTimeout(r, 10000));
  ...
  // 等 subnet 内 compute 端口（RDS 节点网卡）彻底释放（否则 DeleteSubnet 报 used by private IP）
  if (subnetId) {
    for (let i = 0; i < 60; i++) {
      const ps = parse(hcloud(['VPC', 'ListPrivateips', `--subnet_id=${subnetId}`]));
      const computePorts = (ps?.privateips || []).filter((p) => (p.device_owner || '') === 'compute:cn-north-4a');
      if (computePorts.length === 0) break;
      await new Promise((r) => setTimeout(r, 10000));
    }
  }
  ...
  if (subnetId && vpcId) {
    ...
    // 先删 subnet（router interface 移除后即可删）
    let subnetGone = false;
    for (let i = 0; i < 12; i++) {
      const d = hcloud(['VPC', 'DeleteSubnet', `--subnet_id=${subnetId}`, `--vpc_id=${vpcId}`]);
      if ((d.status === 0 || d.status === undefined) && !/error|used by private IP|RouterInUse|does not belong|not found/i.test(`${d.stdout}`)) { subnetGone = true; break; }
      await new Promise((r) => setTimeout(r, 10000));
    }
    note(`D3-S7 subnet 删除=${subnetGone}`);
    await new Promise((r) => setTimeout(r, 2000));
    // 兜底：若 subnet 因 router 移除成为孤儿（vpc_id 清空），用 neutron API 删除
    if (!subnetGone) {
      const orphan = parse(hcloud(['VPC', 'ListSubnets']));
      const left = (orphan?.subnets || []).find((s) => s.id === subnetId);
      if (left) {
        const nid = left.neutron_subnet_id || left.id;
        hcloud(['VPC', 'NeutronDeleteSubnet', `--subnet_id=${nid}`]);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if ((dv.status === 0 || dv.status === undefined) && !/RouterInUse|Router contains subnets/i.test(`${dv.stdout}`)) break;
      await new Promise((r) => setTimeout(r, 5000));
    }
  }
  const vpcs = parse(hcloud(['VPC', 'ListVpcs/v3']));
  const subsAfter = parse(hcloud(['VPC', 'ListSubnets']));
  const remainV = (vpcs?.vpcs || []).filter((v) => (v.name || '').startsWith('tctest-s7-')).length;
  const remainS = (subsAfter?.subnets || []).filter((v) => (v.name || '').startsWith('tctest-s7-')).length;
  check('D3-S7', '测后 tctest-s7- 资源归零', remainV === 0 && remainS === 0, true);
} catch (e) {
