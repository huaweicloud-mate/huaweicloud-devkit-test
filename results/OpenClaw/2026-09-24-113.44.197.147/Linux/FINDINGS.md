...
> **落盘路径**：`results/OpenClaw/2026-09-24-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-24 06:05（北京时间）
> **被测版本**：`v1.1.7-next.1`（npm @next 预发布，gitHead `657ceb7`）
> **工具全集**：40（`tools.mjs` TOOL_DEFINITIONS）
> **本清单是统一提单脚本的解析输入**：`scripts/file_issue.py` 硬编码解析标题与「根因」字段，格式必须严格。

## 去重结论（本轮先读）

本轮按「每日测试」强制完整重跑，全量探针 fresh 执行、证据全新落盘，被测 v1.1.7-next.1（gitHead `657ceb7`）。v1.1.7-next.1 相对 v1.1.7-next.0 仅为沙箱 `tools.mjs` 的 devbridge 提示文案 + `huawei-sandbox` 技能文档更新（14 文件 114 增/67 删，无任何安全/协议/服务目录/认证功能代码变更），故 14 项 FAIL/SPEC 均为**历史同源**缺陷（多客户端多轮复现，上游已开单），`file_issue.py` 将自动查重，命中即不重复提单并生成 `HISTORY_LINKS.md`。
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:398-405` env-dump 门正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/i` 未覆盖 `HW_` 前缀；下方变量引用门只匹配 `$HW_*`/`printenv HW_*` 语法，`env | grep HW_SECRET_KEY` 无 `$` 也不含 `printenv`，两门均落空回退 `allow`。
- **影响**：高危凭证环境变量打印可绕过分类放行。
- **证据**：`evidence/d4-security-core/probe-p0-security.stdout.log`
- **状态**：历史同源（#650 等，`echo/printenv` 子项已修复，`env | grep` 子项残留）
...
## #6【P2】D4-25 Python hook 写命令遥测误分类为 cli:invoke
...
- **现象**：真机沙箱 E2E 核心步骤 connect / check_user / upload_project / deploy_nginx 全部 PASS，仅 expose 失败：沙箱内 `devbridge 0.1.13-release` 硬编码网关 `cn-north-4-bridge.myhuaweicloud.com` 已迁移失效（`devbridge host` 报 `Connection failed/Connection lost`），且未预置 0.2.x API Key（`/tmp/hw_api_key` 不存在、`sandbox_credentials` 无 `api_key` 注入），无法升级暴露公网 URL。
- **说明**：缺的是账号级 DevBridge API Key 凭证 + 沙箱镜像 0.2.x devbridge，非 hdk 源码缺陷（hdk 1.1.7-next.1 已实现 `sandbox_credentials` 的 `api_key` 注入路径 + 运行时能力探测）。计 BLOCKED + blockedReason，不计入产品缺陷提单。
