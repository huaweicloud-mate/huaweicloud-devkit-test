...
> **生成时间**：`2026-09-17 06:55`（北京时间）
> **执行归档**：`results/OpenClaw/2026-09-17-113.44.197.147/Linux/`
...
| hcloud / 依赖 | hcloud（doctor 全项通过，确认已配置） |
...
> **执行方法**：本轮「强制完整重跑」——重新执行全部 38 个探针脚本，直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数 + 走真实 mcp-server JSON-RPC 协议 + 真机 CLI + 真云 E2E，证据全新落盘 `evidence/<case-id>/`。探针入口脚本 `evidence/run_all.sh` 记录本轮执行日志。
...
| FAIL | 11 | D4-2/D4-16/D4-21/D4-23（P0×4）+ D4-6/D4-7/D4-17/D9-2/D9-4/D10-3（P1×6）+ D9-7（P2×1），根因见缺陷清单 |
| BLOCKED | 0 | — |
...
> 本轮 13 项缺陷（设计级 11 FAIL + D9-7 P2 + D9-9 SPEC）均有明确根因（`文件:行号`）+ 复现证据。经 `file_issue.py` 查重口径逐条核对上游 open issue：**全部命中历史同源单，本轮不重复提单，无新增缺陷**。详见同目录 `FINDINGS.md` 与 `HISTORY_LINKS.md`。

| # | 级别 | 用例ID | 缺陷描述 | 根因（文件:行号） | 状态 |
|---|---|---|---|---|---|
| 1 | P0 | D4-2 | 凭证 env 打印拦截残留（`env\|grep HW_*`） | `safety-policy.mjs:398-399` | 历史 #651/#652 等（不重复提单） |
...
| 4 | P1 | D9-2 | JSON-RPC -32602 未区分（invalid params） | `mcp-protocol.mjs:46-98` | 历史 #704/#643/#672 |
...
| 8 | P1 | D4-6 | adminPass 空格形式回显未脱敏 | `safety-policy.mjs:42` | 历史 #651/#673/#679/#712 |
| 9 | P1 | D4-7 | hook_check_artifacts broad IAM 失效 | `cloud-risk-rules.json:188-196` | 历史 #651/#652 |
| 10 | P1 | D10-3 | 中文意图路由未命中（仅英文关键词） | `tools.mjs:1776-1910` | 历史 #705/#689/#680 |
...
- **待裁决 SPEC**：`D9-9`（capabilities.cancellation 未暴露，历史 #698）；`D4-24` 令牌 TTL 契约。
- **本轮结论**：SUT 未变（v1.1.5 gitHead e7ed6f6），13 项 FAIL/SPEC 与昨日完全一致、逐一命中上游 open issue，无新增缺陷，本轮不重复提单。
- **建议**：长期未闭环项（`D4-2` env dump HW_ 前缀残留、`D4-16` sh -c 文本路径、`D4-21`/`D4-7` HCL broad IAM、`D10-3` serviceCatalog 中文路由、`D4-23` rules 注入缺失）建议优先排期修复，避免每日测试持续报同源缺陷。