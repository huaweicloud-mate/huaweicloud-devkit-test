// OpenClaw Linux 真云补测探针（2026-09-16）—— 重跑 2026-09-15 因凭证标 BLOCKED 的真云用例
// 覆盖：D4-13 最小权限、D3-C4 服务矩阵(真机建删归零)、D4-14 操作可审计(CTS)、D4-18/19/20 审批流、D2 认证真云项
...
// 执行 hcloud，显式注入 AK/SK（避免 hcloud 读 ~/.hcloud 管理员 profile，确保只读子账号真正生效）
...
// 仅提取首个平衡 JSON 对象（KooCLI 输出 JSON 后可能尾随诊断文本）
...
const deniedRe = /SYS\.0403|not authorized|Forbidden|forbidden|No permissions|doesn't allow|DBS\.280032|DCS\.2003|CTS\.0013|ces\.0017|insufficient/i;
...
// ============ D4-13 最小权限（只读子账号 test001）============
{
  // 只读子账号凭证有效性：IAM KeystoneListProjects（test001 被授予的读）
  ...
  check('D4-13', '只读子账号 IAM 读(KeystoneListProjects) 成功', n > 0, true);

  // 写操作应被 IAM 拒绝（CreateSecurityGroup 策略拒绝）
  ...
  check('D4-13', '只读子账号 写操作被 IAM 拒绝(SYS.0403)', writeDenied, true);

  // 只读矩阵：test001 未授予的读命令应被 IAM 拒绝（非崩溃、非含糊）
  const reads = [
    ['ECS', 'NovaListServers'],
    ['VPC', 'ListVpcs'],
    ...
    ['DDS', 'ListInstances'],
    ['DCS', 'ListInstances'],
    ['CTS', 'ListTraces', '--trace_type=system', '--limit=1'],
  ];
  let deniedReads = 0;
  for (const a of reads) {
    const r = hcloud(credRO.ak, credRO.sk, a);
    if (deniedRe.test(r.raw)) deniedReads++;
    else note(`D4-13  ${a[0]} ${a[1]} 读 => 未命中拒绝模式: ${r.raw.slice(0, 90)}`);
  }
  check('D4-13', '只读子账号 未授权读命令均被 IAM 拒绝', deniedReads, reads.length);

  // 已授予的读命令应可用（EVS ListVolumes 为 test001 实际被授予的读）
  const evs = hcloud(credRO.ak, credRO.sk, ['EVS', 'CinderListVolumes']);
  const evsAllow = !deniedRe.test(evs.raw);
  check('D4-13', '只读子账号 已授权读(EVS ListVolumes)可用', evsAllow, true);
}
...
// ============ D3-C4 服务矩阵 + D4-14 操作可审计（真机建删归零）============
{
  ...
  // D4-14 CTS 审计：按资源 id 反查创建记录（CTS 有秒级传播延迟，轮询最多 ~20s）
  let myTrace = null;
...
// ============ D2 认证真云项 ============
{
  ...
  // D2-16 import 读取后擦除：action=temporary（仅内存 setRuntimeCredentials + 无条件 clearImportFile），零 S1/S2/S3 落盘副作用
  ...
  try {
    ...
    check('D2-16', 'temporary 仅内存不落盘(status=ok/scope=temporary)', importRes?.scope, 'temporary');
  } finally {
    if (prevHome === undefined) delete process.env.HUAWEICLOUD_HOME; else process.env.HUAWEICLOUD_HOME = prevHome;
    rmSync(isoHome, { recursive: true, force: true });
  }

  // D2-11 R3 STS token 拒绝落盘：真实 S1 不应含明文明文 STS token
  ...
  // D2-1 三端同步（真云）：KooCLI / OBS / 凭证库(S1) 三端就绪 + 真云 API 实际可用
...
console.log('\n=== OpenClaw Linux 真云补测探针结果 (2026-09-16) ===');
