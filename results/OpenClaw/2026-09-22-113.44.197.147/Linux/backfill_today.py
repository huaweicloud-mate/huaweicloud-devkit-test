#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""回填 2026-09-21 OpenClaw Linux 每日测试执行状态。"""
import csv, os
...
# 历史同源 FAIL/SPEC 状态与昨日一致；新增 2 项新缺陷（D4-25, D3-S5）
design = {
    # D1 安装
    ...
    'D3-S3': ('BLOCKED','evidence/realcloud','【补环境】真机沙箱 E2E 核心步骤 connect/upload_project/deploy_nginx 均 PASS，仅最后 expose 步骤失败：沙箱内 devbridge 0.2.2-release 已移除 --huaweicloud --access-key --secret-key 登录参数（现仅剩 --api-key），且沙箱环境未预置 api-key，devbridge host 报 API key expired, please login again。此为沙箱镜像 devbridge 版本变更（2026-09-21 同探针旧参数 Login successful），非 hdk 源码缺陷（devkit 无 devbridge 鉴权/预置路径）。解除条件：沙箱镜像预置 devbridge api-key 或恢复 AK/SK 登录，或维护方明确 expose 步骤新鉴权方式。'),
...
for i in range(1,16):
    ...
    if eid in e_pass:
        ...
        expanded[eid] = ('BLOCKED','evidence/d10-routing','【补环境】真实 Agent 会话诊断意图层（"ECS启动失败帮我分析原因"）需真实 LLM Agent 判断是否路由 huaweicloud_explain_error；eval/harness/run-eval.mjs 的 serviceCatalog 确定性路由层无法代理该诊断意图。缺可交互真实 Agent 会话级 harness（仅 DSH/装了 dsh 客户端可用）。解除条件：接入可交互真实 Agent 客户端会话自动化。')
    else:
