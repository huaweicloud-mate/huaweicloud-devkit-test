#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""回填 2026-09-23 OpenClaw Linux 每日测试执行状态（v1.1.6 46152dd）。"""
import csv, os
...
design = {
    ...
    'D3-S3': ('BLOCKED','evidence/realcloud','【补环境】真机沙箱 E2E 核心步骤 connect/check_user/upload_project/deploy_nginx 均 PASS，仅 expose 步骤失败：沙箱镜像仍预置 devbridge 0.1.13-release（硬编码网关 cn-north-4-bridge.myhuaweicloud.com 已于 2026-09 迁移失效，host 报 Connection failed），且未预置 0.2.x API Key（/tmp/hw_api_key 不存在，sandbox_credentials 无 api_key 注入），无法 upgrade 到 0.2.x 暴露公网 URL。非 hdk 源码缺陷（hdk 1.1.6 已支持 api_key 注入路径，缺的是账号级 API Key 凭证 + 沙箱镜像 0.2.x devbridge）。解除条件：预置 DevBridge API Key（devstation.connect.huaweicloud.com/space/devbridge/apikey）或沙箱镜像更新 devbridge 0.2.x。'),
