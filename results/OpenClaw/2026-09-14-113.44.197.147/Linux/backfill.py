# -*- coding: utf-8 -*-
"""OpenClaw/Linux 2026-09-14 每日测试执行状态回填（只动自己目录的 CSV 副本）。
...
DESIGN_STATUS = {
    ...
    # D1 需真实安装/PTY/跨进程注入/白名单 merge —— NOT_RUN
    "D1-1": ("NOT_RUN", ""),
    "D1-2": ("NOT_RUN", ""),
    "D1-3": ("NOT_RUN", ""),
    "D1-4": ("NOT_RUN", ""),
...
def apply_design():
    ...
    for c in ("执行状态", "执行时间", "evidencePath"):
        assert c in fields, f"设计级缺列 {c}: {fields}"
    for r in rows:
        cid = r.get("ID", "").strip()
...
def apply_expanded():
    ...
    fields = list(rows[0].keys())
    for c in ("执行状态", "执行时间", "evidencePath"):
        assert c in fields, f"展开级缺列 {c}: {fields}"
    for r in rows:
        cid = r.get("ID", "").strip()
...
def apply_tracing():
    # 追踪表为纯设计追踪，仅追加「执行时间」列；本轮对已执行的设计级用例打时间戳
    with open(TRACING, encoding="utf-8-sig") as f:
        rows = list(csv.DictReader(f))
    fields = list(rows[0].keys())
    for r in rows:
        dc = r.get("designCaseId", "").strip()
        if dc in DESIGN_STATUS and DESIGN_STATUS[dc][0] != "NOT_RUN":
            r["执行时间"] = TS
        else:
            r["执行时间"] = r.get("执行时间", "") or ""
