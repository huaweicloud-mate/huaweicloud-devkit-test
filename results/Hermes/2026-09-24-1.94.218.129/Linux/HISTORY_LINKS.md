...
> 版本对比结论（补测）：今日应测版本为 v1.1.7-next.1（`657ceb7b`），首轮误测为 v1.1.6（`46152dd`），本轮补测更正 SUT=v1.1.7-next.1 后 15 项缺陷**全部仍在复现**，无新增缺陷、无缺陷修复回归。
...
> 结论：本轮补测 15 项缺陷**全部命中历史 open issue**（已复核 21 个关联单号全部仍 open），均为 v1.1.6 及更早版本已登记的已知缺陷，在 v1.1.7-next.1（`657ceb7b`）中仍复现。**不重复开单**，仅留本关联清单供维护者汇总。