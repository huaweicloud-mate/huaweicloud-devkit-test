...
- **现象**：`rules/huawei-agent-rules.mdc` 存在于源码，但 `package.json` files 白名单不含 `rules/`，npm 安装后全局包无 `rules/`、无 `.mdc`。
...
- **根因**：`tools.mjs:1329`（uploadProjectWithSession）与 `tools.mjs:1382`（deployCheck）结果未透出为 MCP content。
- **证据**：`evidence/d3-sandbox/stdout.log`（sandbox-upload/sandbox-deploy-check FAIL）

## #11【P2】D3-S5 复合意图仅命中单一服务（未分层多路由）

- **现象**：`serviceCatalog(intent='我要部署一个 Web 应用，数据存到数据库，文件存到对象存储')` → 仅命中 1 服务，期望 ≥2。
- **断言**：复合意图应命中多个对应服务。
...
- **现象**：Python hook 遥测中 `hcloud ECS CreateServers`（写）被归类 cli:invoke，无 cli:write。
- **断言**：写操作应归类 cli:write，只读 cli:read，其他 cli:invoke。
- **根因**：`hooks/huaweicloud-safety.py:46` `WRITE_OPERATION_RE` 边界无法匹配前缀前为空格（"ECS CreateServers"）。
- **证据**：`evidence/d4-hooks/stdout.log`（D4-25 cli-write-classified FAIL）

## #14【P2】D4-26 findings.evidence 小写 ak=/sk=/token= 未脱敏

- **现象**：`evaluateCommandRisk('cat ... ak=ACCNO sk=SECNO token=TOKNO')` 的 findings[].evidence 仍含明文。
- **断言**：findings.evidence 中 ak=/sk=/token= 应替换 `<redacted>`，无明文。
- **根因**：`risk-rule-engine.mjs:19-25` `redactEvidence()` 正则未覆盖小写 `ak=`/`sk=`/`token=`。
