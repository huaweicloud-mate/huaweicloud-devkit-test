...
> **执行归档**：`results/Hermes/2026-09-13-1.94.218.129/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：`PARTIAL`（3 项 P0 缺陷 + 2 项 P1 缺陷 + 1 项 P2 规范漂移）
...
| 被测版本（SUT） | `v1.1.4-next.3`（`npm install -g huaweicloud-devkit@next` 实装；本机私有 registry `127.0.0.1:45998` 的 `next` dist-tag 解析为 `1.1.3-next.2`，环境快照滞后，见阻塞项） |
| 工具全集 | `39`（`tools.mjs` TOOL_DEFINITIONS） |
| hcloud / 依赖 | hcloud 7.2.12 / doctor 10 项全 PASS |
| 真云凭证 | 未使用（本轮仅源码级探针 + 只读 npm/registry 查询，未创建云资源） |
...
| D3-A1 | P1 | skill 检索完整性 | PASS | `evidence/D3-A1/stdout.log` | 29 skills |
...
| 2 | P0 | D4-16 | 命令包裹穿透未拦截内层写命令 | `sh -c 'hcloud ecs DeleteServers...'` 应 `deny` | 返回 `allow` | `safety-policy.mjs:313,346`（splitSimpleCommand 折叠内层） | P | 待提单 |
| 3 | P0 | D4-23 | 全局规则 huawei-agent-rules.mdc 未注入 | 全部目标注入且约束可执行 | 安装后无 rules 目录 | `package.json:files` 缺 `rules` + setup-cli.mjs 无注入 | P | 待提单 |
| 4 | P1 | D4-4 | 写操作审批门漏词 Change* | `ChangeServerOs*` 应 `deny` | `allow/unknown_read` | `safety/policy.json` writeOperationPrefixes 缺 `Change` | P | 待提单 |
| 5 | P1 | D4-11 | 提示注入防护：自然语言夹带写命令 | 注入 payload 应被拒绝 | `allow/unknown_read`（仅 warn） | `safety-policy.mjs:345` 仅识别起始 hcloud | P | 待提单 |
...
- 实际：返回 `allow`
- 根因：`safety-policy.mjs:313` `splitSimpleCommand` 把引号内层折叠为单 token，
  `:346` `classifyHcloudArgs` 将 `sh` 当 service，`writeOperationPrefixes` 的 `Delete`
  因前置空格不匹配 → 放行

**#3 [P0] D4-23 全局规则未注入**
- 期望：11 安装目标均有 huawei-agent-rules.mdc 注入
- 实际：`package.json` `files` 不含 `rules`，安装后无 rules 目录
- 根因：`package.json:files` + `setup-cli.mjs` 复制白名单缺失
...
- 实际：`allow/unknown_read`（仅 `hwc-destructive-delete-operation` warn）
- 根因：`safety-policy.mjs:345` 仅当命令以 hcloud 起始时走写分类；自然语言前缀丢语义
...
- **建议**：① `writeOperationPrefixes` 补 `Change`/`Rebuild`（对齐真实 ECS 写动词）；② env-dump 规则补 `HW_` 前缀；③ `package.json.files` 纳入 `rules` 目录；④ mcp-server 按方法区分 JSON-RPC 错误码。