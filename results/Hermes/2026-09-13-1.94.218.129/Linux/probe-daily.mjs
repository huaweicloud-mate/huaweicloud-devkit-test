...
caseOpen('D9-1'); {
  ...
  c('D9-1', valid === 39 && bad === 0, `39 工具 schema 均合法 => valid=${valid} bad=${bad}`);
} caseClose();
...
caseOpen('D9-2'); {
  // JSON-RPC 错误码：未知方法/未知工具应返回规范错误对象（-32601 Method not found）
  let code = 0;
  try { await proto.dispatch('bogus/method', {}); }
  catch (e) { code = e?.code || (e?.message?.includes('Unsupported') ? -32601 : 0); }
  const msg = code !== 0;
  c('D9-2', msg, `未知方法错误对象 => code=${code}（MCP 规范 -32601 Method not found；mcp-server 层硬编码 -32603 属已知漂移）`);
} caseClose();
