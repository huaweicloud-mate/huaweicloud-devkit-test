#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""今日(2026-09-19)每日测试回填：证据落盘 + CSV 执行状态回填。
基于本日 fresh 探针实测结果（reverify-2026-09-19.sh + D10 run-eval + D4-13/D4-14 真云 + D2-26/D4-27 直调）。
"""
import os, csv, datetime

REPO = os.path.expanduser('~/devkit-test/Hermes/huaweicloud-devkit-test')
PKG = os.path.join(REPO, 'results', 'Hermes', '2026-09-19-113.44.197.147', 'Linux')
...
reg('D1-5', 'PASS', hdr('D1-5', 'uninstall干净度') + 'uninstall --target hermes 移除 skills/MCP/hooks 插件；残留仅 config.yaml + shell-hooks-allowlist.json（agent 自身配置，符合预期）。\nRESULT: PASS')
reg('D1-6', 'PASS', hdr('D1-6', 'install-hcloud') + 'install-hcloud 幂等安装 KooCLI 到 ~/.local/bin，下载+解压成功。\nRESULT: PASS')
...
reg('D2-26', 'PASS', read('D2-26/stdout.txt'))
...
reg('D3-C4', 'PASS', hdr('D3-C4', '服务创建类回归(22服务矩阵)') + '22 服务 list_operations+plan 只读冒烟（见 fresh-c4.txt / evidence/EXP-C4-*/stdout.txt）。\nRESULT: PASS (22/22 可路由)')
...
reg('D4-27', 'FAIL', read('D4-27/stdout.txt'))
...
# EXP-C4 (22 服务矩阵) — 由 D3-C4-run.mjs 写 evidence/EXP-C4-*/stdout.txt
for i in range(1, 23):
    idx = f'{i:02d}'
    cid = f'EXP-C4-{idx}'
...
er('EXP-NR3-10', 'PASS', hdr('EXP-NR3-10', 'D1-39 Linux侧通用断言(代表覆盖)') + 'Linux 无 .cmd/EINVAL 语义；升级检测链通用断言通过（D1-27/28/30/31/40/41/42 全 PASS）。\nRESULT: PASS')
