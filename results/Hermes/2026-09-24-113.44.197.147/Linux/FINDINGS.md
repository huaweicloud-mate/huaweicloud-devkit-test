...
> **落盘路径**：`results/Hermes/2026-09-24-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-24 05:14（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.7-next.1（npm @next，gitHead `657ceb7`，PR #806）
> **执行方式**：2026-09-24 `run-today.sh` 22 支源码级探针 fresh 重跑（v1.1.7-next.1）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零 / D3-S3 沙箱预览）+ D2-26/D4-27 源码级直调探针 + 收尾补删 OBS 遗留空桶归零
> **提单说明**：SUT 由 v1.1.7-next.0@0790e92a 升级至 v1.1.7-next.1@657ceb7，`git diff` 确认为帮助文案/文档变更（`tools.mjs` apiKeyHint + `huawei-sandbox/SKILL.md` + test 文件，零逻辑改动），全部缺陷实测复现、根因未变；经上游 open issue 查重命中已跟踪缺陷单，不重复开单（`file_issue.py` 内置历史查重 → HISTORY_LINKS.md）。
...
- **现象**：沙箱 `upload_project` 成功、`deploy_nginx` ok、`deploy_check` 返回 `nginx_serving=PASS`（v1.1.6 已归一），但 `devbridge_tunnel=FAIL`、`tunnel_url_accessible=FAIL (no tunnel)`、`publicUrl=undefined`。
- **断言**：沙箱预览场景终点应返回可访问公网 URL（`nginx_serving=true` + `devbridge_tunnel=true` + `tunnel_url_accessible=true`）。
- **根因**：沙箱侧 DevBridge 隧道未建立（v1.1.6 迁移 DevBridge 0.2.x：s2 gateway + API Key auth 后，CLI 沙箱 connect 流未建立隧道出公网 URL）。
