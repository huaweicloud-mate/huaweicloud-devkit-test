...
> **落盘路径**：`results/Hermes/2026-09-25-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：`2026-09-25 05:20`（北京时间）
> **被测版本**：`v1.1.7`（npm latest，gitHead `7456d05`，merge PR #813 release-1.1.7）
> **版本差异说明**：v1.1.7 相对上轮 v1.1.7-next.1 有实质代码变更——`auth/credentials.mjs`(+127)、`hcloud-cli.mjs`(+119)、`setup-cli.mjs`(+68)、`safety-policy.mjs`/`risk-rule-engine.mjs` 的 redact 正则（补 `token` 关键字 + `admin[_-]?pass` 变体）。35+ 支探针对 7456d05 全量 fresh 重跑，其中 **D4-27 双路径脱敏上轮缺陷（裸 token / admin-pass 变体）已修复转 PASS**；其余缺陷按根因原样复现，另新增 P0 用例 D9-12/D9-13（D9-13 全通过，D9-12 两处安全基线缺口见 #5）。
...
## #5【P0】D9-12 initialize 握手协议安全基线两处缺口（新用例）
