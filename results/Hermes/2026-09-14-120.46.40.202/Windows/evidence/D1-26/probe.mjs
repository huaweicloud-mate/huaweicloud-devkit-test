...
try { await request('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'p',version:'1'}});
  ...
  console.log("check_update found:", !!checkUpdate);
  console.log("upgrade found:", !!upgrade);
  if (checkUpdate) {
    console.log("check_update has description:", !!checkUpdate.description);
    console.log("check_update has inputSchema:", !!checkUpdate.inputSchema);
  }
  if (upgrade) {
    console.log("upgrade has description:", !!upgrade.description);
    console.log("upgrade has inputSchema:", !!upgrade.inputSchema);
  }
  if (checkUpdate && upgrade && checkUpdate.description && checkUpdate.inputSchema && upgrade.description && upgrade.inputSchema) {
    console.log("RESULT: UPDATE_TOOLS_OK");
  } else { console.log("RESULT: UPDATE_TOOLS_FAIL"); }
