...
try { await request('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'p',version:'1'}});
  ...
  for (const t of tools) {
    if (!t.name || !t.description || !t.inputSchema) { allValid = false; console.log("Invalid:", t.name); }
    if (seen.has(t.name)) { dupes.add(t.name); }
    seen.add(t.name);
  }
  console.log("All valid (name+desc+schema):", allValid);
  console.log("Duplicates:", dupes.size);
  if (allValid && dupes.size === 0 && tools.length >= 39) { console.log("RESULT: COMPLIANCE_OK"); }
  else { console.log("RESULT: COMPLIANCE_FAIL"); }
