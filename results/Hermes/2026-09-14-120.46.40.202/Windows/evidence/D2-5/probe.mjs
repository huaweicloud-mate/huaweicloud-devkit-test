...
try { await request('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'p',version:'1'}});
  ...
  // Check auth_status with credentials present (our env has them)
  const status = await call('huaweicloud_auth_status', {});
  const text = status.result?.content?.[0]?.text || '';
  const parsed = JSON.parse(text);
  console.log("credentialsConfigured:", parsed.credentialsConfigured);
  console.log("kooCliInstalled:", parsed.kooCliInstalled);
  console.log("kooCliStatus:", parsed.kooCliStatus);
  console.log("RESULT: AUTH_STATUS_CHECKED");
