...
try { await request('initialize',{protocolVersion:'2024-11-05',capabilities:{},clientInfo:{name:'p',version:'1'}});
  ...
  const skills = await call('huaweicloud_retrieve_skill', { query: 'ECS' });
  console.log("isError:", skills.result?.isError);
  const text = skills.result?.content?.[0]?.text || '';
  console.log("content length:", text.length);
  console.log("content (first 300):", text.substring(0, 300));
  console.log("RESULT: SKILL_RETRIEVAL_OK");
