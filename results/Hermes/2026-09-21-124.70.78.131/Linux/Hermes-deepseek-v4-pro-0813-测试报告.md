...
> **生成时间**：`2026-09-21 05:20`（北京时间）
> **执行归档**：`results/Hermes/2026-09-21-124.70.78.131/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：`FAIL`（10 项产品缺陷，其中 9 项历史查重命中上游 open issue 不重复提单；1 项新增：D1-68 region 优先级 SPEC-MISMATCH，待统一提单）
...
| 真云凭证 | cn-north-4（AK/SK 已配置 + 只读子账号 test001；本节真机建删 VPC/OBS/沙箱并归零验证） |
| daily 基础用例 | 设计级 100 / 展开级 43（Hermes+Linux 预筛后） |

> 本轮 SUT 未变（v1.1.5，gitHead e7ed6f66），与 09-20 同一版本、同一 daily 用例集（设计级 100 + 展开级 43）。全部探针/harness 重新执行并落盘今天的新鲜证据；真云用例如实真机建删归零。
...
| 红线（I 类）违规 | 0 |
| 资源释放 | 本轮 Hermes 建删资源（VPC×2 + OBS bucket×1 + 沙箱 session×1）全部归零 |

> v1.1.5 复测结论（与 09-20 一致）：已修复有效（`printenv HW_SECRET_KEY`→deny、`sh -c "hcloud ecs DeleteServer"`→deny、未知 method→-32601、裸 `env|grep HUAWEICLOUD`→deny）；残留缺口（`env|grep HW_ACCESS_KEY`、`sh -c "env|grep …"`、未知 tool→-32603、HCL broad IAM、fail-open、中文路由 21.4%、redactString 字符串路径）。
...
> 根因为 v1.1.5 源码文件:行号；10 项中 9 项历史查重命中 open issue 不重复提单，1 项新增待提单。完整格式见 FINDINGS.md。

| # | 级别 | 用例ID | 缺陷描述 | 断言 | 根因（文件:行号） | 状态 |
|---|---|---|---|---|---|---|
| 1 | P0 | D4-2 | env 打印拦截未覆盖 `HW_` 前缀 | `env\|grep HW_ACCESS_KEY` 应 deny | `safety-policy.mjs:398-399` | 历史 |
| 2 | P0 | D4-16 | env-dump 被 shell 包裹穿透 | `sh -c "env\|grep …"` 应 deny | `safety-policy.mjs:398` | 历史 |
| 3 | P0 | D4-21 | HCL broad IAM 未拦截 | HCL `actions=["*"]` 应 deny | `cloud-risk-rules.json:179,196` | 历史 |
| 4 | P1 | D9-2 | 未知 tool 错误码未区分 -32602 | 未知 tool 应 -32602 | `mcp-server.mjs:169` | 历史 |
| 5 | P1 | D4-17 | hook 模糊输入 fail-open | 异常输入应 deny | `risk-rule-engine.mjs:106` | 历史 |
| 6 | P1 | D10-3 (+S5/S6/S7) | 中文意图路由 21.4% 缺失 | 中文意图应命中（≥90%） | `tools.mjs:1778-1908` | 历史 #705 |
| 7 | P1 | D4-27 | 字符串路径裸 token/admin-pass 未脱敏 | 双路径无明文残留 | `safety-policy.mjs:42` | 历史 #726 |
| 8 | P2 | D4-25 | Python hook 写操作分类为 cli:invoke | 写操作应 cli:write | `hooks/huaweicloud-safety.py:46` | 历史 #752 |
| 9 | P2 | D4-26 | findings.evidence JSON 带引号 key 明文泄漏 | evidence 应 `<redacted>` | `risk-rule-engine.mjs:19` | 历史 #761 |
...
| FunctionGraph（D3-S6） | 未创建(路由 MISS) | — | 未创建无残留 ✓ |
| RDS（D3-S7） | 未创建(仅路由命中 RDS) | — | 未创建无残留 ✓ |
...
3. **D1-68 region 优先级**需维护者确认契约方向（HUAWEICLOUD_REGION vs HW_REGION 谁优先），同步修正用例或实现（本轮唯一新增，已提单）。
