#!/usr/bin/env bash
# Hermes Linux 2026-09-21 每日测试 —— 全量重跑（v1.1.5）
set -u
...
HDKMCP="$HDK/plugins/huaweicloud-core/src/mcp-server.mjs"
SAMPLE="$P/sample-react"
ISO_HOME="$HOME/devkit-test/Hermes/iso-home-20260921"
...
echo "===== [34] realcloud-probe (D2-1/D3-C4/D4-14/D4-18/19/20) ====="
cd "$BASE/evidence/realcloud-probe"
node probe.mjs > stdout.log 2>&1; echo "exit=$?"; grep -c "OK\|FAIL" stdout.log

echo "===== [35] D4-13 只读子账号 ====="
cd "$BASE/evidence/D4-13"
node probe.mjs > stdout.log 2>&1; echo "exit=$?"; tail -3 stdout.log
