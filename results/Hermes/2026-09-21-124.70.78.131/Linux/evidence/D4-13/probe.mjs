// D4-13 最小权限凭证通过率（Hermes Linux 2026-09-18）
...
function run(cmd) {
  try {
    const r = execSync(cmd, { encoding: 'utf8', shell: '/bin/bash', timeout: 45000, maxBuffer: 10 * 1024 * 1024 });
    return { code: 0, out: r };
  } catch (e) {
...
for (const [svc, op] of reads) {
  ...
  const denied = /not authorized|Policy doesn|no sufficient|\b403\b|EVS\.1027|IMG\.0026|SYS\.0403|forbidden|PolicyNotAuthorized/i.test(r.out);
