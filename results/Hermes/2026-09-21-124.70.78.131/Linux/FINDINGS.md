...
> **落盘路径**：`results/Hermes/2026-09-21-124.70.78.131/Linux/FINDINGS.md`
...
- **状态**：历史查重命中 #752（AtomCode/Linux 已提单），不重复提单

## #9【P2】D4-26 findings.evidence 脱敏未覆盖 JSON 带引号 key（明文 secret_key/adminPass 泄漏）

- **现象**：v1.1.5 直调 `risk-rule-engine.evaluateArtifacts`——触发 `hwc-iam-admin-policy`（deny）的 JSON 制品含 `"secret_key":"SKSECRETVALUE9"`、`"adminPass":"MyP@ss12345"` 时，`findings[].evidence` 原样保留明文（仅 access_key 被 `AKIA***` 部分脱敏）。
...
- **状态**：历史查重命中 #761（OfficeAce/Windows 已提单），不重复提单
