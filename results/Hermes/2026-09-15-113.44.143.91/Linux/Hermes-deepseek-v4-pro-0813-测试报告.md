...
> **结论**：`FAIL`（复现确认 4 项 P0 缺陷，安全红线未收敛）
...
| 真云凭证 | session 级（已配置但实测 `APIGW.0301 Unauthorized`，真云未使用） |
| 测试类型 | 源码级断言库（96 断言）/ 真机 CLI（install/doctor/status/update/uninstall）/ MCP 协议 stdio / hook 双路径 / 文档与静态补充探针 |
| daily 基础用例 | 设计级 81 / 展开级 48（按 Hermes+Linux 预筛后）/ 追踪表 183 行 |

> **执行方法**：复用源码级断言库 `_lib/hdk-asserts.mjs`（直调 `hdk/plugins/huaweicloud-core/src/*`，证据落 `evidence/<case-id>/stdout.log` 与 stdout.txt）；CLI/hook/协议/白名单/文档探针落 `evidence/*-stdout.txt`。
...
| BLOCKED | `6` | 真云凭证无效 / Windows 缺失 / 下载 Permission denied |
...
| 7 | P1 | `D10-3`+`EXP-E*` | 中文意图路由未命中 | 中文意图命中服务 | 12/15 miss | `tools.mjs:1776-1908` | 已提单(复现) |
...
| `D1-39` | Windows 专属（`.cmd`/EINVAL 语义） | Windows 机器 | 补 Windows 终端 |
