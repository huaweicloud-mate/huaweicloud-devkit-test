...
> **落盘路径**：`results/Hermes/2026-09-17-124.70.78.131/Linux/FINDINGS.md`
> **生成时间**：`2026-09-17 07:12`（北京时间）
> **被测版本**：`v1.1.5`（npm latest 正式版，gitHead `e7ed6f66`）
> **本清单是统一提单脚本的解析输入**：`scripts/file_issue.py` 硬编码解析标题与「根因」字段，格式严格遵循。
> **结论**：#1–#7 产品缺陷全部经历史查重命中上游 open issue，**不重复提单**（v1.1.5 对 #1/#2/#4 有部分修复，但各用例仍残留可复现缺口）；#8 为【非产品缺陷】（凭证供应），不计入提单。
...
- **现象**：v1.1.5 复测——`eval/harness/run-eval.mjs` 对 15 条评测集跑出 HIT=3 MISS=11 N/A=1（准确率 21.4%）：EXP-E06(DCS)/E09(CCE)/E15(代金券) 命中；E01~E05/E07/E10~E14 全部回退 `Run hcloud --help` 未命中。源码级直调同源：中文「创建云服务器/对象存储桶上传对象/权限管理」均回退，英文正确命中；仅「部署网站」「领取代金券」含 CJK 关键字命中。
...
## #8【非产品缺陷】D4-13 只读子账号 test001 读权限通过率 5/6（凭证供应问题）

- **现象**：真机实测 test001 只读子账号跑 6 条只读命令，5 条通过（ECS NovaListServers/VPC ListVpcs/EVS ListVolumes/CES ListMetrics/EIP ListPublicips），仅 IMS ListImages 被 IAM 拒绝；写操作 VPC CreateVpc 被 IAM 拒绝（PolicyNotAuthorized，符合预期），归零验证通过。较 09-16（0/6）有改善但未达 100%。
- **断言**：只读子账号应「只读 100% 可用、写被 IAM 拒绝」。
- **说明**：test001 的 IMS 只读 policy（ims:images:list）未在账号侧挂载，属凭证/环境供应问题，非 huaweicloud-devkit 产品缺陷（产品正确透传 IAM 拒绝）。
