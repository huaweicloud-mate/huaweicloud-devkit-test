...
> **执行归档**：`results/Hermes/2026-09-22-1.94.218.129/Linux/`
> **被测对象**：huaweicloud-devkit v1.1.5（npm 官方 latest，gitHead `e7ed6f6`）
> **结论**：`PARTIAL`（3 项 P0 历史缺陷复现 D4-16/D2-4/D4-23 + 11 项 P1/P2 缺陷；真云 E2E 真机完成且归零；本轮较 09-21 结果完全一致，无新增缺陷、无缺陷修复回归）
...
| OS / 架构 | Linux x86_64（Ubuntu 24.04） |
| Node / npm / Python | Node v22.13.0 / npm 10.9.2 / Python 3.12.3 |
| 被测版本（SUT） | v1.1.5（npm 官方 latest，gitHead `e7ed6f6`） |
...
> **执行方法**：探针（.mjs）直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数，结果落 `evidence/<id>/stdout.log`；协议走 `eval/harness/protocol-probe.mjs`，评测走 `eval/harness/run-eval.mjs`；真云走 hcloud 实机建删归零。本轮 28 组探针 + 2 个 harness 全部重跑，证据全部本机新鲜落盘（非复用昨日 stdout.log）。

> **环境漂移提示**：本机 npm 私有镜像 `127.0.0.1:45998` 的 `latest` 标签仍停在 1.1.3（无 1.1.5），与官方 npm registry 漂移；且 `prepare_env.py --update` 会经私有镜像把被测包降级回 1.1.3。本轮已手动修正：固定 SUT=官方 latest 1.1.5 重装，并 checkout hdk 至 `e7ed6f6`，与 09-20/09-21 被测版本一致（跨日可比）。
...
**本轮与上一轮（09-21）缺陷完全一致**：15 项缺陷逐一在 v1.1.5（gitHead `e7ed6f6`）复现，无新增缺陷、无缺陷修复回归（上游未发新版，`latest` 仍为 1.1.5）。
...
- 本轮结果与 09-21 完全一致（15 项缺陷全部复现、127 项执行无 diff），说明上游未发新版、无修复回归、无新增退化。
- 本轮未覆盖（范围）：D3-S6/D3-S7 真云多服务编排、真实 Agent 会话中文意图路由（需 LLM harness）。
- 建议：上游按 FINDINGS.md 推进修复——重点 P0 #1/#2/#3（安全策略 + 规则注入 + 凭证脱敏），P1 #6/#8（-32602 错误码 + serviceCatalog 路由）。
- 环境建议：修复本机 npm 私有镜像 `127.0.0.1:45998` 与官方 npm registry 的版本漂移（latest 停在 1.1.3 vs 官方 1.1.5），否则 `prepare_env --update` 每次都会把被测包误降级回 1.1.3。