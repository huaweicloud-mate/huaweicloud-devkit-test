...
> **生成时间**：`2026-09-26 05:08:00`（北京时间）
> **执行归档**：`results/Hermes/2026-09-26-124.70.78.131/Linux/`
...
> **较上轮（v1.1.7 2026-09-25）变化**：**无代码变更**（版本与 gitHead 完全一致）。今日全量 fresh 重跑，35+ 支探针 + 真云 E2E（42 项建删资源）结果与上轮一致：PASS 77/FAIL 12/BLOCKED 6/SPEC 3/NOT_RUN 4（设计级），PASS 32/FAIL 11（展开级）。15 项缺陷全部按根因原样复现；D9-12 两处安全基线缺口上一轮已提单（#814），本轮为历史复核。
...
| 5 | P0 | D9-12 | initialize 握手安全基线两缺口（版本检查不在此期 + 非法时序不拒） | mcp-protocol.mjs:32-59 | 历史复核 #814 |
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| VPC（realcloud-probe D4-14） | 是（tctest-hermes-20260926-*） | 已删 | ListVpcs 不含本次资源（containsOwn=false） |
...
- 新增 P0 用例 D9-12 两处缺口（版本检查不在 initialize 阶段 + 非法时序不拒 -32600）上一轮已统一提单（上游仓 #814），本轮复核仍复现。
