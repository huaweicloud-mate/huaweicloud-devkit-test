...
> **落盘路径**：`results/Hermes/2026-09-21-113.44.143.91/Linux/FINDINGS.md`
> **生成时间**：2026-09-21 05:32 CST（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.5（npm latest，gitHead e7ed6f66）
...
- **现象**：`redactSecrets({ak:'AKID12345678', sk:'SK1234567890abcdef'})` 对裸 `ak`/`sk` 键不脱敏；对字符串 `ak=AKID12345678 sk=SK1234567890abcdef` 返回原文不脱敏（`show_profile_redacted` 路径输出明文 AK/SK）。
...
- **证据**：`evidence/d2-auth/stdout.log`（D2-4 redact-json 实测泄漏）；`evidence/deep-probe.console.log`（redactSecrets-lower-ak FAIL）。
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:397-419` — `classifyTextCommand` 的 env-dump 门 `(^|\s)(env|printenv…)` 不拆 shell 包裹（`stripExecutable` 仅作用于 `classifyHcloudArgs` 的 hcloud 命令缠绕，#650 未覆盖文本命令 env-dump）。
- **影响**：凭证环境变量可经 shell 包裹绕过安全钩子读取（P0）。
- **证据**：`evidence/d4-security/stdout.log`（D4-16 wrap-sh FAIL=allow）。

## #3【P0】D4-17 hook 模糊测试 fail-open（无规则命中默认 allow）

- **现象**：`evaluateCommandRisk` 对无规则命中的畸形/未知输入返回 `decision='allow'`（fail-open），非 fail-closed。
- **断言**：异常/未识别输入应默认拒绝（deny，fail-closed），不得误放行。
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:105-108` — `evaluate` 末尾 `decision: hasDeny ? 'deny' : hasWarn ? 'warn' : 'allow'`，无命中即 allow。
- **影响**：规则盲区下危险命令被放行，fail-closed 预期落空（P0 安全语义）。
- **证据**：`evidence/deep-probe.console.log`（D4-17 行为确认 fail-open）。

## #4【P0】D4-23 全局规则 huawei-agent-rules 注入未生效
...
- **现象**：`redactSecrets` 与 `redactOutput` 双路径对小写 `ak=`/`sk=` 均不脱敏（`redactOutput('AK="AKID12345678"')` 大写可脱敏、小写 `ak=` 泄漏）。
...
- **证据**：`evidence/deep-probe.console.log`（redactSecrets-lower-ak FAIL）。

## #6【P1】D10-3 路由准确率 21.4%（中文意图路由大面积缺失）

- **现象**：`run-eval.mjs` 评测集 15 条中文意图，HIT=3 / MISS=11 / N/A=1，准确率 21.4%（<90%）；EXP-E01~E15 中 11 条 MISS（含 ECS/EIP/RDS/CBR/FunctionGraph/BSS/CES/ELB/IAM/OBS 复合意图）。
...
- **影响**：Agent 无法正确路由中文任务到服务，复合意图不拆多服务（P1）。
- **证据**：`evidence/run-eval.log`（MISS=11）、`eval/results/eval-run-20260920211123.csv`。

## #7【P1】D9-2 JSON-RPC 非法参数无 -32602 错误对象

- **现象**：未知方法返回 `-32601` 正确；但非法参数调用无 `error` 对象（应 `-32602`）。
...
- **证据**：`evidence/protocol-probe.log`（D9-2b-invalid-params FAIL=无 error 对象）。

## #8【P1】D3-S3 沙箱预览 URL 未就绪（deploy_check nginx FAIL）
...
- **断言**：终点必返回可访问公网 URL（HTTP 200/nginx serving PASS）。
- **根因**：沙箱 deploy 链 `deploy_check` nginx_serving 状态 FAIL（预览服务未就绪）。
- **影响**：沙箱预览场景终点不可达（P1）。
- **证据**：`evidence/_scenarios3-probe.mjs` 输出（sandbox-connect PASS / deploy-check-url nginx_serving=FAIL）。

## #9【P2】D4-25 Python hook 写命令遥测分类错误（cli:invoke ≠ cli:write）
...
## #10【P2】D4-26 findings 证据脱敏漏小写 ak=/sk=
...
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:24` — `(AK|SK)` 正则缺 `i`（与 D2-4 同源）。
- **影响**：高危触发证据中第三凭证明文泄漏（P2）。
- **证据**：`evidence/deep-probe.console.log`（redactEvidence 逻辑同 D2-4 根因）。

## #11【P2】D8-9 sanitizeValue 不脱敏凭证

- **现象**：`sanitizeValue('ak=AKID12345678 token=abc')` 返回原文（仅 trim/截断），AK/token 明文未移除。
- **断言**：sanitizeValue 移除 AK/SK/token 等敏感值，不改变合法值。
- **根因**：`plugins/huaweicloud-core/src/telemetry/telemetry.mjs:189-196` — 仅 `replace([\r\n\t]) + trim + 截断`，无凭证脱敏。
- **影响**：遥测事件值中第三凭证明文上报（P2）。
- **证据**：`evidence/deep-probe.console.log`（sanitize-cred FAIL）。

## #12【P2】D8-1 文档与实现工具数漂移（39 vs 40）
...
- **证据**：`evidence/d3s5.console.log`（composite-obs+sandbox FAIL、single-ecs FAIL）。

## #14【P1】D9-9 tools/call 取消能力未声明（SPEC-MISMATCH）
...
- **证据**：`evidence/protocol-probe.log`（D9-9a-capabilities.cancellation SPEC-MISMATCH）。

## #15【P1】D4-24 确认令牌精确 JSON 契约未实现（SPEC-MISMATCH）
...
- **根因**：`plugins/huaweicloud-core/src/tools.mjs:1246-1257` — 无 CONFIRM_TOKEN_EXPIRED/already_processed 字段实现。
- **影响**：审批流令牌过期/重放的机器可断言契约缺失（契约漂移，待裁决）。
- **证据**：`evidence/_p0remaining-probe.mjs` + 源码 tools.mjs:1246-1257。

---

> 合计 15 项缺陷（P0×4 / P1×6 / P2×5）；其中 D2-4 / D4-26 / D4-27 三项同根因（大小写敏感脱敏正则），D10-3 / D3-S5 两项同根因（routeMap 中文路由缺失）。