...
> **落盘路径**：`results/Hermes/2026-09-26-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-26 05:33（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.7（npm @latest，gitHead `7456d05`，PR #813）
> **执行方式**：2026-09-26 `run-today.sh` 35+ 支源码级探针 fresh 重跑（v1.1.7）+ `probe-d9-12.mjs`/`probe-d9-13.mjs` P0 探针 + `eval/harness/run-eval.mjs` + 真云 E2E（D4-13 只读子账号 / D4-14 VPC 建删+CTS 归零 / D3-S1/S2/C13/S3/S4/S6 真云场景建删归零）+ D2-26/D4-27 源码级直调 + 收尾补删 OBS 遗留空桶归零。
> **提单说明**：SUT v1.1.7@7456d05 与上一轮（2026-09-25）**完全一致，无任何代码变更**。今日全量 fresh 重跑，15 组缺陷（12 FAIL + 3 SPEC-MISMATCH + 若干测试侧/非产品缺陷）按根因**原样复现**；其中 D9-12 两处安全基线缺口上一轮已统一提单（上游仓 #814），本轮视为历史缺陷复核，经 `file_issue.py` 内置查重 → HISTORY_LINKS.md 关联，不重复开单。
...
## #5【P0】D4-23 全局规则 huawei-agent-rules.mdc 安装未注入

- **现象**：隔离 HOME 执行 `install --target hermes` 后，产物仅含 `safety/rules/cloud-risk-rules.json`，仓库根 `rules/huawei-agent-rules.mdc`（含 MUST 约束）零复制零引用。
- **断言**：install 后安装目标目录应包含全局 MUST 级规则文件 `huawei-agent-rules.mdc`。
...
- **现象**：`hcloud ecs ChangeServerOsWithoutCloudInit`、`hcloud vpc ChangeVpc`、`hcloud rds ChangeInstanceConfiguration` 未被审批门拦截（Create/Delete 正常 deny）。
...
- **影响**：`obsutil` 小写凭证与文本路径 `ak=` 形式访问令牌仍经日志/对话泄漏。
...
- **根因**：`plugins/huaweicloud-core/src/proxy/proxy-config.mjs:42-47` `shouldBypassProxy()` 仅做 hostname 后缀匹配，无 CIDR 网段匹配逻辑。
...
- **根因**：沙箱侧 DevBridge 隧道未建立（v1.1.7 DevBridge 0.2.x CLI 沙箱 connect 流未建立隧道出公网 URL）。
- **影响**：场景「本地网页出公网预览 URL」终点仍不可达（仅 nginx 侧归一）。
- **证据**：`evidence/D3-S3/stdout.txt`
...
- **根因**：`plugins/huaweicloud-core/src/tools.mjs` `serviceCatalog()` routeMap 英文-only（同 #10）。
...
- **现象**：`hcloud ecs DeleteServer`、`hcloud ecs CreateServer` 均分类 `cli:invoke`（期望 `cli:write`）；`configure show` 分类 `cli:read`。
- **断言**：只读→`cli:read`、写→`cli:write`、其他→`cli:invoke`。
- **根因**：`plugins/huaweicloud-core/hooks/huaweicloud-safety.py` `record_cli_event()` `WRITE_OPERATION_RE` 写前缀匹配失败故落 `cli:invoke`。
...
## #20【P2】D8-9 遥测值脱敏：sanitizeValue 未脱敏敏感值（SPEC-MISMATCH）

- **现象**：`sanitizeValue('ak=AK123456 sk=SKsecret token=Tok123')` 返回原文（未脱敏）。
...
- **断言**：各中文意图应路由到对应服务（查云主机→ECS、静态网站→OBS、绑 EIP→EIP、MySQL→RDS、备份→CBR、函数→FunctionGraph、费用→BSS、监控告警→CES、HTTPS 证书→ELB、权限审计→IAM）。
...
## #22【P0】D9-12 initialize 握手非法时序未拒绝（上一轮已提单 #814）

- **现象**：未先 `initialize` 直接发 `tools/list`，MCP server 返回 200（40 工具），未按设计预期返回 JSON-RPC 错误。
- **断言**：非法时序（未 initialize 先 tools/list）应被拒（设计预期 -32600）。
- **根因**：`plugins/huaweicloud-core/src/mcp-server.mjs` `handleMessage` 未跟踪 initialize 状态；`mcp-protocol.mjs` `dispatch` 对 `tools/list` 无条件处理，无「未初始化」守卫。
