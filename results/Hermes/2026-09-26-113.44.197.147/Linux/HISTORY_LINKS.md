...
## D4-23 全局规则 huawei-agent-rules.mdc 安装未注入
...
- 今日证据：`evidence/D3-S3/stdout.txt`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D8-9 遥测值脱敏：sanitizeValue 未脱敏敏感值（SPEC-MISMATCH）
...
## D9-12 initialize 握手非法时序未拒绝（上一轮已提单 #814）
