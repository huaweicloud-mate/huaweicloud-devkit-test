...
## D2-4 redactString 漏小写 ak=/sk=（obsutilconfig 格式）
- 今日证据：`evidence/D2-4/stdout.log`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#712](https://github.com/huaweicloud/huaweicloud-devkit/issues/712)（open）**[P1] D4-6 adminPass ?????????(CodeArtsWork v1.1.5 ????)**
    - 历史单内容：## ???? - **??**: D4-6 - **??**: P1 - **???**: CodeArtsWork (GLM-5.2) - **OS**: Windows - **????**: v1.1.5 ## ?? hook_check_command('hcloud ECS CreateServers --adminPass MyPassword123') ?? ^Gllow,admi
...
- 今日证据：`evidence/cli-stdout.txt`（D4-23 段：`[缺] 未找到 huawei-agent-rules.md`）
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#683](https://github.com/huaweicloud/huaweicloud-devkit/issues/683)（open）**[测试报告] huaweicloud-devkit v1.1.4 全量测试缺陷合并单（10 项）**
    - 历史单内容：## 测试概览 - 被测版本：v1.1.4 - 缺陷：10 项 ## 缺陷清单 ### 1. [P0] 凭证脱敏漏小写 ak=/sk=（obsutilconfig 格式） - **描述**：`redactSecrets('ak=AK123456 sk=SKsecret')` 返回原文未脱敏；而 `access_key=`/`secret_key=` 大写键与对象路径 `{AK,SK}` 均正常脱敏
  - [#679](https://github.com/huaweicloud/huaweicloud-devkit/issues/679)（open）**[测试报告] huaweicloud-devkit v1.1.4 全量测试缺陷合并单（8 项）**
...
## D4-8 Python/Node 钩子策略不一致（写操作与 configure show 放行）
...
- 今日证据：`evidence/D10-3/stdout.log` + `eval/results/eval-run-*.csv`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#689](https://github.com/huaweicloud/huaweicloud-devkit/issues/689)（open）**[测试报告] huaweicloud-devkit v1.1.4 每日测试缺陷合并单（5 项）**
    - 历史单内容：## 测试概览 - 被测版本：v1.1.4 - 缺陷：5 项 ## 缺陷清单 ### 5. [P1] Python/Node 安全钩子策略不一致 - **描述**：同一高危输入（`hcloud configure show`、`hcloud ECS DeleteServers`），Node hook 返回 `deny`，Python hook 返回空（放行）。 - **预期（精确断言）**：Pyt
  - [#683](https://github.com/huaweicloud/huaweicloud-devkit/issues/683)（open）**[测试报告] huaweicloud-devkit v1.1.4 全量测试缺陷合并单（10 项）**
...
- 今日证据：`evidence/D9-2/stdout.log` + `eval/results/protocol-probe-*.json`
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#704](https://github.com/huaweicloud/huaweicloud-devkit/issues/704)（open）**[D9-2] tools/call 缺少 required 参数校验**
    - 历史单内容：## 缺陷描述 `tools/call` 请求缺失 inputSchema 中标注为 required 的参数时，server 未返回参数校验错误（-32602 Invalid Params），而是直接执行导致 undefined 异常。 ## 复现步骤 1. 启动 MCP server 2. 调用任意工具但故意省略 required 参数：`{"jsonrpc":"2.0","method":"
...
- 今日证据：`evidence/D9-9/stdout.log` + `eval/results/protocol-probe-*.json`
- **关联历史单（已作为缺陷提过，本次为复核）**：
