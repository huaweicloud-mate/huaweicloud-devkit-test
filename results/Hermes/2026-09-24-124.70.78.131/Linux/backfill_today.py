# -*- coding: utf-8 -*-
"""Hermes Linux 2026-09-24 每日测试：证据分发（stdout.txt）+ 回填三 CSV（v1.1.7-next.1）。"""
...
# realcloud + D4-13 + D4-23 证据已由 run_all.sh / 独立探针直接生成 stdout.txt
print("== realcloud / D4-13 / D4-23 (已有 stdout.txt + probe.mjs) ==")
write_case("_baseline", "huaweicloud-devkit v1.1.7-next.1\ngitHead 657ceb7b\n", None)
print("  _baseline: v1.1.7-next.1 (gitHead 657ceb7b)")
...
BLOCKED_D = {
    ...
    "D3-S3":"【补环境】v1.1.7-next.1 沙箱 connect/upload(md5)/deploy/close 全链路真实执行，但 deploy 返回 url 为空、devbridge_tunnel FAIL（DevStation 公网隧道/预览外发环境缺依赖）",
}
