...
> **本清单是统一提单脚本的解析输入**：`scripts/file_issue.py` 硬编码解析标题与「根因」字段，格式严格遵循。每项均含「级别 + 现象 + 断言 + 根因（文件:行号）+ 证据」，根因已源码级定位（safety-policy.mjs / cloud-risk-rules.json / huaweicloud-safety.py / mcp-server.mjs / mcp-protocol.mjs / tools.mjs）。
> **结论**：4 项缺陷在 `v1.1.4` 正式版（1.1.4 GA）**复现确认，全部未修复**。均为已知缺陷，已统一跟踪于 **#652**（OPEN，本工作流于 2026-09-13 提单）。本轮按「不重复拆单」约定**不重复提单**，仅复测确认 + 回填状态。
...
- **状态**：已跟踪 #652-1（v1.1.4 复现确认）

## #2【P0】env-dump 规则被 shell 包裹穿透（D4-16）

- **现象**：`sh -c "env | grep HUAWEICLOUD_ACCESS_KEY"`、`bash -c "printenv HUAWEICLOUD_SDK_AK"`、`eval "env | grep HUAWEICLOUD_ACCESS_KEY"` 均返回 `allow, risk=not_huaweicloud`（应 `deny`）；对照组 `sh -c "hcloud csms ShowSecretVersion ..."`、`sh -c "cat ~/.config/huaweicloud/credentials.json"`、`sh -c "hcloud ecs DeleteServer --force"` 均正确 `deny`（直接形态 `env | grep HUAWEICLOUD_ACCESS_KEY` 也正确 `deny`）。
- **断言**：`sh -c "env | grep HUAWEICLOUD_ACCESS_KEY"` 应返回 `deny`（解开 shell 包裹后识别内层 env-dump）。
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:335` + `plugins/huaweicloud-core/safety/rules/cloud-risk-rules.json:35` — env-dump 检测 `(^|\s)(env|printenv|...)` 词边界匹配原始文本，`sh -c "..."` 包裹使内层 `env` 前为引号而非词边界，正则漏命中；而 destructive-delete / secret-read / credential-file 规则用子串/模式匹配，天然抗包裹，故仅 env-dump 规则被穿透。
- **影响**：shell 包裹绕过 hook，高危 env-dump 凭证提取可穿透预检。
- **证据**：`evidence/D4-16/stdout.log`（含 `wrap-probe.mjs` 补充探针）
- **状态**：已跟踪 #652-2（v1.1.4 复现确认）

## #3【P0】hook_check_artifacts 未拦截 Terraform HCL 形态 broad IAM 制品（D4-21 补充探针）

- **现象**：`evaluateArtifacts` 对 `resource "huaweicloud_iam_policy" "adm" { ... statement { effect="Allow" actions = ["*"] resources=["*"] } }` 返回 `decision=allow`、`findings` 空（应 `deny`）；`data "huaweicloud_iam_policy" ... { name = "AdministratorFullAccess" }` 同样 `allow`。对照 JSON 形态 `{"Statement":[{"Effect":"Allow","Action":"*","Resource":"*"}]}` 正确 `deny`（D4-21 主断言 PASS）。
...
- **根因**：`plugins/huaweicloud-core/src/mcp-server.mjs:169`（`handleMessage` catch 对所有 `dispatch` 异常硬编码 `code:-32603`）+ `plugins/huaweicloud-core/src/mcp-protocol.mjs:95`（`Unsupported method`）+ `plugins/huaweicloud-core/src/tools.mjs:1485`（`Unknown tool`）——异常未携带可区分错误码，被统一吞成 -32603。
- **影响**：MCP 客户端无法区分「方法不存在」与「内部错误」，错误处理语义失真。
- **证据**：`evidence/D9-2/stdout.log`
- **状态**：已跟踪 #652-4 / #638（v1.1.4 复现确认）