...
> **落盘路径**：`results/Hermes/2026-09-18-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-18 05:20（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.5（npm latest，gitHead `e7ed6f6`，PR #696）
> **执行方式**：`reverify-2026-09-18.sh` 16 支源码级探针 fresh 重跑（safety-policy/protocol/hooks/tools/matrix/auth/approval/c4/cli/d158）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号/D4-14 建删归零+CTS）+ 新增 D2-26/D4-27 源码级直调探针
> **提单说明**：本轮 11 项设计级缺陷 + 12 项展开级缺陷在 v1.1.5 实测**全部复现**，根因未变，经上游仓 `huaweicloud/huaweicloud-devkit` 历史 open issue 查重（GitHub API 实拉确认全部 open）**命中已跟踪缺陷单**，不重复提单（关联清单见 `HISTORY_LINKS.md`）。**本轮新增 1 项设计级缺陷 D4-27（redactSecrets/redactOutput 裸 `token=` 关键字未脱敏）**，为历史单未覆盖的新发现，据此单独提 1 张合并单。
...
- **现象**：隔离 HOME 执行 `install --target hermes` 后，全目录 `find` 无 `huawei-agent-rules.md` 任何产物（`[缺] 未找到 huawei-agent-rules.md`）。
...
- **证据**：`evidence/eval-harness.txt`、`evidence/fresh-matrix.txt`、`eval/results/eval-run-*.csv`
...
## #12【P1】D4-27 redactSecrets/redactOutput 双路径脱敏缺裸 token 关键字（本轮新增）
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:42` `redactString` 第二条 replace 的关键字列表 `(access[_-]?key|secret[_-]?key|security[_-]?token|x[_-]?auth[_-]?token|authorization|password|passwd|adminPass|credential)` **未包含裸 `token`**；`plugins/huaweicloud-core/src/hcloud-cli.mjs:587` `redactOutput` 复用 `redactSecrets`，故双路径同步缺口。小写 `ak=`/`sk=` 部分同 #3（D2-4），根因 `safety-policy.mjs:45` 缺 `/i`，属已跟踪 #683。
- **影响**：`token=` 形式的访问令牌经日志/对话输出泄漏，脱敏完整性不达标（D4-27 设计级 P1 新增用例）。
- **证据**：`evidence/D4-27/stdout.txt`、`evidence/D4-27/probe.mjs`
- **状态**：已提单 #726（本轮新增：裸 token 关键字；小写 ak/sk 部分已跟踪 #683）
