...
> **执行归档**：`results/Hermes/2026-09-16-113.44.197.147/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：**PARTIAL**（11 项设计级 FAIL + 12 项展开级 FAIL，均为 v1.1.4 已跟踪缺陷的历史复现；本轮 v1.1.5 已修复 D4-2 / D9-2 两项，另两项 BLOCKED 真云用例（D4-13/D4-14）消解为 PASS）
...
| 测试类型 | 源码级探针 / 真机 CLI（install/doctor/status）/ MCP 协议 stdio / 真云 E2E（建删归零 + CTS 审计 + 只读子账号最小权限） |
| daily 基础用例 | 设计级 78 / 展开级 48（Hermes+Linux 预筛后） |

> **执行方法**：探针脚本（.mjs）直调 `hdk/plugins/huaweicloud-core/src/*` 导出函数（classifyTextCommand / classifyHcloudArgs / redactSecrets / TOOL_DEFINITIONS 等）与 MCP stdio 协议直调，结果落 `evidence/<case-id>/stdout.txt`；CLI 真机执行记录日志；真云 E2E 用管理员凭证建最小规格 VPC→查 CTS→删除归零。本轮针对 v1.1.5（gitHead `e7ed6f6`）**全量 fresh 重跑**（`reverify.sh` 12 支探针 + D10 run-eval harness + D4-13/D4-14 真云补测）。
...
| 状态 | 数量 | 说明 |
|---|---|---|
| PASS | 66 | 有证据且通过 PASS 门禁（本轮 v1.1.5 修复 D4-2/D9-2 转 PASS，D4-13/D4-14 真云补测转 PASS，D3-C4 22 服务矩阵补全转 PASS） |
| FAIL | 11 | D4-16/D4-5/D2-4/D4-23/D4-4/D4-8/D4-17/D4-11/D10-3/D2-11/D8-1，根因见缺陷清单 |
| BLOCKED | 0 | 全部消解（D1-39 改 NOT_RUN、D4-13/D4-14 转 PASS） |
| SPEC-MISMATCH | 0 | D9-2 已在 v1.1.5 修复（-32603 → -32601） |
...
| 9 | P1 | D10-3 | 中文意图路由 miss | 中文意图 → 对应服务 | 11/15 miss（准确率 21.4%） | tools.mjs:1780-1878 英文-only | #651/#683/#679/#689 |
...
| 12 | P1 | EXP-E01~E14 | 中文意图路由 miss（展开级 12 条，E08 为诊断类） | 各自服务 | 11 MISS + 1 N/A(诊断) | tools.mjs serviceCatalog | #651/#683/#679/#689 |

> 完整现象/断言/根因/证据逐条见 `FINDINGS.md`；历史查重映射见 `HISTORY_LINKS.md`。
> **本轮 v1.1.5 修复项**：D4-2（HW_ 凭证 env 打印拦截，新增 credential-variable-reference 规则）、D9-2（JSON-RPC -32603 → -32601）均实测 **PASS**，不再列为缺陷。
> **残余提示（非阻断）**：D4-2 的 `env | grep HW_ACCESS_KEY`（grep-filter 形式、无 `$`/不含 printenv）仍 `allow`（safety-policy.mjs:398-419 新规则只匹配 `$VAR`/`printenv VAR`），见 FINDINGS #14。
...
|---|---|---|---|---|---|---|
| D1-39 | 设计级 | P0 | NOT_RUN | 调归属 | 实测 2026-09-16·Windows 专属 OS 用例（OS 列含「专属」）：升级检测链 `.cmd`/EINVAL 语义无法在本机 Linux 复现；Linux 侧由 EXP-NR3-10 通用断言已 PASS；解除=Windows 测试机执行 | OS 列已标 Windows-only，如需 Linux 侧独立断言可新增 Linux 专属用例 |

> 本轮 BLOCKED 已全部消解：D4-13（只读子账号 test001 凭证已下发，env 动态切换 PASS + 写被 IAM 拒绝 PASS）、D4-14（真云 VPC 建删归零 + CTS 审计 PASS）。无 BLOCKED 残留。
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| VPC（hdk-audit-513659009，22de0a6f-dbc7-4dbe-a837-0c857b0a88fe） | 是 | 已删 | ListVpcs 已不含该资源（归零） |
...
- 本轮 v1.1.5 已修复：D4-2（凭证 env 打印）、D9-2（JSON-RPC 错误码）。两处均实测 PASS。
- 11 项设计级 + 12 项展开级缺陷均为 v1.1.4 稳定版历史复现，根因未变，经上游 open issue 查重全部命中已跟踪单（#683/#679/#689/#671/#651），本轮不重复提单。
- 建议：中文意图路由（D10-3/EXP-E*，准确率 21.4%）与 Change* 写动词（D4-4/D4-5，policy.json 缺 Change 前缀）影响面最大，优先排期修复；D4-16 的 classifyTextCommand shell-wrap 解包（#650 只修了 classifyHcloudArgs 路径）仍留 bypass，建议将解包前移到 classifyTextCommand 路由前。