# D1 安装域探针（Hermes Linux 每日回归，隔离版）：同时隔离 HOME 与 HERMES_HOME
# 覆盖 D1-1(install) / D1-3(doctor) / D1-4(status/update 幂等) / D1-5(uninstall 干净度)
set -u
export PATH="$HOME/nodejs/bin:$HOME/bin:$PATH"
ISO_HOME="${1:-$HOME/devkit-test/Hermes/iso-home-$(date +%s)}"
rm -rf "$ISO_HOME"
...
find "$HERMES_HOME" -maxdepth 4 2>/dev/null | sort
