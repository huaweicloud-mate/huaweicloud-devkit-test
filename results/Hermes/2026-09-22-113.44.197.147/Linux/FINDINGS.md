...
> **落盘路径**：`results/Hermes/2026-09-22-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-22 05:25（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.5（npm latest，gitHead `e7ed6f6`，PR #696）
> **执行方式**：2026-09-22 `run-today.sh` 22 支源码级探针 fresh 重跑（security/supplement2/remaining/final/d1-41/tools/matrix/protocol/supplement/auth/approval/c4/newcases/d9-10 + cli/hook/hookchain/d158/d4-25）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零）+ D2-26/D4-27 源码级直调探针
> **提单说明**：SUT 与昨日同为 v1.1.5@e7ed6f6，全部缺陷实测复现、根因未变；经上游 open issue 查重命中已跟踪缺陷单，不重复开单（`file_issue.py` 内置历史查重 → HISTORY_LINKS.md）。
...
- **现象**：`serviceCatalog("帮我查一下我账号有哪些云主机")` 返回空 missed（未命中 ECS），后续 `run_readonly_command(ecs ListServersDetails)` 正常返回 0 实例。
...
- **现象**：沙箱 `upload_project` 成功、`deploy_nginx`/`deploy_check` 返回 `nginx_serving=FAIL`、`tunnel_url_accessible=FAIL`，`publicUrl` host 非法（`https://-8080.cn-north-4...`）。
