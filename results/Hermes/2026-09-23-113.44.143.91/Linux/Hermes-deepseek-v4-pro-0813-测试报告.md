...
> **执行归档**：`results/Hermes/2026-09-23-113.44.143.91/Linux/`
...
| OS / 架构 | Linux aarch64（113.44.143.91） |
...
> **执行方法**：复用参考机器探针逻辑（探针脚本路径改写到本机）fresh 重跑 22 支源码级探针（v1.1.6）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零 / D3-S3 沙箱预览）+ D2-26/D4-27 源码级直调探针。证据统一落 `evidence/<case-id>/stdout.txt`。
...
| 14 | P1 | D3-S3 | 场景-沙箱预览无公网URL（nginx_serving=FAIL + DevBridge 隧道未建） | sandbox DevBridge 隧道未建立 + nginx 未就绪 | 复现 |
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| VPC（D3-S2 hdk1-s2-*） | 是 | 已删（run_approved 确认后删除） | 全量 ListVpcs 不再含本次 VPC（归零） |
| VPC（D4-14 hdk1-audit-*） | 是 | 已删（DeleteVpc） | ListVpcs 不再含本次 VPC（归零） |
| OBS 桶（D3-C13 hdk1-s13-*） | 是 | 已删（rm -r -f 清对象） | 需补删桶归零（见下） |
...
> 注：D3-C13 探针删除使用 `OBS rm -r -f`（仅清对象、未删桶），本轮已手动 `OBS rm obs://<bucket> -f` 补齐删桶并复查 `hcloud OBS ls` 无 hdk1 残留。
...
- 本轮复查复现缺陷：D1-70（代理 no_proxy CIDR）、D3-S1/D3-S5（中文路由 miss，同 D10-3）、D3-S3（沙箱预览公网 URL，nginx_serving=FAIL 且 DevBridge 隧道未建）、D4-25（Python hook 遥测分类）、D4-26（findings 证据脱敏）、D1-68（HW_REGION 优先级 SPEC）、D8-9（sanitizeValue SPEC），均由 `file_issue.py` 查重后处理。
