...
> **落盘路径**：`results/Hermes/2026-09-17-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-17 07:05（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.5（npm latest，gitHead `e7ed6f6`，PR #696）
> **执行方式**：`reverify-2026-09-17.sh` 15 支源码级探针 fresh 重跑（safety-policy/protocol/hooks/tools/matrix/auth/approval/c4）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 建删归零+CTS）
> **提单说明**：本轮 11 项设计级缺陷 + 12 项展开级缺陷在 v1.1.5 实测**全部复现**，根因未变，经上游仓 `huaweicloud/huaweicloud-devkit` 历史 open issue 查重（GitHub API 实拉确认全部 open）**全部命中已跟踪缺陷单**（#683 / #679 / #689 / #671 / #651）——**本轮不重复提单**，关联清单见同目录 `HISTORY_LINKS.md`。**v1.1.5 已修复 D4-2、D9-2（实测 PASS），本轮不再列为缺陷。**
...
- **状态**：已提单 #683/#671/#651/#679（v1.1.5 复现，上游视为仓库级行为文档，非安装注入设计）

## #5【P1】D4-4 写操作审批门 Change* 漏拦截
...
## #6【P1】D4-8 Python/Node 安全钩子策略不一致
...
## #9【P1】D10-3 serviceCatalog 中文意图路由未命中
...
- **证据**：`evidence/D10-3/stdout.txt`、`evidence/fresh-matrix.txt`、`eval/results/eval-run-*.csv`
- **状态**：已提单 #651/#683/#679/#689（v1.1.5 复现，未修复）

## #10【P2】D2-11 auth_switch persist 的 R2 冲突门先于 R3 STS 检查
...
- **证据**：`evidence/EXP-E*/stdout.txt`、`evidence/fresh-matrix.txt`、`eval/results/eval-run-*.csv`
- **状态**：已提单 #651/#683/#679/#689（v1.1.5 复现，未修复）
