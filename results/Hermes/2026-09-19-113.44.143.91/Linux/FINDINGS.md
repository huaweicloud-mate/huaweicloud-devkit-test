...
> **落盘路径**：`results/Hermes/2026-09-19-113.44.143.91/Linux/FINDINGS.md`
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:399` —— env-dump 关键字正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/i` 缺 `HW_` 前缀；`env | grep HW_*` 管道打印形态无法命中（printenv 直接形态已覆盖 HW_）。
...
- **现象**：源码仓 `plugins/` `bin/` `integrations/` `scripts/` `src/` 全量 grep `agent-rules`/`huawei-agent-rules` 零命中（仅 `docs/audit-report-2026-08-20.md` 出现，非安装产出）。
...
## #8【P1】D4-27 redactSecrets/redactOutput 双路径漏小写 ak=/sk= + 裸 token 关键字

- **现象**：混合文本 `ak=AKID... sk=SKsecret... password=xxx adminPass=xxx token=RawToken...`，`redactSecrets` 与 `redactOutput` 两条路径均仅脱敏 `password=`/`adminPass=`，`ak=`/`sk=`/`token=` 明文残留（两路径均泄露）。
...
- **证据**：`evidence/D10-3/stdout.log` + `eval/results/eval-run-20260918210909.csv`
...
- **证据**：`evidence/D9-2/stdout.log` + `eval/results/protocol-probe-20260918210916.json`
...
## #13【SPEC-MISMATCH】D4-24 确认令牌过期/重复的精确 JSON 契约未实现

- **现象**：①过期提交期望 `{status:'rejected', code:'CONFIRM_TOKEN_EXPIRED'}`，实测 `runApprovedCommand` 抛通用 Error 串「Invalid or expired approval token. Please re-plan the command.」；②同 token 重复期望 `{status:'ok', outcome:'already_processed'}`，实测第二次消费返回 null，无 `already_processed` 字段。源码 `grep CONFIRM_TOKEN_EXPIRED/already_processed` 零命中。
