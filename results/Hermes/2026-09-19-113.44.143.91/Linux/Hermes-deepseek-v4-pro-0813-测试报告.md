...
> **生成时间**：2026-09-19 05:40（北京时间）
> **执行归档**：`results/Hermes/2026-09-19-113.44.143.91/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：`FAIL`（存在 5 P0 + 5 P1 产品缺陷 + 2 SPEC 契约漂移，均为历史已知缺陷复核）
...
| hcloud / 依赖 | hcloud 7.2.12（KooCLI） |
| 真云凭证 | cn-north-4，管理员 AKSK + 只读子账号 test001（均实测可用） |
| 测试类型 | 源码级探针 / 真机 CLI（doctor）/ MCP 协议 / 真云 E2E（建删归零） |
...
| 资源释放 | 全部归零（真云 VPC/Subnet/ECS/OBS/sandbox 建删归零验证，CTS 审计佐证） |
...
| 8 | P1 | D4-27 | redactSecrets/redactOutput 双路径漏小写 + 裸 token | 两路径均应 `<redacted>` | 两路径均明文残留 | safety-policy.mjs:34-45 + hcloud-cli.mjs:587-596 | 待历史查重 |
...
| 13 | SPEC | D4-24 | 确认令牌过期/重复精确 JSON 契约未实现 | `CONFIRM_TOKEN_EXPIRED`/`already_processed` | 抛通用 Error 串，TTL=5min | tools.mjs:1748-1750 + hcloud-cli.mjs:14 | 待历史查重 |
...
**#2 D2-4 / #8 D4-27 小写 ak/sk 漏脱敏**：`safety-policy.mjs:34-45` 密钥正则仅覆盖大写 `(AK|SK)`；`hcloud-cli.mjs:587-596` `redactOutput` 直接转发 `redactSecrets`，两路径同漏；另裸 `token=` 关键字也未脱敏。证据 `evidence/D2-4/stdout.log`、`evidence/D4-27/stdout.log`。
...
|---|---|---|---|---|---|---|
| D1-39 | 设计级 | P0 | NOT_RUN | 调归属 | Windows 专属用例（npm.cmd spawnSync 无 shell:true 抛 EINVAL 静默失败），本机为 Linux，无 .cmd/EINVAL 语义 | 无需改用例：已由展开级 EXP-NR3-10 在 Linux 侧代表覆盖；Windows 由 Windows 侧客户端覆盖 |
...
- [x] 凭证泄漏事件：`0`（真云 AK/SK 未出现在任何证据 stdout.log；D4-13 只读子账号证据仅含 API 响应，无 AK/SK 明文）
- [x] 写操作误判 read-only：`0`
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| VPC (testbot3-hermes-e2e-\*) | 是 | 已删 | DeleteVpc+deleteRouter 落 CTS，ShowVpc 返回 VPC.9904(not found) |
| Subnet | 是 | 已删 | DeleteSubnet 落 CTS |
| ECS (serverIds 6fb25054) | 是 | 已删 | ShowServer status=DELETED |
| OBS bucket | 是 | 已删 | 删桶归零（Delete bucket successfully） |
| sandbox session | 是 | closed | close_session ok |

> 真云只删本次创建资源（唯一时间戳命名 `testbot3-hermes-*`）；删除前盘点，未触碰既有/他人资源（历史遗留 `testbot3-hermes-e2e-9679*/9717*` 为其他会话创建，不属本次，未触碰）。D4-13 只读子账号 CreateVpc 未产生资源（VPC.0114 OverQuota 拒绝）。

---

## 八、遗留与建议

- 待裁决 SPEC：`D9-9`（capabilities.cancellation 未声明）、`D4-24`（确认令牌 ECTJSON 契约未实现）
- 本轮未覆盖范围：10 客户端终端矩阵（单机仅 Hermes）、真实 LLM Agent 会话行为评测（需 LLM harness，serviceCatalog 路由层已用 run-eval 代理）、macOS 专属
- 建议：`routeMap`（tools.mjs:1778-1886）补充中文意图关键字；`redactString` 补充小写 `ak/sk` 与裸 `token` 关键字；D9/D4-24 精确协议契约优先补齐