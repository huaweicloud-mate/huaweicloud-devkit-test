...
> **结论**：**PARTIAL**（设计级 18 FAIL + 2 SPEC-MISMATCH，展开级 14 FAIL；SUT 由 v1.1.6 升级至 v1.1.7-next.0 = 纯版本号 bump 零源码变更，全部缺陷实测复现、根因未变，统一走 `file_issue.py` 历史查重处理）
...
| 上轮版本对比 | v1.1.6（gitHead `46152dd`）→ v1.1.7-next.0（gitHead `0790e92a`）：`git diff` 确认仅 plugin.json/package.json/README/CHANGELOG 版本号变更、**零源码改动** |
...
> **执行方法**：复用 `run-today.sh` 22 支源码级探针 fresh 重跑（v1.1.7-next.0）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零 / D3-S3 沙箱预览）+ D2-26/D4-27 源码级直调探针 + 收尾补删 OBS 空桶归零。证据统一落 `evidence/<case-id>/stdout.txt`。
...
| P0 / P1 / P2 新增缺陷 | 0 / 0 / 0（v1.1.6→v1.1.7-next.0 纯版本 bump，全部为历史缺陷复现，见 HISTORY_LINKS） |
| 红线（I 类）违规 | 0 |
| 资源释放 | 真云 VPC/OBS/FunctionGraph/沙箱会话均已建删归零；只读账号写全被 IAM 拒绝 |

> **版本变更摘要**：`git diff 46152dd..0790e92a` 显示 v1.1.7-next.0 仅改 13 个文件（plugin.json/package.json/README/CHANGELOG 等，+13/-43），**无任何 `plugins/huaweicloud-core/src|hooks|safety` 源码变更**。故本轮 33 项缺陷与 v1.1.6 完全一致、逐条实测复现、根因未变。
...
> SUT 由 v1.1.6@46152dd 升级至 v1.1.7-next.0@0790e92a（纯版本号 bump，零源码改动），缺陷全部实测复现、根因未变，经上游 open issue 查重命中已跟踪缺陷单，本轮不重复提单（关联清单见 `HISTORY_LINKS.md`）。
...
- 历史缺陷（D2-4/D2-11/D4-4/D4-5/D4-8/D4-11/D4-16/D4-17/D4-23/D4-27/D10-3/EXP-E*）在 v1.1.7-next.0 全部复现、根因未变，经上游 open issue 查重命中，本轮不重复提单。
...
- 版本观察：v1.1.7-next.0 为 dev 预发布线上的纯版本号推进，未修复任何既有缺陷；建议待正式版修复后回归。
