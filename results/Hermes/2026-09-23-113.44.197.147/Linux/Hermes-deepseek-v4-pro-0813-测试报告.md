...
> **执行归档**：`results/Hermes/2026-09-23-113.44.197.147/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：**PARTIAL**（设计级 18 FAIL + 2 SPEC-MISMATCH，展开级 14 FAIL；SUT 由 v1.1.5 升级至 v1.1.6 后全部缺陷实测复现、根因未变，统一走 `file_issue.py` 历史查重处理）
...
| 被测版本（SUT） | v1.1.6（npm latest，gitHead `46152dd`，PR #795/#796） |
| 上轮版本对比 | v1.1.5（gitHead `e7ed6f6`）→ v1.1.6（gitHead `46152dd`），含 sandbox DevBridge 0.2.x 迁移、MCP 错误码修复（-32700/-32602）、doctor/uninstall 修复 |
...
> **执行方法**：复用 `run-today.sh` 22 支源码级探针 fresh 重跑（v1.1.6）+ `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零 / D3-S3 沙箱预览）+ D2-26/D4-27 源码级直调探针。证据统一落 `evidence/<case-id>/stdout.txt`。
...
| P0 / P1 / P2 新增缺陷 | 0 / 0 / 0（SUT 升级 v1.1.5→v1.1.6，全部为历史缺陷复现，见 HISTORY_LINKS） |
| 红线（I 类）违规 | 0 |
| 资源释放 | 真云 VPC/OBS/FunctionGraph/沙箱会话均已建删归零；只读账号写全被 IAM 拒绝 |

> **版本变更摘要**：v1.1.6 引入 sandbox DevBridge 0.2.x 迁移、MCP 协议 -32700/-32602/-32600 错误码修复、doctor 重启提示优化等。实测 D3-S3 沙箱预览的 nginx_serving 由 FAIL 转 PASS（归一改进），但公网 URL（DevBridge 隧道）仍不可达；其余 33 项缺陷在 v1.1.6 下全部复现、根因未变。
...
| 状态 | 数量 | 说明 |
|---|---|---|
| PASS | 78 | 有证据且通过 PASS 门禁（含 D4-13/D4-14 真云 PASS、D2-26 备份恢复闭环 PASS、D3-C13 OBS 静态网站真云 PASS、D3-S6 FunctionGraph 建删归零 PASS、D8-1/D8-4 文档复核 PASS、D5-1/D5-3 工具全量枚举 PASS） |
...
| 14 | P1 | D3-S3 | 场景-沙箱预览无公网URL（nginx 已就绪但 DevBridge 隧道未建） | sandbox DevBridge 隧道未建立 | 复现（v1.1.6 nginx 侧归一） |
...
> SUT 由 v1.1.5@e7ed6f6 升级至 v1.1.6@46152dd，缺陷全部实测复现、根因未变，经上游 open issue 查重命中已跟踪缺陷单，本轮不重复提单（关联清单见 `HISTORY_LINKS.md`）。
...
| OBS 桶（D3-C13 hdk1-s13-*） | 是 | 已删（-r -f 清对象） | 需补删桶归零（见下） |
...
> 注：D3-C13 探针删除使用 `OBS rm -r -f`（仅清对象、未删桶），本轮收尾需补 `OBS rm obs://<bucket> -f` 删桶并复查 `hcloud OBS ls` 无 hdk1 残留（收尾步骤单独执行）。
...
- 本轮复查复现缺陷：D1-70（代理 no_proxy CIDR）、D3-S1/D3-S5（中文路由 miss，同 D10-3）、D3-S3（沙箱预览公网 URL，nginx 已归一但 DevBridge 隧道未建）、D4-25（Python hook 遥测分类）、D4-26（findings 证据脱敏）、D1-68（HW_REGION 优先级 SPEC）、D8-9（sanitizeValue SPEC），均由 `file_issue.py` 查重后处理。
...
- 版本观察：v1.1.6 sandbox 变更使 D3-S3 的 nginx_serving 由 FAIL 归一为 PASS，但公网 URL（DevBridge 0.2.x 隧道）仍未打通，建议沙箱侧优先补齐隧道出 URL 路径。
- 测试侧改进：D3-C13 探针的 OBS 桶清理应改用 `OBS rm obs://<bucket> -f`（删桶）而非 `-r -f`（仅清对象），避免遗留空桶；本轮已在收尾手动补齐归零。
