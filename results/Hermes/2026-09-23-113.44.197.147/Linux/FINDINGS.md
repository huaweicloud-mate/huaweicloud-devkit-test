...
- **现象**：沙箱 `upload_project` 成功、`deploy_nginx` ok、`deploy_check` 返回 `nginx_serving=PASS`（v1.1.6 已归一，上轮为 FAIL），但 `devbridge_tunnel=FAIL`、`tunnel_url_accessible=FAIL (no tunnel)`、`publicUrl=undefined`。
