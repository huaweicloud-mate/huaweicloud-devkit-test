...
> **执行归档**：`results/Hermes/2026-09-23-1.94.218.129/Linux/`
> **被测对象**：huaweicloud-devkit v1.1.6（npm 官方 latest，gitHead `46152dd`）
> **结论**：`PARTIAL`（3 项 P0 历史缺陷 D4-16/D2-4/D4-23 + 11 项 P1/P2 缺陷；真云 E2E 真机完成且归零；上游已从 v1.1.5 升到 v1.1.6，15 项缺陷**全部仍在复现**，无新增缺陷、无缺陷被修复）
...
| 上一轮版本 | v1.1.5（gitHead `e7ed6f6`），本轮升版复测 |
| 工具全集 | 40（protocol-probe tools/list 实测 40） |
| hcloud / 依赖 | hcloud 已配置 / doctor 环境就绪 |
...
> **版本漂移修正**：`prepare_env.py --update` 默认「取 latest/next 最高」会误装 next 预发布 `1.1.7-next.0`。每日测试按 AGENTS.md 应测 latest 正式版，本轮已显式固定 SUT=v1.1.6（`npm install -g huaweicloud-devkit@1.1.6 --registry https://registry.npmjs.org` + `git checkout 46152dd`），与「latest 正式版」口径一致。
...
**版本对比结论**：本轮 SUT 从 v1.1.5（`e7ed6f6`）升到 v1.1.6（`46152dd`），上游合入 30+ commit（含沙箱 DevBridge 0.2.x 迁移、mcp -32602 修复 #704、mcp 容错 -32700、uninstall 崩溃修复等），但上述 15 项缺陷**逐一在 v1.1.6 复现**——提到的 #704 修复仅覆盖 `tools/call` unknown tool / 缺必填参数，尚未覆盖 `tools/list` 对非 object params 的校验（本缺陷 D9-2 仍 FAIL）。无新增缺陷、无缺陷被修复。
...
| D3-S6 | 设计级 | P2 | NOT_RUN | 补环境 | FunctionGraph 定时任务真云场景：需函数代码包 + IAM 执行委托 + 定时触发器三件套，daily 范围未预置；本轮已覆盖 serviceCatalog 路由层判定 EXP-E10 MISS + 工具链路 |
| D3-S7 | 设计级 | P1 | NOT_RUN | 补环境 | 跨服务交付(Web+RDS)真云多服务编排：RDS 创建计费 5-15 分钟 + 连接串注入 + 读写验证 + 归零，超 daily 单资源范围；本轮已覆盖 D3-C4 单资源真云建删归零 + D3-S2 删除确认 + D3-S3 沙箱预览 |
| D3-S4 | 设计级 | P1 | BLOCKED | 补环境 | 领券一人一次，本账号已领取（voucher_status=claimed=true），无法复跑 claim 闭环；status 步已实测。沙箱 user 状态：realnameVerified=true / agreementSigned=true |
...
- [x] 凭证泄漏事件：`0`（探针全程用假凭证；D2-26/D4-27 隔离 HOME 测试；真云凭证只经子进程注入，未打印到日志）
- [x] 写操作误判 read-only：`0`（D4-5 delete/create 分类正确；D4-13 只读子账号写被 PolicyNotAuthorized 拒绝）
- [x] 红线（I 类）违规：`无`
- [x] 脱敏复核：证据目录无原始管理员凭证/未脱敏日志。本轮复测发现晨间 d3-cloud 探针 `hwlink-getCredentials-struct` 曾把 `getCredentials()` 的真实 ak/sk 明文写入 stdout.log（`{ak:"HPUAN1...",sk:"Exz..."}`），本轮已修正探针脱敏（ak 仅留 `HPUA****`、sk 全遮 `****`）并重跑落盘干净证据；其余真云 result 输出已做 `[A-Z0-9]{20,}` 遮罩。
...
- 本轮 SUT 升版到 v1.1.6，15 项缺陷全部复现（无修复回归、无新增退化）；#704 仅部分修复了 tools/call 的错误码，D9-2（tools/list 参数类型校验）仍缺。
...
- 环境建议：`prepare_env.py --update` 默认「自动取 next 最高」会误装预发布包，每日测试应显式走 `--latest`（或脚本默认改为 latest，除非显式 `--next`）；本机 npm 私有镜像 `127.0.0.1:45998` latest 与官方 registry 漂移问题仍在。