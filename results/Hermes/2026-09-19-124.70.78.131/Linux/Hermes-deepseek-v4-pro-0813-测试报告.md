...
> **生成时间**：`2026-09-19 05:12`（北京时间）
> **执行归档**：`results/Hermes/2026-09-19-124.70.78.131/Linux/`
...
> **本轮要求**：强制完整重跑（不得以「今天已跑过」为由跳过）。相对 2026-09-18：SUT 未变（v1.1.5，gitHead e7ed6f66），用例集未变（设计级 80 + 展开级 48），全部探针/真云/harness 重新执行并落盘新鲜证据；关键结论与 09-18 一致——8 项产品缺陷全部复现且全部历史查重命中，D4-13 只读子账号 6/6 可用（探针缺陷已修，非产品缺陷）。
...
> - 残留缺口：`env | grep HW_ACCESS_KEY` 仍 allow；`sh -c "env | grep …"` 仍 allow；未知 tool 仍 `-32603`；HCL broad IAM 仍 allow；fail-open 仍 allow；中文路由仍 21.4%；redactString 字符串路径缺裸 `token` + `admin_pass` 变体（D4-27）。
...
**#8 [P1] D4-27**：直调 `redactSecrets`/`redactOutput` 5 项断言 2 项 PASS、3 项 FAIL——对象路径 `{token:...}` 与 `redactOutput(JSON)` 正确脱敏（`isSecretKeyName` 经 `policy.json` `secretKeyNamePatterns` 含 `token`/`ak`/`sk`）；但**字符串路径** `redactString`（`safety-policy.mjs:42`）key=value 白名单缺裸 `token`（仅 `security[_-]?token`/`x[_-]?auth[_-]?token`），且 `adminPass` 仅字面 camelCase 未覆盖 `admin_pass`/`admin-pass` 变体，故 `token=T0K3N...`、`--admin-pass=...` 与 `redactOutput` 非 JSON 回退均泄漏。证据 `evidence/D4-27/stdout.log`。

**D4-13（较 09-17 修正后的稳定结论，非缺陷）**：用 test001 只读子账号实测 6 条只读命令**全部通过**（ECS NovaListServers / VPC ListVpcs / EVS ListVolumes / IMS ListImages / CES ListMetrics / EIP ListPublicips）；写操作 VPC CreateVpc 被 IAM 拒绝（PolicyNotAuthorized / VPC.0010，符合预期），归零验证通过。09-17 报告「5/6（IMS 被拒）」源于探针缺陷（execSync 默认 1MB `maxBuffer` + `403` 正则误匹配），探针修正后 IMS 只读长期可用。证据 `evidence/D4-13/stdout.log`。
...
| 资源 | 创建 | 销毁 | 归零验证 |
|---|---|---|---|
| 真云 VPC `tctest-hermes-20260919-*`（D4-14） | 是 | 已删 | ListVpcs 不含本次资源 = 通过 |
| 真云安全组 `tctest-hermes-sg-pf-*`（D4-19） | 是 | 已删 | ListSecurityGroups 不含本次 SG = 通过 |
| 隔离 HERMES_HOME `iso-home-20260919`（install/uninstall D1-1/3/4/5） | 是 | 已删 | 退出即清理 |
...
- 观察（非缺陷）：
  ...
  2. `serviceCatalog` routeMap 仅少量条目含 CJK 关键字，中文意图大面积 miss（#6，已跟踪）。
  3. v1.1.5 修复为**部分到位**：env-dump 裸命令/shell 包裹破坏性命令/`printenv HW_*`/未知 method `-32601` 已修；`env | grep HW_*`、shell 包裹 env-dump、未知 tool `-32602`、HCL broad IAM、fail-open、中文路由仍未修；redactString 字符串路径仍缺裸 `token` 与 `admin_pass` 变体（#8，D4-27）。
  4. D4-13 只读子账号本日复测 **6/6 全部可用**（探针 maxBuffer/正则缺陷已修，IMS 只读长期可用，非产品缺陷）。
