...
- **现象**：`printenv HW_ACCESS_KEY` / `env | grep HW_SECRET_KEY` 实测 `classifyTextCommand` 返回 `allow`（应 `deny`）；而 `env | grep HUAWEICLOUD_ACCESS_KEY` 同命令可正确 `deny`（对比验证）。
...
- **状态**：已提单 huaweicloud/huaweicloud-devkit#650 / #651 / #652（去重；今日 latest v1.1.4 复测仍复现）
...
- **证据**：`evidence/d4-security/stdout.log`
- **状态**：已提单 huaweicloud/huaweicloud-devkit#650 / #652（去重；今日复测仍复现）
...
- **断言**：broad IAM 制品应被拦截（deny / findings 非空）
...
- **状态**：已提单 huaweicloud/huaweicloud-devkit#651 / #652（去重；今日复测仍复现）

## #4【P0】全局规则 huawei-agent-rules 注入生效性 — kms Decrypt 直连未识别为 secret

- **现象**：`classifyTextCommand('hcloud kms Decrypt --ciphertext-blob x')` 返回 `{decision:"allow", ...}`（应 `deny`/`secret`）；同源 `hcloud csms ShowSecretVersion` 可正被 `deny`（对比验证）。
- **断言**：`hcloud kms Decrypt` 应被识别为 secret 并 `deny`（decision=deny 或 risk 含 secret）
- **根因**：`plugins/huaweicloud-core/safety/policy.json:26` + `blockedSecretOperations` 仅含 `["ShowSecretVersion","DownloadSecret","GetSecretValue"]`，未含 `Decrypt`；`plugins/huaweicloud-core/src/safety-policy.mjs:177` 的 secret 正则 `/secret…|showsecretversion|getsecretvalue/i` 亦未含 `decrypt`，故 `kms Decrypt` 落入 `safety-policy.mjs:349` 的 secret 拦截分支之外、被当作普通只读命令放行
- **影响**：KMS 解密明文回显可绕过 secret 拦截进上下文，违背全局规则「禁直连 kms decrypt」契约
- **证据**：`evidence/d4-rules/stdout.log`
- **状态**：已跟踪 huaweicloud/huaweicloud-devkit#650 / #651（去重；今日复测仍复现）
...
## #6【测试侧】D5-3 工具全量枚举断言过时 — 39 → 40

- **现象**：`TOOL_DEFINITIONS.length` 实测 `40`（新增 `huaweicloud_obs_set_website_config`），设计级用例 `D5-3` 预期值硬编码为 `39`，导致「工具全集数量 = 39」断言 FAIL；其余断言（含 hook_check_command / auth_status / list_operations、schema 完整）均 PASS。
- **断言**：`TOOL_DEFINITIONS.length` 应等于 `tools.mjs` 实际注册数量（当前 40），或用例改用动态计数
- **根因**：`plugins/huaweicloud-core/src/tools.mjs` 新增第 40 个工具 `huaweicloud_obs_set_website_config`，属正常功能演进；`test-cases/daily` 母版 `D5-3` 的预期结果仍写死 `39`（改用例）
- **影响**：非产品缺陷，仅用例预期值滞后；需维护 agent 修订母版 `D5-3` 预期（39 → 40 或动态断言）
- **证据**：`evidence/d5-tools/stdout.log`、`evidence/d9-protocol/stdout-d9-8.log`
- **状态**：不予提单（测试侧，改用例）