# -*- coding: utf-8 -*-
"""AtomCode Linux 每日测试回填：执行状态 + 执行时间 + evidencePath + blockedReason。
基于 2026-09-18 全量复跑探针的实际结果回填（v1.1.5 gitHead e7ed6f6）。"""
...
DESIGN = {
    ...
    "D4-19": ("PASS", "evidence/d4-confirm/stdout-probe.log", ""),
    ...
    "D2-26": ("PASS", "evidence/d2-auth/stdout-probe-d2-26.log", ""),
    "D4-27": ("PASS", "evidence/d4-security/stdout-probe-d4-27.log", ""),
}
...
def backfill(path, resolver, trace=False):
...
n3 = backfill(os.path.join(BASE, "需求-设计-证据追踪表.csv"), None, trace=True)
print(f"回填完成：设计级 {n1} 行、展开级 {n2} 行、追踪表 {n3} 行，时间戳 {TS}")