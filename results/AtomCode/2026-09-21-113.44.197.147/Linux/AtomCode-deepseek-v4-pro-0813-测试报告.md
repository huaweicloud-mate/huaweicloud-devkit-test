...
> **执行归档**：`results/AtomCode/2026-09-21-113.44.197.147/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：`FAIL`（存在 4 个 P0 缺陷，与 v1.1.5 上一测次一致）
...
| 测试类型 | 源码级探针 / 真机 CLI（help/doctor/status）/ MCP 协议 / 真云 E2E |
| daily 基础用例 | 设计级 100 / 展开级 39（预筛后）/ 追踪表 209 行 |
...
> P0 缺陷集中在**安全拦截/制品预检/规则注入**三处（safety-policy、risk-rule-engine、package setup），为高危漏洞面；D10-3 路由准确率 21.4% 与设计断言 ≥90% 差距巨大，为核心功能缺陷。本测次 SUT 与上一测次（2026-09-20，同 gitHead `e7ed6f6`）完全一致，15 项缺陷均稳定复现。
...
- Python hook 遥测事件文件 `plugins/telemetry/hook-events.jsonl` 测试后删除。
