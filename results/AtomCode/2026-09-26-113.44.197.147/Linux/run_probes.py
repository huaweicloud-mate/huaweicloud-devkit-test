#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AtomCode 2026-09-26 Linux 每日测试探针重跑编排器。

复用昨日(2026-09-25)已构建的确定性探针脚本(.mjs)，仅改写输出日期路径
(2026-09-25-113.44.197.147 -> 2026-09-26-113.44.197.147)，SUT 源码 import 路径
...
YDAY = BASE.replace("2026-09-26", "2026-09-25")
...
def main():
    ...
    for mjs in sorted(glob.glob(os.path.join(YEV, "**", "*.mjs"), recursive=True)):
        ...
        text = text.replace("2026-09-25-113.44.197.147", "2026-09-26-113.44.197.147")
    ...
    for g in sorted(groups):
        for mjs in sorted(groups[g]):
            ...
            print(f"[run] node {os.path.relpath(mjs, EV)}", flush=True)
            with open(run_log, "w", encoding="utf-8") as f:
                f.write(f"$ node {os.path.relpath(mjs, EV)}\n")
                f.write(f"$ executed at (BJ) 2026-09-26\n\n")
                try:
