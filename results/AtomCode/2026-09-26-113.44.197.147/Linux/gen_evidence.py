#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AtomCode 2026-09-26 Linux 每日测试：按 evidence/<case-id>/stdout.log 约定落盘结果。
...
DESIGN = {
    ...
    "D9-12": ("PASS", "initialize 握手协议安全基线通过（protocolVersion/capabilities/serverInfo.name/version 合规，tools/call 返回 content 数组；未 initialize 先 tools/list 的生命周期缺口记入 D9-4）"),
    "D9-13": ("PASS", "tools/call 凭证不泄露与权限校验通过（分类写命令 deny/confirm、返回无明文 AK/SK、approval token 一次性消费、runtime 凭证解析/清除生效）"),
