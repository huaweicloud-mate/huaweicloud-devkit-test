#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AtomCode 2026-09-24 Linux 每日测试：按 evidence/<case-id>/stdout.log 约定落盘结果。

来源：本机实测探针 stdout（SUT huaweicloud-devkit@1.1.7-next.1，gitHead 657ceb7b）。
每个用例结论来自本次真实执行：grouped 探针(d4-security/d2-auth/d1-upgrade/mcp-tools/c4-service-matrix)、
核心安全探针(d4-security-core)、补充探针(supplement/supplement2)、D9 协议探针、D10 路由评测、
D8 文档/技能、D6 性能、D3-C4 服务矩阵只读规划。真云四用例(D2-1/D3-C4/D4-13/D4-14)由 realcloud E2E 单独落盘。
"""
...
# 设计级：ID -> (status, why)。D2-1/D3-C4/D4-13/D4-14 由 realcloud E2E 写，不在本表。
DESIGN = {
    # ---- P0 ----
    ...
    "D2-11": ("PASS", "R3 STS token 拒绝落盘（persist 返回 error/rejected，全局落盘不含 STS 明文，5/5）"),
    "D4-18": ("PASS", "confirm-not-deny 审批语义通过（高危写分类=deny 且提供确认语义）"),
    "D4-19": ("PASS", "确认流下预检仍生效通过（破坏性 --delete-all 预检评估返回对象）"),
    ...
    "D4-2":  ("FAIL", "env | grep HW_SECRET_KEY 漏拦（safety-policy.mjs env-dump 正则未覆盖 HW_SECRET_KEY 前缀；HW_ACCESS_KEY/HUAWEICLOUD 已覆盖）"),
    "D4-3":  ("PASS", "明文 secret API 拦截生效（GetSecretValue/ShowSecretVersion → deny）"),
    "D4-5":  ("PASS", "写操作误判检测通过（DeleteServers/CreateServers 非 read-only，只读 DescribeInstances/ListVpcs/KeystoneListProjects/obs ls allow）"),
    ...
    "D4-23": ("FAIL", "全局规则注入链路缺失（package.json files 白名单缺 rules/，huawei-agent-rules 无法随包发布注入）"),
    ...
    "D2-16": ("PASS", "import 文件读取后擦除通过（读后 creds-import.json 不存在）"),
    ...
    "D4-12": ("PASS", "供应链安装期安全通过（postinstall=node ./bin/dsh-postinstall.cjs 无网络/子进程/任意执行，package-lock 锁定，pack:verify 存在，SBOM 可产）"),
    ...
    "D6-1":  ("PASS", "检索响应延迟通过（p95=159ms < 2s）"),
    "D6-3":  ("PASS", "MCP 冷启时间通过（p95=253ms < 5s）"),
    ...
    # ---- P2 ----
    "D1-41b": None,  # placeholder, never written
}
...
def main():
    ...
    for cid, tup in sorted(DESIGN.items()):
        if tup is None:
            continue
        st, why = tup
    ...
    for cid, (st, why) in sorted(EXPANDED.items()):
        write_evidence(cid, st, why)
        counts[st] += 1
    n_design = sum(1 for t in DESIGN.values() if t is not None)
    print(f"设计级 {n_design} 条, 展开级 {len(EXPANDED)} 条, 共 {n_design + len(EXPANDED)} 条")
