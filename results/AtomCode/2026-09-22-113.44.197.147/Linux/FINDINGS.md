...
> **落盘路径**：`results/AtomCode/2026-09-22-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-22 05:07（北京时间）
> **被测对象**：huaweicloud-devkit **v1.1.6-next.0**（gitHead `faaefb8f`）
...
- **根因**：`src/safety-policy.mjs:399`（`classifyTextCommand`）— 凭证 env 检测正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/i` 未覆盖 `HW_` 前缀；`:418-419` 的补充规则仅匹配 `$(...)`/`printenv` 直接形态，`env | grep HW_SECRET_KEY` 这类经 grep 管道的形式仍落入 allow。
