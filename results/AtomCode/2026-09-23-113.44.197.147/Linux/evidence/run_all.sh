#!/usr/bin/env bash
# 2026-09-18 OpenClaw Linux daily test — fresh probe execution
# Each probe prints assertions to stdout; we redirect to <name>.stdout.log and capture exit code.
set -u
...
run_probe() {
  ...
  echo "" > "$log"
  local out="$log"  # redirect subsequent node stdout/stderr to log
  local code=0
...
run_probe d10-routing/run-eval "$REPO/eval/harness/run-eval.mjs" "$SERVER"
