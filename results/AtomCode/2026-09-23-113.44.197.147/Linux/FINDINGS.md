...
> **被测对象**：huaweicloud-devkit **v1.1.6**（npm latest 正式版，gitHead `46152dd`）
...
- **根因**：`src/safety-policy.mjs:399`（`classifyTextCommand`）— 凭证 env 检测正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/i` 未覆盖 `HW_` 前缀；补充规则仅匹配 `$(...)`/`printenv` 直接形态，`env | grep HW_SECRET_KEY` 经 grep 管道形式仍落入 allow。
...
- **根因**：`src/safety-policy.mjs:384`（`classifyTextCommand`）— 文本命令分类未对 `sh -c "..."` 的引号内片段做与 `stripExecutable` 等价的解包裹识别，仅对部分包装形态生效。
...
- **根因**：`src/risk-rule-engine.mjs:150`（`evaluateArtifacts` → `evaluate('artifact', …)`，规则库 `:38` stage==='artifact' 分支）— 规则库无 broad-IAM（通配 action / `Resource:"*"`）匹配规则，未覆盖该风险类别。
...
- **根因**：`src/safety-policy.mjs:42`（`redactString`）— 密码明文脱敏仅覆盖等号/键值对象路径，空格分隔的单独参数值未命中脱敏分支。
...
- **根因**：`src/risk-rule-engine.mjs:150`（`evaluateArtifacts`）— 同 D4-21，缺少 broad-IAM 规则。
...
- **根因**：`src/safety-policy.mjs:34-42`（`redactString`）— 文本脱敏正则未覆盖裸 `token=` 与小写 `ak=`/`sk=` 形态。
...
- **根因**：`src/mcp-protocol.mjs:30`（`dispatch` 入参校验缺失）+ `src/mcp-server.mjs:162` — 未对 `params` 做类型/必填字段校验，未区分 `-32602`（invalid params）与 `-32603`（application error）。
...
- **根因**：`src/mcp-protocol.mjs:30`（`dispatch`）— 未维护/校验初始化状态，所有方法无时序守卫。
- **影响**：协议时序约束未遵守，状态机不严谨。
...
- **根因**：`src/tools.mjs:1784`（`serviceCatalog` 关键词路由层，`routeMap` `:1786`）— 中文意图（ECS/OBS/EIP/RDS/CBR/FunctionGraph/BSS/CES/ELB/IAM 等）关键词/语义覆盖严重不足，仅少量命中。
...
- **根因**：`hooks/huaweicloud-safety.py:46`（`WRITE_OPERATION_RE` 正则前缀字符组 `(^|[A-Za-z0-9])`）— 要求写动词前不能是空格，但 `"ECS CreateServers"` 中 `Create` 被空格前置，正则不命中，`is_write`（`:95`）为 False，落入 `cli:invoke`（`:96`）。
...
- **根因**：`src/mcp-protocol.mjs:46`（initialize 处理）— 无版本协商/校验逻辑，直接透传请求版本。
...
- **根因**：`src/mcp-protocol.mjs:47`（initialize 的 capabilities 组装）— 未声明取消能力；`dispatch` 无超时语义。
