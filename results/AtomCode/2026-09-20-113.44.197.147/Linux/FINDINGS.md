...
> **落盘路径**：`results/AtomCode/2026-09-20-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-20 北京
...
## #4【P0】D4-23 全局规则 huawei-agent-rules.md 注入链路缺失
...
- **现象**：`record_cli_event('hcloud ECS CreateServers …')` 事件键为 `cli:invoke`（预期 `cli:write`）；只读 `ListServersDetails`→`cli:read`、非 hcloud→`cli:invoke` 均正确。
- **断言**：写命令事件键应为 `cli:write`。
- **根因**：`hooks/huaweicloud-safety.py:96`（`record_cli_event` 的 `is_write` 判定）— 写操作判定未命中 `CreateServers` 等写前缀，落入 else 分支 `cli:invoke`。
