...
> **落盘路径**：`results/AtomCode/2026-09-16-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：`2026-09-16 07:20:00`（北京时间）
> **被测版本**：`v1.1.5`（npm latest，gitHead `e7ed6f66`）
...
- **状态**：历史问题 #650（v1.1.5 已修 printenv 形态，`env|grep` 形态仍复现，部分修复）
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:398`（env-dump 用 `(^|\s)(env|printenv|...)` 单词边界匹配原始文本），`sh -c "env ..."` 内层 `env` 前为引号未命中；v1.1.5 的 `stripExecutable` 解包只作用于 hcloud 命令分支（classifyHcloudArgs），未覆盖 env-dump 分支
- **影响**：命令包裹绕过检测，凭证 dump 可穿透 hook
- **证据**：`evidence/d4-security/stdout.log`
- **状态**：历史问题 #650（v1.1.5 部分修复；env-dump 包裹形态仍复现）
...
- **根因**：`plugins/huaweicloud-core/safety/policy.json:26`（`blockedSecretOperations` 仅含 `["ShowSecretVersion","DownloadSecret","GetSecretValue"]`，未含 `Decrypt`）+ `plugins/huaweicloud-core/src/safety-policy.mjs:432`（secret 正则 `/ShowSecretVersion|GetSecretValue|secret_string|secret_binary/i` 未含 `decrypt`）
- **影响**：KMS 解密明文回显可绕过 secret 拦截进上下文，违背全局规则「禁直连 kms decrypt」契约
- **证据**：`evidence/d4-rules/stdout-probe.log`
- **状态**：历史问题 #650/#651（v1.1.5 复测仍复现）

## #5【P1】D9-2 JSON-RPC 错误码补齐后仍缺 -32602 — invalid params 无 error 对象

- **现象**：v1.1.5 已修复 -32601（未知 method 现返回 -32601 正确）；但向 `tools/list` 传非法 params（`params:'not-an-object'`，应为 object）时，服务端**无 error 对象返回**（应返回 `-32602 Invalid params`）。
...
- **现象**：`eval/harness/run-eval.mjs` 对 15 条中文意图逐条调 `huaweicloud_service_catalog`，实测 HIT=3 MISS=11 N/A=1，路由准确率 21.4%（分母 HIT+MISS=14），远低于评测级目标 ≥90%。11 条中文意图（创建云主机/部署静态站/绑定EIP/查RDS/备份/函数/费用/云监控/HTTPS证书/IAM审计/…）均返回通用 `Run hcloud --help to list available services.`。
...
- **断言**：support cancellation 的服务端应在 capabilities 声明 `notifications.cancellation`（MCP 取消能力契约）
...
- **状态**：待裁决（SPEC-MISMATCH；超时 -32000 / 取消 2s 中止子断言仍需可注入延迟夹具）

## #8【测试侧】D3-C4 hcloud CLI 服务覆盖差异 — OBS/DMS/DEW 不可 `--help` 执行（19/22）

- **现象**：真云最小权限账号实测 22 服务 `hcloud <svc> --help`，OBS（`No such command: --help`，需 `obs help`）、DMS/DEW（`Unsupported service`）3 项不可执行；devkit `list_operations` 源层级 22/22 路由正常（EXP-C4 证据）。
