...
> **生成时间**：`2026-09-17 07:06:00`（北京时间）
> **执行归档**：`results/AtomCode/2026-09-17-113.44.197.147/Linux/`
> **被测对象**：huaweicloud-devkit（GitHub `huaweicloud/huaweicloud-devkit`）
> **结论**：`FAIL`（v1.1.5 上 4 项 P0 安全缺陷仍复现 + 1 项 P1 JSON-RPC 错误码漏 + 1 项 P1 中文意图路由 + 1 项 SPEC-MISMATCH）
...
| 已执行 | `117` |
| PASS / FAIL / BLOCKED / SPEC-MISMATCH / NOT_RUN | `87 / 19 / 9 / 1 / 1` |
| 通过率（分母 = PASS+FAIL+SPEC-MISMATCH） | `81.3%`（87/107） |
| P0 / P1 / P2 新增缺陷 | `0 / 0 / 0`（本轮缺陷均为 v1.1.5 上已复现/已记录项） |
...
|---|---|---|---|---|---|---|---|---|
| 1 | P0 | `D4-2` | 凭证 env 打印拦截不完整（`env\|grep HW_*` 形态） | `env \| grep HW_SECRET_KEY` → `deny` | `allow` | `safety-policy.mjs:398-399` | P | 历史 #650 仍复现 |
| 2 | P0 | `D4-16` | 命令包裹穿透（sh -c 包 env dump） | `sh -c "env \| grep HUAWEICLOUD_ACCESS_KEY"` → `deny` | `allow` | `safety-policy.mjs:398` | P | 历史 #650 仍复现 |
| 3 | P0 | `D4-21` | IaC 制品 broad IAM（HCL `actions=["*"]`） | HCL `actions=["*"]` 应拦截 | findings 空 | `cloud-risk-rules.json:192` | P | 历史 #651 仍复现 |
| 4 | P0 | `D4-23` | kms Decrypt 直连未识别为 secret | `hcloud kms Decrypt` → `deny`/secret | `allow`/unknown_read | `policy.json` blockedSecretOperations + `safety-policy.mjs:432` | P | 历史 #650/#651 仍复现 |
| 5 | P1 | `D9-2` | invalid params 未返回 -32602 | 非法 params → `-32602` | 无 error 对象 | `mcp-protocol.mjs:73` | G | 待提单（-32601 已修，-32602 残留） |
| 6 | P1 | `D10-3` | 中文意图路由准确率 21.4% | 路由准确率 ≥90% | 21.4%（3/14 HIT） | `tools.mjs:1776` | G | 待提单 |
| 7 | 测试侧 | `D3-C4` | hcloud CLI OBS/DMS/DEW 不可执行 | 22 服务可执行 | 19/22 | hcloud CLI 7.2.12 | E | 非产品缺陷 |
...
- 建议：优先修复 4 项 P0 安全残留（D4-2 env|grep 形态、D4-16 env-dump 包裹、D4-21 HCL broad IAM、D4-23 kms Decrypt），并补齐 D9-2 的 -32602 与 D10-3 中文意图路由。