...
> **落盘路径**：`results/AtomCode/2026-09-17-113.44.197.147/Linux/FINDINGS.md`
...
- **状态**：历史问题 #650（v1.1.5 已修 printenv 形态，`env|grep` 形态仍复现）
...
- **根因**：`plugins/huaweicloud-core/safety/rules/cloud-risk-rules.json`（规则 `hwc-iam-admin-policy`）的 IAM 正则只覆盖 JSON 形态，未覆盖 Terraform HCL 小写 `actions = ["*"]` 块语法
...
- **根因**：`plugins/huaweicloud-core/safety/policy.json`（`blockedSecretOperations` 未含 `Decrypt`）+ `plugins/huaweicloud-core/src/safety-policy.mjs`（secret 正则未含 `decrypt`）
...
- **根因**：`plugins/huaweicloud-core/src/tools.mjs`（`serviceCatalog(intent)` 意图→服务关键词匹配规则不足，中文意图几乎全部 MISS 落到通用 help）
