...
> **落盘路径**：`results/AtomCode/2026-09-25-113.44.197.147/Linux/FINDINGS.md`
> **生成时间**：2026-09-25 05:20:00（北京时间）
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:398-399` env-dump 正则 `/HUAWEICLOUD|HWC_|HCLOUD|OS_/` 未覆盖 `HW_` 裸字面；`:418-419` 新增 `$HW_*`/`printenv HW_*` 检查仅覆盖 `$` 前缀/printenv 形态，`env | grep HW_SECRET_KEY` 无 `$` 前缀仍穿透。
...
- **根因**：`plugins/huaweicloud-core/src/safety-policy.mjs:398-399` env-dump gate 正则 `(^|\s)(env|printenv|…)` 要求 `env`/`printenv` 前是空白/行首，`sh -c "env …"` 的引号包裹使关键词前是 `"` 无法匹配；`:66-80` `stripExecutable` 只对 hcloud 参数分类生效，不作用于通用 env/凭证门。
- **影响**：sh -c 包裹绕过凭证打印拦截（安全风险，I 类）。
- **证据**：`evidence/d4-security-core/probe-p0-security.run.log`（`D4-16 wrap-sh FAIL`）、`evidence/D4-16/stdout.log`
...
- **证据**：`evidence/d4-security-core/probe-p0-security.run.log`（`D4-21 broad IAM FAIL`）、`evidence/D4-21/stdout.log`
...
## #6【P0】D9-12 initialize 握手时序守卫缺失（未 initialize 先 tools/list 未返回 -32600）

- **现象**：未 `initialize` 先 `tools/list` 仍正常返回工具列表（`code=null`），未按设计契约返回 -32600；initialize/协商/`_decorateResult`/`listSkillDirs` 基线均正常。
- **断言**：非法时序（未 initialize 先 tools/list）应返回 JSON-RPC `-32600` 错误。
- **根因**：`plugins/huaweicloud-core/src/mcp-protocol.mjs:57`（`dispatch` tools/list 分支）无 initialize 时序守卫/会话状态检查（与 D9-4 协议生命周期同根因）。
- **影响**：握手协议时序安全基线不满足，客户端误用不报错。
- **证据**：`evidence/d9-12-13-p0/probe-d9-12-13-p0.run.log`（`D9-12 未 initialize 先 tools/list => code:null`）、`evidence/D9-12/stdout.log`

## #7【P1】D10-3 中文意图路由准确率仅 21.4%
...
- **根因**：`plugins/huaweicloud-core/src/tools.mjs:1817`（`serviceCatalog` routeMap）缺排障/诊断分支，`:1123`（`explain_error` dispatch）未被 serviceCatalog 入口路由。
- **影响**：故障诊断意图无法自动路由到排障工具。
- **证据**：`evidence/supplement/probe-supplement.run.log`（`D3-S8 FAIL`）、`evidence/D3-S8/stdout.log`

## #11【P1】D9-2 JSON-RPC 非法入参未返回 -32602

- **现象**：`tools/list` 传非法 `params` 返回正常 `result`（未返回 `-32602`）；`tools/call` 缺参/未知工具返回 -32602 正常。
- **断言**：非法 `params`（非对象）应返回 `error.code===-32602`。
- **根因**：`plugins/huaweicloud-core/src/mcp-protocol.mjs:57`（`tools/list` 分支无入参类型校验，`-32602` 只在 `tools/call` 内触发）。
...
## #13【P1】D9-9 capabilities 未声明 cancellation（SPEC-MISMATCH）
...
## #14【P2】D4-25 Python hook 写命令未分类 cli:write
...
## #15【P2】D8-9 遥测 sanitizeValue 未脱敏
