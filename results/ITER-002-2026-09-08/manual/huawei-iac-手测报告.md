...
## 二·B、阻塞用例（BLOCKED ×15）原因说明

**阻塞范围**：用例 5B / 5C / 6B / 6C（4 小节，15 个检查点）

**唯一根因：需「人工领券 + 保证金」前置**（账号资金门槛，非插件缺陷）：

| 阻塞项 | 前置条件 | 为什么执行不了 |
|---|---|---|
| 5B/5C（部署成本/账单验证） | 实缴保证金 + 代金券到账 | 测试账号现金仅 **1 元**，被 huawei-iac 成本关卡拦下（技能设计"余额不足不能续跑"） |
| 6B/6C（计费/账单调账单核对） | 真实下单计费验证 | 余额 1 元无法真实触发计费（EIP 计费码用官网估算替代），避免欠费风险 |
| 全部 4 小节 | 代金券走完人工领取流程 | 需人工在华为云控制台完成领券→到账→验证 |

**复核结论**：0 FAIL / 37 PASS / 2 PARTIAL / 15 BLOCKED——阻塞项为账号资金与人工前置所致，**非功能缺陷**；huawei-iac 的成本关卡（保证金第一句/领券优先/免费额度分层）本身工作正常。阻塞项排期：代金券/保证金就绪后可补（ITER-003 已列）。

> ⚠️ **metrics 口径差异标注**：metrics/execution.csv 记为 `17(需人工前置)`，与明细过滤后的 15 BLOCKED 差 2——差异来源 = 2 个 PARTIAL（2.1 批量审批呈现 / 3.3 TIMER 触发器）也计入"非完全通过"；**15 = 纯 BLOCKED，17 = BLOCKED + PARTIAL 合并口径**。
