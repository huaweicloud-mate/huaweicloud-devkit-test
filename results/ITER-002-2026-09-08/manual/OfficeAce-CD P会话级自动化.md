...
| 项 | 结果 | 证据 |
|---|---|---|
| G1 工具可见 | ✅ 已修复（连接器 connected，37 工具注入） | UI 已连接 / sqlite connected / 会话内工具可用 |
| G2 只读调用 | ✅ 真实 MCP 调用（ListServersDetails count 0 + OBS ls Bucket 0） | agent 直接调 MCP 工具（5 工具链） |
| G3 审批框 | ❌ **无审批框**（写操作直接执行） | OBS-11 实证 |
| G5 多轮 | ✅ **连接修复后完整验证**（00:21：复述上轮 ECS 结果+OBS 新查询+汇总表，上下文延续） | "结论：当前账号在该区域/项目下无 ECS 实例"→OBS ls Bucket number: 0 |
