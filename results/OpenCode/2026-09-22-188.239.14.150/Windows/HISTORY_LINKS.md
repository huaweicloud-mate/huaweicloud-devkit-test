...
## D1-39 Windows 升级检测链 EINVAL 静默失败
- 今日证据：`evidence/D1-39/stdout.log`（check_update 返回 check_failed + queryDistTags 返回 null）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D4-2 凭证 env 打印拦截缺失
- 今日证据：`evidence/D4-2/stdout.log`（hook_check_command 返回 allow，0 findings）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D4-3 明文 secret API 拦截缺失
- 今日证据：`evidence/D4-3/stdout.log`（hook_check_command 返回 allow，0 findings）
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#770](https://github.com/huaweicloud/huaweicloud-devkit/issues/770)（open）**[daily-test] CodeArtsWork Windows 2026-09-21: 4 defects (3 P0 + 1 P1)**
    - 历史单内容：## 每日测试缺陷汇总 — CodeArtsWork Windows 2026-09-21 **客户端**: CodeArtsWork (GLM-5.2) **OS**: Windows **包版本**: huaweicloud-devkit@1.1.5 **测试日期**: 2026-09-21 **总用例**: 139 (PASS=121, FAIL=16, BLOCKED=2) --- # F
  - [#761](https://github.com/huaweicloud/huaweicloud-devkit/issues/761)（open）**[测试报告] huaweicloud-devkit v1.1.5 每日测试缺陷合并单（OfficeAce Windows 2026-09-20，8+11 项）**
...
## D4-28 Node 安全 hook 链路凭证泄露检测缺失
- 今日证据：`evidence/D4-28/stdout.log`（hook_check_command 返回 allow，0 findings）
- **关联历史单（已作为缺陷提过，本次为复核）**：
  - [#770](https://github.com/huaweicloud/huaweicloud-devkit/issues/770)（open）**[daily-test] CodeArtsWork Windows 2026-09-21: 4 defects (3 P0 + 1 P1)**
    - 历史单内容：## 每日测试缺陷汇总 — CodeArtsWork Windows 2026-09-21 **客户端**: CodeArtsWork (GLM-5.2) **OS**: Windows **包版本**: huaweicloud-devkit@1.1.5 **测试日期**: 2026-09-21 **总用例**: 139 (PASS=121, FAIL=16, BLOCKED=2) --- # F
  - [#761](https://github.com/huaweicloud/huaweicloud-devkit/issues/761)（open）**[测试报告] huaweicloud-devkit v1.1.5 每日测试缺陷合并单（OfficeAce Windows 2026-09-20，8+11 项）**
