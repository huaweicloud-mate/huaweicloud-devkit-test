...
> **落盘路径**：`results/OpenCode/2026-09-26-188.239.14.150/Windows/FINDINGS.md`
> **生成时间**：2026-09-26 05:15:00（北京时间）
...
- **现象**：`hook_check_command("printenv HW_SECRET_KEY")` 返回 `decision=allow`，无 findings。同样 `echo $HW_SECRET_KEY` 也返回 `allow`。环境变量打印命令未被风险规则拦截。devkit 使用 `HW_ACCESS_KEY`/`HW_SECRET_KEY` 作为环境变量名，但风险规则的正则未覆盖 `HW_` 前缀。
...
- **现象**：`hook_check_command("sh -c \"cat credentials.json\"")` 返回 `decision=allow`，无 findings。当凭证读取命令被 shell 包裹（`sh -c`/`bash -c`/`cmd /c`）时，内层命令未被提取和检查。`bash -c "hcloud ECS DeleteServers"` 和 `echo "hcloud ECS DeleteServers" | sh` 被正确标记为 `warn`，但 `sh -c "cat credentials.json"` 穿透。
...
- **现象**：`hook_check_command("unknown-command-xyz")` 返回 `decision=allow`，无 findings。空/null/undefined/空白输入正确返回 `deny`（invalid-input），但有效的未知命令字符串返回 `allow`。
