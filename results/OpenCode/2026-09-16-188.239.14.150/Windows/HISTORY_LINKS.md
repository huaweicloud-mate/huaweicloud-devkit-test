...
## D4-2 凭证 env 打印拦截规则未覆盖实际 HW_ACCESS_KEY/HW_SECRET_ACCESS_KEY 变量名
- 今日证据：`evidence/D4-2/probe.mjs`（D4-2 测试项，hook_check_command 返回 allow）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-4 MCP server 未强制 initialize 握手时序，initialize 前可处理 tools/call 请求
- 今日证据：`evidence/_probes/d9-protocol-probe.stdout.log`（D9-4 section: initialize前tools/call返回result，时序判定=false）
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D10-3 serviceCatalog 路由对中文 prompt 大面积 MISS（准确率 21.4%）
- 今日证据：`evidence/d8-d10-harness/eval-stdout.log` + `eval/results/eval-run-20260915230006.csv` + `evidence/EXP-E01` ~ `evidence/EXP-E14`
- **关联历史单（已作为缺陷提过，本次为复核）**：
...
## D9-9 MCP server 无取消（cancellation）能力支持
- 今日证据：`evidence/_probes/d9-protocol-probe.stdout.log`（D9-9 section: capabilities={tools:{}}，cancel通知后请求正常完成）
- **关联历史单（已作为缺陷提过，本次为复核）**：
