...
> **落盘路径**：`results/OpenCode/2026-09-23-113.44.143.91/Linux/FINDINGS.md`
> **生成时间**：2026-09-23 17:44（北京时间）
> **被测版本**：huaweicloud-devkit v1.1.6（npm latest，gitHead `46152dd`）
> **执行方式**：2026-09-23 fresh 全量重跑（OpenCode/Linux）—— 19 支源码级/协议探针 + `eval/harness/run-eval.mjs` + 真云补测（D4-13 只读子账号 / D4-14 VPC 建删归零+CTS / D3-S1/S2/C13/S4 真云场景 / D3-S6 FunctionGraph 建删归零 / D3-S3 沙箱预览）+ D2-26/D4-27 源码级直调探针
> **提单说明**：SUT 为 v1.1.6@46152dd，缺陷实测复现、根因未变；经上游 open issue 查重命中已跟踪缺陷单，不重复开单（`file_issue.py` 内置历史查重 → HISTORY_LINKS.md）。
...
- **现象**：隔离 HOME 执行 `install --target opencode` 后，全目录无 `huawei-agent-rules.md` 任何产物。
...
## #10【P1】D10-3/D3-S1/D3-S5 serviceCatalog 中文意图路由未命中
...
- **影响**：中文用户意图路由准确率 21.4% < 90%，不达标；场景级「查 ECS/复合意图」第一步即 miss。
- **证据**：`evidence/eval-harness.txt`、`evidence/fresh-matrix.txt`、`evidence/D10-3/stdout.log`、`evidence/D3-S1/stdout.txt`、`evidence/D3-S5/stdout.txt`
- **状态**：复现（历史跟踪）
...
## #13【P1】D3-S3 场景-沙箱预览出URL：公网 URL 不可达

- **现象**：沙箱 `upload_project` 成功、`deploy_nginx` ok、`deploy_check` 返回 `nginx_serving=PASS`、`devbridge_tunnel=FAIL`、`tunnel_url_accessible=FAIL (no tunnel)`、`publicUrl=undefined`（score 2/4）。
- **断言**：沙箱预览场景终点应返回可访问公网 URL（`nginx_serving=true` + `devbridge_tunnel=true` + `tunnel_url_accessible=true`）。
- **根因**：沙箱侧 DevBridge 隧道未建立（v1.1.6 迁移 DevBridge 0.2.x 后 CLI 沙箱 connect 流未建立隧道出公网 URL）。
- **影响**：场景「本地网页出公网预览 URL」终点不可达。
- **证据**：`evidence/D3-S3/stdout.txt`、`evidence/realcloud-s3-d414.mjs`
- **状态**：复现（待提单，需沙箱服务侧定位）

## #14【测试侧】D3-C4 / EXP-C4-14(DMS) / EXP-C4-18(DEW) list_operations 返回 unsupported
...
## #15【P2】D4-25 Python hook 事件遥测分类：写操作落 cli:invoke
...
## #16【P2】D4-26 findings 证据脱敏：evidence 明文泄漏
...
- **根因**：`plugins/huaweicloud-core/src/risk-rule-engine.mjs:97` `evidence: excerpt(context.text)` 未做 secret 脱敏。
- **影响**：危险命令的审计证据里夹带明文凭证。
- **证据**：`evidence/D4-26/stdout.txt`、`evidence/fresh-newcases.txt`
- **状态**：复现（待查重）

## #17【P2】D8-9 安装 ID 与遥测值脱敏：sanitizeValue 未脱敏敏感值（SPEC-MISMATCH）
...
## #18【P2】D1-68 图标离线/区域环境变量：HW_REGION 优先于 HUAWEICLOUD_REGION（SPEC-MISMATCH）
...
## #19【P1】EXP-E01~E14 中文意图路由 miss（展开级 12 条，同 D10-3）
...
- **证据**：`evidence/eval-harness.txt`、`evidence/EXP-E*/stdout.log`
- **状态**：复现（历史跟踪）

## #20【非产品缺陷】D1-39 Windows 专属用例本机无 Windows 环境

- **现象**：Windows 升级检测链（`.cmd`/EINVAL 语义）用例 D1-39 无法在本机 Linux 执行。
- **说明**：本机仅 Linux + OpenCode；D1-39 标 NOT_RUN（OS 专属，OS 列标注「专属」），Linux 侧由展开级通用断言覆盖。非产品缺陷。
