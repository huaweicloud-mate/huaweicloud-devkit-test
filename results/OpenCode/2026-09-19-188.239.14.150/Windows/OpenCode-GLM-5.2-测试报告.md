...
> **生成时间**：2026-09-19 05:10:00（北京时间）
> **执行归档**：`results/OpenCode/2026-09-19-188.239.14.150/Windows/`
...
| OS / 架构 | `Windows` (win32 x64, Windows Server 2022) |
...
| P0 / P1 / P2 新增缺陷 | `0 / 0 / 0`（2 项 P0 为历史已知缺陷，EXP-E MISS 为已知基线，不计新增缺陷） |
| 红线（I 类）违规 | `0` |
| 资源释放 | `全部归零（本轮未创建真云资源）` |
...
|---|---|---|---|---|---|---|---|---|
| 1 | P0 | `D4-16` | sh -c 命令包裹穿透：`sh -c "printenv HUAWEICLOUD_ACCESS_KEY_ID"` 绕过 env dump 拦截 | `classifyTextCommand('sh -c "printenv HUAWEICLOUD_ACCESS_KEY_ID"').decision === 'deny'` | 返回 `allow` | `safety-policy.mjs:397-426` | P | 历史已知（#730/#726/#694 等） |
| 2 | P0 | `D2-4, D4-27` | redactSecrets 不脱敏 JSON 字符串中的小写 sk 字段 | `!redactSecrets('{"sk":"SK1234567890abcdef"}').includes('SK1234567890abcdef')` | 原值未被脱敏 | `safety-policy.mjs:45` | P | 历史已知（#726/#721/#694 等） |
...
- [ ] D4-16 缺陷：`sh -c` 包裹可绕过 env dump 拦截（已记录为 P0 历史已知缺陷）
...
- 历史已知缺陷：D4-16（sh -c 包裹穿透）+ D2-4/D4-27（redactSecrets JSON 脱敏）→ 已关联历史单 #730/#726/#694 等，未重复提单（见 HISTORY_LINKS.md）
