...
// Public exposure via command classification
...
// Artifact check for public exposure in IaC
...
// Deploy plan: IAM admin policy with all 3 conditions matching
const iamJson = '{"Statement": [{"Action": "*", "Effect": "Allow"}]}';
const deployResult = evaluateDeployPlan({ plan: iamJson });
...
// Deploy plan: AdministratorAccess + Effect=Allow
const deployResult2 = evaluateDeployPlan({ plan: 'IAM role AdministratorAccess Effect=Allow' });
...
// Deploy plan: OBS anonymous write
const deployResult3 = evaluateDeployPlan({ plan: 'OBS bucket Principal="*" with PutObject FULL_CONTROL' });
