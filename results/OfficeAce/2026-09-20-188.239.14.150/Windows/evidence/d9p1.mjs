// AI生成
/**
 * D9协议 P1 综合探针 (Node.js)
 */
import { spawn } from 'node:child_process';
import { writeFileSync, mkdirSync, existsSync, readFileSync, readdirSync } from 'node:fs';
import { resolve } from 'node:path';

const EV_DIR = resolve(import.meta.dirname);
const MCP = 'C:/Users/Administrator/devkit-test/OfficeAce/hdk/plugins/huaweicloud-core/src/mcp-server.mjs';
const ENV = { ...process.env, HUAWEICLOUD_HOME: 'C:/Users/Administrator/devkit-test/.hc_test' };
const results = {};
let idc = 1;
const nid = () => idc++;
...
class Mcp {
  ...
  start() {
    ...
    this.proc = spawn('node', args, { stdio: ['pipe', 'pipe', 'pipe'], env: ENV });
  ...
  _parse() {
    while (true) {
      ...
      if (he !== -1) {
        const hdr = this.buf.slice(0, he);
        const m = hdr.match(/Content-Length:\s*(\d+)/i);
        if (!m) { this.buf = this.buf.slice(he + 4); continue; }
        const len = parseInt(m[1]);
        const bs = he + 4, be = bs + len;
        if (this.buf.length < be) break;
        const body = this.buf.slice(bs, be);
        this.buf = this.buf.slice(be);
        try {
      ...
      if (lf !== -1) {
        const line = this.buf.slice(0, lf).trim();
  ...
  _dispatch(r) {
    const w = this.waiters.find(w => w.id === r.id);
    if (w) { w.resolve(r); this.waiters = this.waiters.filter(x => x !== w); }
  ...
  wait(id, timeout = 20000) {
    return new Promise((resolve, reject) => {
      const existing = this.queue.find(r => r.id === id);
      if (existing) { this.queue = this.queue.filter(r => r !== existing); resolve(existing); return; }
      const timer = setTimeout(() => { this.waiters = this.waiters.filter(w => w !== w2); reject(new Error(`Timeout id=${id}`)); }, timeout);
      const w2 = { id, resolve: (r) => { clearTimeout(timer); resolve(r); }, reject };
      this.waiters.push(w2);
    });
  }
  async httpPost(msg, timeout = 10000) {
    const url = `http://${this.opts.host}:${this.opts.port}`;
    const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(msg) });
    return resp.json();
  }
  close() {
...
async function d9_1() {
  ...
  try {
    await init(c);
    const id = nid();
    c.send({ jsonrpc: '2.0', id, method: 'tools/list', params: {} });
...
async function d9_3() {
  ...
  try {
    ...
    const r1 = await c.wait(id1, 25000);
...
async function d9_4() {
  ...
  try {
    ...
    const cr = await c.wait(cid, 25000);
