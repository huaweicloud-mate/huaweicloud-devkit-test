...
## 场景级回归（D3-S 系列，2026-09-19 补）

推广用的「提示语级端到端」链路，能力级用例（单工具/单函数）照不到，补 6 条场景用例，覆盖 4 种链路形态：

| 用例 | 场景 | 链路形态 | 优先级 |
|---|---|---|---|
| D3-S1 | 只读查 ECS（带「不改」约束） | A 只读链路 | P1 |
| D3-S2 | 删 VPC 先确认 | B 写操作安全链路 | P1 |
| D3-S3 | 沙箱预览出 URL | C 多服务编排链路 | P1 |
| D3-S4 | 领券闭环 | D 激励状态闭环 | P1 |
